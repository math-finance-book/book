...
in other words, the discounted value of a non-dividend paying investment or any dividend-reinvested strategy is a martingale.
...
and the risk-neutral measure is one which makes $B_t + \frac{\mu - r}{\sigma}t$ a Brownian motion. Girsanov's Theorem tells us how to change the probability measure so that $B_t + \frac{\mu - r}{\sigma}t$ is a Brownian motion. Define $\kappa = \frac{\mu - r}{\sigma}$. The term $\kappa$ is called the price of risk, or Sharpe ratio.

::: Principle
Define 
$$Z_t = \exp\left(-\frac{1}{2}\kappa^2 t - \kappa B_t \right).$$
Because $Z_T$ is lognormal, $\mathbb{E}[Z_T]=1$. Then under the probability measure defined for any $Y$ which is a function of $B_t;~0\le t \le T$ by
...
This is a simple version of Girsanov's Theorem. Given the original measure under which $B_t$ is a Brownian motion, Girsanov's Theorem tells us how to construct the risk-neutral measure. In our leading case, we use Ito's Lemma to show that $Z_t \frac{S(t)}{R(t)}$ is a martingale. Alternatively, using our expectations of lognormal random variables we can see, recalling $\kappa = \frac{\mu-r}{\sigma}$,
$$Z_t \frac{S(t)}{R(t)}= S(0) \exp\left(\left(\mu -r -\frac{1}{2}\kappa^2 \right)t +(\sigma - \kappa)B_t \right) = S(0) \exp\left(-\frac{1}{2}(\sigma - \kappa)^2 t + (\sigma - \kappa)B_t \right) $$
so 
...
::: Extra
To illustrate why this form of the Theorem holds, consider the special case where we compute $\mathbb{E}[Z_T f(B_T)]$. The distribution of a Brownian motion at time $T$ has mean zero and standard deviation $\sqrt{T}$. The density is
...
but under the probability $P^{\kappa}$ defined by the density 
$$\frac{1}{\sqrt{2 \pi T}}\exp\left(-\frac{1}{2} \frac{(x+\kappa T)^2}{T} \right),$$
$B_T$ has mean $-\kappa T$. Adding $\kappa T$ to this expression gives $B_T+\kappa T$ has zero mean and standard deviation $\sqrt{T}$ and we conclude $B_t + \kappa t$ is a Brownian motion.
:::
...
We can also use Ito's Lemma to show $m_t S(t)$ is an Ito process without drift in our original probability measure:
...
$$ dS(t) = \left(r + \sigma^2 \right) S(t) \mathrm{d} t + \sigma S(t) \mathrm{d} B_t^S. $$

Alternatively, we can use Girsanov's Theorem. In this case, we solve for $\kappa$ which makes $Z_t \frac{R(t)}{S(t)}$ into a martingale in the original measure. Using the product rule from Ito's Lemma,
...
::: Rule
If $S$ is the price of a non-dividend paying asset, then
$$
...
$$\{ S(T) \ge K\} = \{\log(S(T)) \ge \log(K)\} = \{\frac{\log\left(\frac{S(0)}{K}\right) + \left(r - \frac{\sigma^2}{2} \right) T}{\sigma \sqrt{T}} \ge - \frac{B_T}{\sqrt{T}} \} ,$$
and $-\frac{B_T}{\sqrt{T}}$ is distributed according to a standard normal random variable (mean 0 and standard deviation 1). Therefore, the value of this digital option is
$$ e^{- r T} N(d_2)$$
where $d_2= \frac{\log\left(\frac{S(0)}{K}\right) + \left(r - \frac{\sigma^2}{2} \right) T}{\sigma \sqrt{T}}$ and $N(d)$ is the probability that a standard normal random variable is less than or equal to $d$; that is
$$N(d)= \frac{1}{\sqrt{2 \pi}} \int_{-\infty}^d e^{-\frac{x^2}{2}} dx. $$

The payoff $S(T) {\bf{1}}_{\{S(T) \ge K\}}$ can, in principle, be found by taking the risk-neutral expectation of the discounted payoff:
$$\mathbb{E}^R\left[e^{-rT} S(T) {\bf{1}}_{\{S(T) \ge K\}}\right].$$
However, it is easier to use the stock as numeraire:
$$S(0) \mathbb{E}^S\left[ {\bf{1}}_{\{S(T) \ge K\}}\right]$$
Then since
$$ \mathrm{d} S(t) = (r+ \sigma^2) S(t) \mathrm{d} t + \sigma S(t) \mathrm{d} B_t^S $$
where $B_t^S$ is a Brownian motion in the measure $P^S$. But finding the expectation of the indicator is now the same exercise as before. By writing the explicit solution for $S(T)$, the price of the payoff is found to be 
$$S(0) N(d_1)$$
where $d_1= \frac{\log\left(\frac{S(0)}{K}\right) + \left(r + \frac{\sigma^2}{2} \right) T}{\sigma \sqrt{T}}$.
...
$$\int_0^t {\bf{\kappa}}_s^{\prime} \mathrm{d} {\bf{B}}_s = \sum_{i=1}^d \int_0^t \kappa_{i,s} \mathrm{d} B_{i,s} $$
...
$B_{i,t}^\kappa = B_{i,t} + \int_0^t \kappa_{i,s} \mathrm{d} s$ is a Brownian motion for $0 \le t \le T$. Moreover, if $M_t$ is a martingale under $P^\kappa$ then $Z_t M_t$ is a martingale under the original probability measure.
:::
...
Let $V$ be the price of another asset that does not pay dividends. Let $r_t$ denote the instantaneous risk-free rate at date $t$ and let $R_t = \exp\left(\int_0^t r_s\mathrm{d} s\right)$. Assume
\begin{align*}
...
is a martingale and 
$$\frac{V(t)}{R(t)} = \exp\left(-\int_0^t r_s\mathrm{d} s\right) V(t) $$
is a martingale.
...
$$ {#eq-riskneutral12}
where $B_{1,t}^R$, $B_{2,t}^R$, and $W_t^R = \rho_t B_{1,t}^R + \sqrt{1-\rho_t^2} B_{2,t}^R$ are Brownian motions under the risk-neutral probability. 
:::



We can use Girsanov's Theorem to solve for $\kappa$. We should have $Z_t \frac{S(t)}{R(t)}$ is a martingale and $Z_t \frac{V(t)}{R(t)}$ is a martingale. Therefore, using the product rule from Ito's Lemma
...
$$\mathrm{d} Z_t \frac{V(t)}{R(t)} = (\theta_t-r_t ) Z_t \frac{V(t)}{R(t)} \mathrm{d} t + (\rho_t \phi_t - \kappa_{1,t} \rho_t) Z_t \frac{V(t)}{R(t)} \mathrm{d} B_{1,t} + (\sqrt{1-\rho_t^2} \phi_t - \kappa_{2,t}\sqrt{1-\rho_t^2}) Z_t \frac{V(t)}{R(t)} \mathrm{d} B_{2,t}+ \mathrm{d} <Z,\frac{V}{R}>_t $$
...
$$\frac{\mathrm{d} Y}{Y} =r_t\mathrm{d} t - \frac{\mathrm{d} S}{S} + \left(\frac{\mathrm{d} S}{S}\right)^2 = (r_t + \sigma_t^2)\mathrm{d} t - \frac{\mathrm{d} S}{S}\; .$$
Because the drift of $\mathrm{d} Y/Y$ must be zero, this implies that the drift of $\mathrm{d} S/S$ is $(r_t + \sigma_t^2)\mathrm{d} t$.
...
$$ \frac{\mathrm{d} V}{V} = \left(r_t + \rho_t \phi_t \sigma_t \right) \mathrm{d} t + \phi_t \mathrm{d} W^S $$
We conclude that:

::: Rule
If $S$ is the numeraire, then
$$
\frac{\mathrm{d} S}{S} = (r_t + \sigma_t^2)\mathrm{d} t+\sigma_t\mathrm{d} B^S\;,
$$ {#eq-own11}
$$ \frac{\mathrm{d} V}{V} = \left(r_t + \rho_t \phi_t \sigma_t \right) \mathrm{d} t + \phi_t \mathrm{d} W^S $$
...
We can use our previous example to compute the dynamics of $S$ and $V$ when $V$ is the numeraire. We conclude that:

::: Rule
If $V$ is the numeraire, then
$$
\frac{\mathrm{d} S}{S} = (r_t + \rho_t\sigma_t\phi_t)\mathrm{d} t+\sigma_t\mathrm{d} B_{1,t}^V\;,
$$ {#eq-other11}
$$ \frac{\mathrm{d} V}{V} = \left(r_t + \phi_t^2 \right) \mathrm{d} t + \phi_t \mathrm{d} W^V $$

where $B^V$ and $W^V$ denote Brownian motions under the probability measure corresponding to the dividend-reinvested risky asset $V$ being the numeraire, and where $\rho$ is the correlation of $S$ and $V$.
:::


Notice that @eq-other11, while more complicated, is also more general than the others. In fact, it includes @eq-riskneutral11 and @eq-own11 as special cases: (i) if $V$ is the price of the instantaneously risk-free asset, then $\phi=0$ and @eq-other11 simplifies to @eq-riskneutral11, and (ii) if $V=S$, then $\phi=\sigma$ and $\rho=1$, so @eq-other11 simplifies to @eq-own11.
...
::: 
::: {#exr-girsanov2}
Calculate the price of the digital option that pays ${\bf{1}}_{\{S(T) \le K\}}$ using the assumptions of the Black-Scholes model.
:::
::: {#exr-girsanov3}
Calculate the price of the digital option that pays $S(T) {\bf{1}}_{\{S(T) \le K\}}$ using the assumptions of the Black-Scholes model.
:::
::: {#exr-girsanov4}
Suppose $S$ and $V$ are as in @sec-s:girsanov with constant coefficients. Find the value of an option which pays $V(T) {\bf{1}}_{\{V(T) \ge S(T)\}}$. Hint: use $V$ as a numeraire.
:::