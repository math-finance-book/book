...
However, when we consider binomial models for multiple assets in @sec-c_montecarlo, we will use the tree proposed by Trigeorgis [@Trigeorgis], \index{Trigeorgis model} because it is the simplest to explain in that context.
...
the previous program as our binomial model. The number of time steps $n$ must be even so $n/2$ is an integer.
...
```
  ...
  <!-- To estimate the option delta as a difference ratio $(C_u-C_d)/(S_u-S_d)$, we can use the perturbations $S_u = 1.01S$ and $S_d = 0.99S$.  The inputs are the same as for the Black-Scholes formula plus the sample size $M$ (the number of stock prices to be simulated).  The reader is asked to perform this analysis in an exercise in the end of chapter exercises. -->
