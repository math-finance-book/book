...
When a trade is made on an exchange, the exchange clearinghouse becomes the counterparty to both side.  For example, anyone who is long a call option has an option to buy the underlying asset at the strike *from the clearinghouse*.  Simlarly, anyone who is short a call option has an obligation to deliver the underlying asset *to the clearinghouse* and receive the strike, should the clearinghouse exercise the option.  The clearinghouse exercises options when an option of the same strike and maturity is exercised against it.  This means that the underlying asset is passed through the clearinghouse from the short party to the long party when the long party exercises.  The strike is passed through in the opposite direction.  The short party against whom the clearinghouse exercises is chosen randomly amonth the people who are short that particular option.  This procedure provides the long party assurance that the option will be honored, which would otherwise be impossible with anonymous trade.  It also makes it possible for contracts to be cancelled by making offsetting trades with a different counterparty than the counterparty to the original trade, 

::: Example
Suppose the first trade is person B buying and person S selling a single contract.  Then, open interest is 1 contract.  Suppose the second trade is person S buying and person R selling.  Then, open interest is still 1 contract, with B being long and R short.  Person S has a gain or loss depending on the trade prices but no longer has a position in the option.  If the next trade is person C buying a contract and person Q selling, then open interest is 2 contracts, with B and C being long a single contract each  and R and Q being short a single contract each.  If now B decides to exercise her option, the clearinghouse will randomly choose either R or Q to exercise against, and the underlying asset will be transferred from R or Q to B, and the strike paid by B will be delivered to R or Q.
After the option exercise, open interest is 1 contract.
:::
