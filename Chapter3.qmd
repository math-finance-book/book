...
In Sect.~\ref{s_girsanov}---see  @eq-riskneutral11---we learned that under the 
...
From Sect.~\ref{s_reinvestingdividends}, we know that $V(t)=\mathrm{e}^{qt}S(t)$ is the price of a non-dividend-paying portfolio.  From our fundamental pricing formula @eq-formula, using $V$ as the numeraire, we have
\begin{align*}
...
\hline\mathrm{n}oalign{\smallskip}
\bfseries{Input}  \bfseries{Input Symbol}  \qquad \bfseries{Greek}  \qquad  \bfseries{Greek Symbol}
$$
$$
\mathrm{n}oalign{\smallskip}\hline\mathrm{n}oalign{\smallskip}
...
$$
$$
\mathrm{n}oalign{\smallskip}\hline
\end{tabular}
\end{table}
...
$$
\mathrm{d} C = \frac{\partial C}{\partial S}\,\mathrm{d} S + \frac{\partial C}{\partial t}\,\mathrm{d} t + \frac{1}{2}\frac{\partial^2C}{\partial S^2} (\mathrm{d} S)^2 \mathrm{n}otag 
$$
$$
...
The gamma of the stock is zero ($\mathrm{d
$$^2 S/\mathrm{d} S^2 = d\,1/\mathrm{d} S = 0$), so to obtain a zero portfolio gamma we need}
0 = - \Gamma + b\Gamma'\;. 
$$ {#eq-portfoliogamma}
...
::: {#exr-problem4.1}
  Create an Excel worksheet in which the user inputs $K$, $r$, $\sigma$, $q$ and $T$.  Compute the delta of a call option 
...
::: {#exr-nolabel}
 Repeat Prob.~\ref{problem4.1} for the gamma of a call option.
:::
::: {#exr-nolabel}
...
:::
::: {#exr-nolabel}
 Create an Excel worksheet in which the user inputs $K$, $r$, $q$ and $T$.  Compute the value of an at-the-money call option ($S=K$) using the function  \verb!Black_Scholes_Call! for volatilities $\sigma = .01, .02, \ldots, 1.0$.  Plot the call value against the volatility.
:::
::: {#exr-nolabel}
...
$$
\max\left(D,\frac{D\mathrm{e}^{(q-\alpha)T}S(T)}{S(0)}\right) = D + \max\left(0, \frac{D\mathrm{e}^{(q-\alpha)T}S(T)}{S(0)}-D\right)\mathrm{n}otag
$$
$$
