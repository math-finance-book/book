...
::: {#exr-exercise121}
  Modify the function \verb!MarketModel_Cap! so that rather than taking \verb!P0! and the vector \verb!P! of discount bond prices as inputs, it looks up discount bond prices from a function \verb!DiscountBondPrice! that returns a discount bond price for any maturity.  For example, \verb!DiscountBondPrice! might be based on a cubic spline fit to the yield curve as discussed in @sec-s_yieldcurve.  To test the new function \verb!MarketModel_Cap!, you will need to create a test function \verb!DiscountBondPrice!.  For example, you could use the following, which corresponds to a rather steeply increasing yield curve, especially at the short end.
