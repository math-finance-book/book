...
The binomial model converges to the continuous-time limit as $N \to \infty$, but convergence is not monotonic. For European options:
...
As $N \to \infty$, the binomial model converges to the Black-Scholes model (see @sec-c:blackscholes_updated), which provides the analytical solution for European options in the continuous-time setting.
