# Asset Pricing Basics {#sec-basics}
...
@sec-continuoustime. We will begin with a brief description of
...
## Longs, Shorts, and Margin {#sec-c1-longs-shorts-and-margin}
...
assets (\$1000 of stock).  If the value of the stock drops sufficiently
...
## Calls and Puts {#sec-c1-calls-and-puts}
...
## Exercise Policies for Calls and Puts {#sec-c1-exercise-policies-for-calls-and-puts}
...
## Compounding Interest {#sec-c1-continuouscompounding}

During most of the first two parts of the book (the only exception being
Chapter @sec-forwardexchange. we will assume there is a risk-free
...
$\mathrm{e}^r = 1+r_a$. More generally, an investment of $x$ dollars for a time
...
$x\mathrm{e}^{rT}$ dollars at the end of the time period.
...
that the investment will grow in six months to $x\mathrm{e}^{0.5 r}$ and that it
will grow in one year to $x\mathrm{e}^r$. We can interpret this rate as being
...
the interest earned in "an instant $d t$" is $x(t)r\,\mathrm{d} t$ dollars.
...
$\mathrm{d} x(t) = x(t)r\,\mathrm{d} t$. The real meaning of this equation is that
...
$x(T) = x(0)\mathrm{e}^{rT}$. Thus, the statement that "the interest earned in
an instant $\mathrm{d} t$ is $x(t)r\,\mathrm{d} t$" is equivalent to the statement that
...
instant $\mathrm{d} t$ equal to $x(t)r(t)\,\mathrm{d} t$. Consider again an investment
...
$$x(t) = x(0)\exp\left(\int_0^t r(s)\,\mathrm{d}s\right)\;\;.$$
The expression $\int_0^t r(s)\,\mathrm{d}s$ can be interpreted as a continuous
...
then $\int_0^t r(s)\,\mathrm{d}s = rt$ and our compounding factor
$\exp\left(\int_0^t r(s)\,\mathrm{d}s\right)$ is $\mathrm{e}^{rt}$ as before.

## State Prices in a One-Period Binomial Model {#sec-c1-oneperiodbinomial}
...
\eqref{C0} in terms of "state
prices." By substituting for $\delta$ in
\eqref{C0}, we can rearrange
it as 
...
\begin{equation}\label{binomialS1}
...
\end{equation}
It is convenient to denote the factors appearing in these
equations as 
...
\eqref{binomialC1}--\eqref{binomialR1} as 
...
These equations have the following interpretation: the
...
(@eq0binomialR2) by considering an investment of \$1 today in the
risk-free asset---it has value 1 today and will have value $\mathrm{e}^{rT}$ in
...
\eqref{binomialC2}--\eqref{binomialR2} show that  $\pi_u$ is the price of the first
Arrow security and $\pi_d$ is the price of the second. For example, the
right-hand side of \eqref{binomialS2} is the value of the stock at date 0 viewed as a
...
\eqref{binomialnoarbitrage}. Thus, we conclude that **in the
...
\eqref{binomialC2}--\eqref{binomialR2} in terms of expectations. The expectation (or
...
The risk-neutral probabilities are defined as $\pi_u\mathrm{e}^{rT}$ for the up
state and $\pi_d\mathrm{e}^{rT}$ for the down state. Denoting these as $p_u$ and
$p_d$ respectively, \eqref{binomialC2}--\eqref{binomialR2} can be written as 
...
The numbers $p_u$ and $p_d$ are both positive (because the
state prices are positive under our no-arbitrage assumption) and
\eqref{binomialR3} states that they sum to one, so it is indeed
sensible to consider them as probabilities. Equations
\eqref{binomialC3} and
\eqref{binomialS3} state that the value of a security today is its
...
Equations \eqref{binomialC3} and
\eqref{binomialS3} can be written in an equivalent form, which,
...
Considering an investment of \$1 today which grows to $\mathrm{e}^{rT}$ at date
$T$, it is sensible to take the price today to be $R=1$ and the price in
the up and down states at date $T$ to be $R_u=R_d=\mathrm{e}^{rT}$.[^1] In terms
of this notation, (@dq-binomialC3)--\eqref{binomialR3} can be written as:

[^1]: All of the equations appearing below will also be true if instead
    we take $R=\mathrm{e}^{-rT}$ and $R_u=R_d=1$.
...
Each of equations
\eqref{binomialC4} and
\eqref{binomialS4} states that the price of a security today
...
$\pi_d$ in \eqref{binomialC2}--\eqref{binomialR2} and continuing to use the notation $R$ for the
...
\end{align}
Equation
\eqref{binomialS5} establishes that we can view the $q$'s as
...
[^1]: We have applied this statement to the risk-free asset, which pays
    dividends (interest). However, the price $R_u=R_d=\mathrm{e}^{rT}$ includes
...
[^1]: We have proven this in the binomial model, but we will not prove
    ...
    \eqref{binomialnoarbitrage} that is sufficient in the binomial
    model.
...
E[1_A\phi(T)\mathrm{e}^{rT}]$$ will be called the "risk-neutral probability
...
as a superscript on the expectation symbol: for example, $\mathrm{e}^S$ will
...
\mathrm{e}^S[X] &= E\left[X\phi(T)\frac{S(T)}{S(0)}\right]\;.
end{aligned}$$
...
[\[probSnumeraire\]](#probSnumeraire){reference-type="eqref"
reference="probSnumeraire"}. At each date $t<T$, let $\mathrm{e}^S_t$ denote the
...
subscript; i.e., $\mathrm{e}^S$ has the same meaning as $\mathrm{e}^S_0$). Let $Y$ denote
the price of another non-dividend-paying asset. We will show that

$$\label{preformula}
\frac{Y(t)}{S(t)} = \mathrm{e}^S_t \left[\frac{Y(T)}{S(T)}\right]\;.$$ Thus, the
...
$$\label{formula}
Y(t) = S(t)\mathrm{e}^S_t \left[\frac{Y(T)}{S(T)}\right]\;.$$
...
Y(t) = \text{num}(t)\mathrm{e}^\text{num}_t \left[\frac{Y(T)}{\text{num}(T)}\right]\;,\tag{\ref{formula}$'$}$$
where now $\text{num}(t)$ denotes the price of the (non-dividend-paying)
numeraire asset at time $t$.

For example, letting $R(t)$ denote the value $\mathrm{e}^{rt}$ of the risk-free
asset and using it as the numeraire, equation
[\[formula\]](#formula){reference-type="eqref" reference="formula"}
becomes $$\label{riskneutralformula}
Y(t) = \mathrm{e}^{rt}\mathrm{e}^R_t\left[\frac{Y(T)}{\mathrm{e}^{rT}}\right] = \mathrm{e}^{-r(T-t)}\mathrm{e}^R_t [Y(T)]\;,$$
...
$$\mathrm{e}^S\left[1_A\left(\frac{Y(T)}{S(T)}-\frac{Y(t)}{S(t)}\right)\right]=0\;.$$
...
$$\mathrm{e}^S_t\left[\frac{Y(T)}{S(T)}-\frac{Y(t)}{S(t)}\right]=0\; ,$$
or, equivalently
$$\mathrm{e}^S_t\left[\frac{Y(T)}{S(T)}\right] = \frac{Y(t)}{S(t)}\;.$$
end{petit}

## Introduction to Option Pricing {#sec-c1-introoptions}
...
Chapter @sec-blackscholes, we will call the first contract a "share
...
## Pricing Share Digitals {#sec-c1-pricing-share-digitals}
...
[\[formula\]](#formula){reference-type="eqref" reference="formula"}
tells us that
$$Y(0) = \text{num}(0) \mathrm{e}^\text{num} \left[\frac{Y(T)}{\text{num}(T)}\right]\; ,$$
...
$$\mathrm{e}^\text{num} \left[\frac{Y(T)}{\text{num}(T)}\right] = \text{prob}^S\big(S(T)\geq K\big)\; ,$$
...
holds for any non-dividend-paying asset, so it holds for $R(t)=\mathrm{e}^{rt}$,
...
Chapter @sec-blackscholes for the Black-Scholes model.

## Pricing Digitals {#sec-c1-pricing-digitals}
...
$$Y(0) = \mathrm{e}^{-rT}\mathrm{e}^R[Y(T)] = \mathrm{e}^{-rT}K\times \text{prob}^R \big(S(T)\geq K\big)\; ,$$
so we need to compute the risk-neutral probability that $S(T) \geq K$.
We will do this by using the fact that $S(t)/R(t) = \mathrm{e}^{-rt}S(t)$ is a
...
## An Incomplete Markets Example {#sec-c1-incomplete}
...
\eqref{binomialnoarbitrage}{. State prices $\pi_u$, $\pi_m$ and
$\pi_m$ must satisfy equations analogous to
\eqref{binomialS2}--\eqref{binomialR2}; specifically,
...
$\pi_d$, as shown in
\eqref{binomialstateprices}. However, in the trinomial case, we
have only two equations in three unknowns. Thus, there exist many
solutions.

Given any particular solution $(\pi_u,\pi_m,\pi_d)$ of
\eqref{binomialSR10combined}), we can define the risk-neutral
probabilities $p_u$, $p_m$ and $p_d$ as before---e.g.,
$p_u = \pi_u\mathrm{e}^{rT}$. Likewise, we can define the probabilities using
...
portfolio at date $T$ will be $a\mathrm{e}^{rT} + bS_x/S$, where
...
\eqref{binomialSR10combined}) and assume that the market uses that
...
Chapter @sec-stochasticvolatility.

##  Exercises {#sec-c1-problems}
