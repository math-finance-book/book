...
<iframe width="720" height="800" src="https://wily-jolie-derivative-securities-5972205c.koyeb.app/"></iframe>

**Enter parameters below to generate binomial trees for an underlying asset price and an option.** The left sidebar has user controls. If it is not visible, use the <i class="bi bi-chevron-right"></i> icon at the top of the figure to open it.  The interest rate should be the rate per period and should be adjusted based on the number of periods.  The "up move" parameter corresponds to the parameter $u$ in the text as $u=1+$ "up move."  The figure takes $d=1/u$.  Notice that the vertical axis is reverse ordered for put options, because higher stock prices produce lower put values.
