...
By taking the Ito differential of $r(s)$ in the following, one can verify that it is the solution of the Vasicek model @eq-vasicek:^[@eq-vasicek_sol implies the distributional properties of $r(s)$ stated earlier.
...
@eq-vasicekbond and Ito's formula (the rule that, if $P=\mathrm{e}^X$, then $\mathrm{d} P/P = dX + (dX)^2/2$) imply that for any date $t$ and any fixed maturity date $u>t$, we have
$$
