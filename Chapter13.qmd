...
According to the @eq-vasicekbondreturn, the change in the value of the short position in the two-year bond  will be
...
In the case $\kappa>0$, one can interpret the volatility of the coupon bond similarly.  The analogue to duration for this model is the weighted average of the function $b(\tau_j)$ of the times to maturity, as the @eq-vasicekcouponreturn shows.  The weights again are the fractions of the bond value that each cash flow contributes.
...
volatility as in @sec-s_timevaryingvolatility and @sec-s_volatility.  Specifically,
...
The volatility $\sigma_{\text{avg}}$ is defined as $\sigma_{\text{avg}} = (u-T)\sigma$ in the continuous-time Ho-Lee model and according to the @eq-vasiceksigmaavg in the Hull-White model.
...
of the discount bond corresponding to the caplet with payment date $t_{i+1}$ has a non-random volatility, and this is the assumption on which  @eq-vasicekfloorlet - @eq-vasicekcaplet is based.  In @sec-s_valuingcaps, we assumed the forward LIBOR rate had a non-random volatility.  The definition~@eq-forward2 of the forward rate is
...
Thus, based on the @eq-extendedvasicekbond for the discount bond prices, we define~$r^*$ by
