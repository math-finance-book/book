...
for constants $\kappa\geq 0$, $\theta$, and $\sigma$, where $B$ is a Brownian motion under the risk-neutral measure.  In this model, $\theta$ is the long-run mean of the short rate process and $\kappa$ is interpreted as the rate of mean reversion.  \index{mean reversion}When $r(t) >\theta$, the drift term will be negative and push $r$ down towards $\theta$.  Likewise, when $r(t) <\theta$, the drift term will be positive and push $r$ up towards~$\theta$.  The rate at which $r$ drifts towards $\theta$ is obviously determined by $\kappa$.  **We are going to depart from our convention and call $\sigma$ the volatility of the short rate** \index{volatility}even though $\sigma^2\,\mathrm{d} t$ is the instantaneous variance of $\mathrm{d} r$ rather than $\mathrm{d} r/r$.
...
\section{Yields and Yield Volatilities}
...
\section{The General Hull-White Model}
