...
$$\mathrm{d} \log r_t = -\frac{\mathrm{d} \log \sigma_t}{\mathrm{d} t}[\theta_t-\log r_t]\,\mathrm{d} t + \sigma_t\,\mathrm{d} B_t\; ,$$
...
$$
P(t,u) = \exp\left(-\int_t^u \phi(s)\,\mathrm{d} s\right)\exp\left(-a(\tau)-b(\tau)\hat{r}_t\right)\;.
$$ {#eq-cirPcal}

In particular, discount bond prices at date $0$ are
$$P(0,u) = \exp\left(-\int_0^u \phi(s)\,\mathrm{d} s\right)\exp\left(-a(u)-b(u)r_0\right)\; ,$$
so the model can be calibrated to market prices $P^{\text{mkt}}(0,u)$ by setting
$$
\exp\left(-\int_0^u \phi(s)\,\mathrm{d} s\right) = \exp\left(a(u)+b(u)r_0\right)P^{\text{mkt}}(0,u)
$$ {#eq-circalibration}
...
$$
P(T,u)  = \exp\left(-\int_T^u \phi(s)\,\mathrm{d} s-a(u-T)-b(u-T)\hat{r}_T\right)\; .
$$
Therefore, the option will finish in the money if and only if
$$
\frac{-\int_T^u \phi(s)\,\mathrm{d} s-a(u-T)-\log K}{b(u-T)} > \hat{r}_T\;.
$$ {#eq-ciroptionprice2}
...
\end{align*}
Given that $Z$ is a martingale, $\mathrm{d} Z/Z$ cannot have a drift, so it must be that $B^*$ defined by $B^*_0=0$ and
$$
...
$$\left( \sigma_1\sqrt{x_1_t}\,\mathrm{d} B_1_t +  \sigma_2\sqrt{x_2_t}\,\mathrm{d} B_2_t\right)^2 = \left(\sigma_1^2x_1_t + \sigma_2^2x_2_t\right)\,\mathrm{d} t\; .
$$
...
$$
\\E_t\left[\exp\left(-\int_t^u x_i(s)\,\mathrm{d} s\right)\right] = \exp\left(-a_i(\tau)-b_i(\tau)x_i_t\right)\;,
$$ {#eq-lsdiscount}
...
::: Exercise
Modify the function in the preceding exercise to create a function `CIR_Calibrated_Caplet_MC` that values a caplet in the CIR model using Monte Carlo, with the model calibrated to the market.  Look up market discount bond prices from a function such as `DiscountBondPrice` in @exr-exercise121.  To compute $\phi_{t_i}$ for $i=1,\ldots, N$ from market bond prices, use @eq-circalibration for dates $t_i$ and $t_{i+1}$ as in @sec-s_vasicek_swaptions.
:::
