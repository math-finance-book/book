...
$$
f(k)=\sum_{j=1}^{N} Exp(-i \frac{2\pi}{N}(j-1) (k-1))g(j), k=1,...,N,
$$
where $N$ is typically a power of 2 and $g(\cdot)$ is a function including weights. The algorithm reduces the number of
multiplications in the required $N$ summations from an order of $N^2$ to that of
$N \log_2(N)$, a very significant reduction. We present in this section the details for writing the integration (5) as an application of the summation (16).

To apply the FFT, we discretize the integral @eq-carrmadan over $u$:

- Define $u_j = \eta j$, where $\eta$ is the spacing between points and $j = 0, 1, \dots, N - 1$.
- Similarly, define $k_m = -b + \lambda \, m$, where 
$$
...
b=\frac{N \lambda}{2} 
$$
is the integration upper limit of $k$, and $m = 0, \dots, N-1$.

The integral @eq-carrmadan becomes a sum:

$$
\tilde{C}(k_m) \approx \frac{e^{-\alpha k_m}}{\pi} \sum_{j=0}^{N-1} e^{-i u_j (-b))} \phi_{C}(u_j) e^{-i 2\pi j\, m / N} \eta.
$$
To further improve the efficiency, we use Simpson's rule to choose the weights of each term to arrive at 
$$
\tilde{C}(k_m) \approx \frac{e^{-\alpha k_m}}{\pi} \sum_{j=0}^{N-1} e^{-i u_j (-b))} \phi_{C}(u_j) e^{-i 2\pi j\, m / N} \frac{\eta}{3}(3+(-1)^{j+1}-\delta_j-3\delta_{j-(N-1)}),
$${#eq-carrmadanFFT}
