# Using Fourier Transform to Price Options {#sec-c_fouriertrasform}

## Introduction

The Fourier Transform has become an essential tool in option pricing, particularly for complex models like those with stochastic volatility, Lévy processes, and variance gamma processes. In this chapter, we will explore how the Fourier Transform simplifies the pricing of derivative securities, examine its advantages, and provide analytical examples and Python code for practical illustration.
...
1. **Simplify Complex Problems**: Many option pricing models lead to complex partial differential equations (PDEs). The Fourier Transform helps transform these PDEs into simpler algebraic equations.
2. **Handle Non-Gaussian Processes**: Many real-world asset price models involve jumps or stochastic volatility, which may lead to non-Gaussian distributions of returns. Fourier Transforms allow us to handle these models efficiently.
3. **Efficiency with Characteristic Functions**: The Fourier Transform is closely related to the characteristic function of the underlying asset price. Using the characteristic function simplifies the integration required in risk-neutral valuation.
4. **Fast Numerical Computation**: By leveraging Fast Fourier Transform (FFT) algorithms, we can calculate option prices for a wide range of strikes and maturities quickly.

In this chapter, we'll demonstrate these advantages through theory, examples, and Python code.
...
Unfortunately, the Fast Fourier Transform  (FFT) cannot be used to evaluate the pricing integral @eq-FTpricing, since the integrand is singular at the required evaluation point $u = 0$. Given the considerable speed advantages of the FFT, we introduce the Carr-Madan method that can leverage the efficiency of  the FFT.

## The Carr-Madan Method

The key idea is to express the option price as an integral in Fourier space and then discretize it for the FFT numerical evaluation.


### Step 1: Modify the Payoff Function

To ensure the square integrability of the Fourier Transform, we first modify the option's payoff function by introducing a dampening factor $\alpha > 0$:

$$
\tilde{C}(K) = e^{\alpha k} C(K)=,
$$

where $k = \ln K$ is the log-strike price. This modification ensures that the transformed function decays sufficiently fast at infinity.^[With $\alpha=0$, when $K$ approaches $0$, $k$ approaches $\infty$ and $C(K)$ approaches $S$, not $0$, making $C(K)$ not square integrable in fourier transform.]
...
$$
\tilde{C}(K) = \frac{e^{-r T}}{2\pi} \int_{-\infty}^\infty e^{-i u k} \phi_{C}(u) \, du,
$$

where $\phi_{C}(u)$ is the Fourier Transform of $e^{\alpha k} (S_T - K)^+$.
...
$$
\phi_{C}(u) = \int_{-\infty}^\infty e^{i u k} e^{\alpha k} (e^{s} - e^{k})^+ \, ds,
$$
...
$$
\phi_{C}(u) = \frac{\phi_{S_T}(u - i (\alpha + 1))}{(\alpha + i u)(\alpha + i u + 1)},
$$ {#eq-carrmadan}

where $\phi_{S_T}(u)$ is the characteristic function of $\ln S_T$.

### Step 4: Discretize the Integral for FFT

We discretize the integral @eq-carrmadan over $u$ to apply the FFT:

- Define $u_j = \eta j$, where $\eta$ is the spacing between points and $j = 0, 1, \dots, N - 1$.
- Similarly, define $k_m = k_{\text{min}} + \Delta k \, m$, where $\Delta k$ is the spacing in log-strike space and $m = 0, 1, \dots, N - 1$.

The integral @eq-carrmadan becomes a sum:

$$
\tilde{C}(K_m) \approx \frac{e^{-r T}}{2\pi} \sum_{j=0}^{N-1} e^{-i u_j k_{\text{min}}} \phi_{C}(u_j) e^{-i 2\pi j m / N} \eta,
$$

which can be efficiently computed using the FFT.^[For options with very short maturities, the pricing errors 
...
- Determine $\eta$ and $\Delta k$ based on $N$ and the desired range of strikes.
...
$$
\phi_{C}(u_j) = \frac{\phi_{S_T}(u_j - i (\alpha + 1))}{(\alpha + i u_j)(\alpha + i u_j + 1)}.
$$

#### 4. Apply the FFT

- Multiply $\phi_{C}(u_j)$ by $e^{-i u_j k_{\text{min}}}$ and the appropriate weights (e.g., Simpson's rule).
- Use the FFT to compute the sum efficiently.

#### 5. Recover the Option Prices

Obtain $\tilde{C}(K_m)$ from the inverse FFT and adjust for the dampening factor:

$$
C(K_m) = e^{-\alpha k_m} \tilde{C}(K_m).
$$
...
The following Python code implements the Carr-Madan method to price a European call option under the Black-Scholes Model and compare it to the closed-form Black Scholes price. The results suggest that the Carr-Madan method yields an accurate call price.  
...
In the Heston stochastic volatility model, the dynamics of the asset price $S_t$ and its variance $v_t$ are given by the following stochastic differential equations (SDEs):
...
-$\kappa$ is the rate at which $v_t$ reverts to the long-term mean $\theta$,
...
In this formula:
- $\gamma$ is the drift term (linear component).
- $\sigma^2$ is the variance of the continuous Gaussian component.
...
- $G(t)$ is a Gamma process with mean rate $t$ and variance rate $\nu$.
- $\theta$ controls the drift of the VG process.
- $\nu$ is the variance rate of the Gamma process.
- $W(t)$ is a standard Brownian motion under the risk neutral measure $\mathbb{Q}$.

The characteristic function of $\log(S(t))$ under the risk-neutral measure with dividend yield $q$ is:
...
- $\sigma$ is the volatility of the Brownian motion component.
...
$$
where:
- $N(t)$ is a Poisson process with intensity $\lambda$, representing the number of jumps up to time $t$.
- $Y_i$ is the jump size, often modeled as a random variable from a log-normal distribution such that $\log(Y_i) \sim \mathcal{N}(\mu_J, \sigma_J^2)$.
