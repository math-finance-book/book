...
for each event $A$.  Thus, it is consistent with the @eq-probSnumeraire of the probability of an event $A$ when we use a non-dividend-paying asset price $S$ as the numeraire.  The relation between the two is that the ratio of path probabilities $Y(T)$ equals $\phi(T)S(T)/S(0)$, where $\phi(T)$ denotes the random state price at date $T$.
