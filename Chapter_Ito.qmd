...
# plot one path
t = np.array(range(0, n + 1, 1)) * \,\mathrm{d} t
...
BrownianMotion = 0
For i = 1 To N
  ActiveCell.Offset(i + 1, 0) = i * \,\mathrm{d} t          ' next time
...
G\,\mathrm{d} B = np.zeros(shape = (n, m))
G\,\mathrm{d} B[0] = np.repeat(DG(0), m) * inc[0]
G\,\mathrm{d} B[1:] = DG(Bt[0:n - 1]) * inc[1:]
SI = np.zeros(shape = (n, m))
# Stochastic Integral is cumulative sum of G'(B)\,\mathrm{d} B plus initial
SI = G\,\mathrm{d} B.cumsum(axis = 0) + 0.5 * (DDG(Bt[0:n])*Q).cumsum(axis =0) + G(0)
# Compare Ito's Lemma 
plt.figure(figsize=(9,6))
plt.plot(t[0:n], SI[:,0])
```
...
```{python}
#| label: Simulated Exact Solution
...
drift = (r-0.5*sig**2)
# Volatility
vol = sig * np.sqrt(\,\mathrm{d} t)

t = np.array(range(0,m + 1,1)) * \,\mathrm{d} t
...
## Tail Probabilities of Geometric Brownian Motions {#sec-s(t)ailprobs}
...
:::
::: {#exr-nolabel}
Use simulation to find  $E^*[e^{-r T}{\mathbf{1}}_{\{S(t) \ge K\}}]$ in the risk neutral measure where
\begin{equation*}
