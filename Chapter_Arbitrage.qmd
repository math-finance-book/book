...
Thus, the expected future (date--$t$) value of the ratio $Y/S$ always equals the current (date--$s$) value when we use $S$ as the numeraire.  As discussed in the preceding section, the mathematical term for a random variable whose expected future value always equals its current value is martingale.  Thus, we can express @eq-preformula as stating that the ratio $Y/P$ is a martingale when we compute expectations using the probability measure that corresponds to $S$ being the numeraire.
