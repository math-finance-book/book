...
::: Example
Suppose a stock price today is $S=100$ and that the price at date $t$ will be either $S_u=110$ or $S_u=90$.  The following figure depicts this situation.
...
The following shows the value of delta shares at the option maturity.
...
The critical feature of the previous figure is that the difference between the up and down values is the same as the difference between the up and down values of the call; that is, it is $5$ dollars.  This is guaranteed by our formula for $\delta$.  The portfolio of $\delta$ shares is worth more than the call at date $t$, but if we owed $22.50$, then value of the portfolio after paying back the loan would match the call value.  At an interest rate of 5% (as a rate from date $0$ to date $t$ instead of continuous compounding, for simplicity), we could borrow $21.43$ and then owe $22.50$.  This is depicted in the following.
...
::: Principle
If there are no arbitrage opportunities, then there exists for each date $t$ a positive random variable $m_t$, called a stochastic discount factor, such that the date--$0$ value of any dividend-reinvested asset with price $P$ is 
$$
P_0 = \E[m_tP_t]\;.
$$ {#eq-continuum1}
:::
...
For example, letting $P_s = R_s$ denote the value $\mathrm{e}^{rs}$ of the risk-free asset and using it as the numeraire, @eq-formula becomes
$$
...
How do the probabilities of different events change when we change probability measures?  Which events become more likely and which become less likely?  These are natural questions to ask.  Let's consider the risk-neutral probability.   @eq-riskneutralformula can be rearranged as
...
Why does this method work?  Fundamentally, it is because pricing is linear. To Be Continued.
