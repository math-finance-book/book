...
\mathrm{n}ext Forward-strike calls and puts satisfy a somewhat unusual form of put-call parity.  The usual put-call parity is of the form:
...
\begin{multline}
-\mathrm{e}^{-rT}K\mathrm{N}(d_2) + \mathrm{e}^{-q T'}S(0)\M\!\left(d_1,d_1',\sqrt{T/T'}\right)
...
\begin{multline}
-\mathrm{e}^{-rT}K\mathrm{N}(-d_2) + \mathrm{e}^{-rT'}K'\M\!\left(-d_2,-d_2',\sqrt{T/T'}\right) 
$$ {#eq-callonaput}
...
d_1 = \frac{\log\left(\frac{Z(0)}{Z^*}\right)+\left(r+\frac{1}{2}\sigma^2\right)T}{\sigma\sqrt{T}}\mathrm{n}otag
$$
$$
...
d_1' = \frac{\log\left(\frac{Z(0)}{K}\right)+\left(r+\frac{1}{2}\sigma^2\right)T'}{\sigma\sqrt{T'}}\mathrm{n}otag
$$
$$
...
\begin{multline}
[S(0)-\mathrm{e}^{-rT}D]\mathrm{N}(d_1) + \mathrm{e}^{-rT}(D-K)\mathrm{N}(d_2)
$$
...
\mathrm{n}oindent For a simple chooser with $K_c=K_p=K$ and $T_c=T_p=T'$, we can find $S^*$ from the put-call parity relation at $T$, leading to $S^*=\mathrm{e}^{(q-r)(T'-T)}K.$
...
\begin{multline}
\mathrm{e}^{-qT}S(0)\left[\mathrm{N}(d_1)-\left(\frac{L}{S(0)}\right)^{2\left(r-q+\frac{1}{2}\sigma^2\right)/\sigma^2}\mathrm{N}(d_1')\right]
$$ {#eq-downout100}

$$ - \mathrm{e}^{-rT}K\left[\mathrm{N}(d_2) - \left(\frac{L}{S(0)}\right)^{2\left(r-q-\frac{1}{2}\sigma^2\right)/\sigma^2}\mathrm{N}(d_2')\right]\;,
\end{multline}
where 


\renewcommand{\labelenumi}{(\arabic{enumi})}
1. if $K>L$, $d_1$, $d_2$ , $d_1'$ and $d_2'$ are defined in @eq-casea35combined,
2. if $K\leq L$, $d_1$, $d_2$, $d_1'$ and $d_2'$ are defined in @eq-caseb35combined.
...
\begin{multline}
\mathrm{e}^{-qT}S(0)\mathrm{N}(d_1)-\mathrm{e}^{-rT}S_{\text{min}}\mathrm{N}(d_2) 
$$ {#eq-fslc100}

$$+\frac{\sigma^2}{2(r-q)}\left(\frac{S_{\text{min}}}{S(0)}\right)^{2(r-q)/\sigma^2}\mathrm{e}^{-rT}S(0)\mathrm{N}(d_2') 
$$
$$
...
$$
V(0) = \mathrm{e}^{-rT}S(0)\exp\left(\frac{(r-q-\sigma^2/2)T}{2} + \frac{\sigma^2T}{6}\right)\mathrm{n}otag
$$
$$
...
at date $T$.  The call option will pay $\max(0,V(T)-K)$ at date $T$.  Let $k$ denote the integer such that $t_{N-k-1} \leq t < t_{N-k}$.  This means that we have already observed the prices $S(t_1), \ldots, S(t_{N-k-1})$ and we have yet to observe the $k+1$ prices $S(t_{N-k}), \ldots ,S(t_N)$.  Define $\mathrm{var}epsilon = (t_{N\!-k}-t)/\Delta t$, which is fraction of the interval $\Delta t$ that must pass before we reach the next sampling date $t_{N\!-k}$.  We will show at the end of this section that 
\begin{multline}
V(t) = \mathrm{e}^{-r(T-t)}S(t)^{\frac{k+1}{N}}\prod_{i=1}^{N-k-1}S(t_i)^{\frac{1}{N}}
$$ {#eq-geometricaveragev2}

$$
\times\,\exp\left(\left[ \frac{(k+1)\mathrm{var}epsilon\mathrm{n}u}{N}\!+\! \frac{k(k+1)\mathrm{n}u}{2N}\! +\! \frac{(k+1)^2\sigma^2\mathrm{var}epsilon}{2N^2}\!+\!\frac{k(k+1)(2k+1)\sigma^2}{12N^2} \right]\Delta t\right)\;,
\end{multline}
where $\mathrm{n}u = r-q-\sigma^2/2$

Again, two points are noteworthy.  Assume the call was written at date 0 and the first observation date $t_1$ is $\Delta t$ years away.  Then, we have $k+1=N$ and $\mathrm{var}epsilon=1$ so
$$
V(0) = \mathrm{e}^{-rT}S(0)\exp\left( \frac{(N+1)\mathrm{n}u\Delta t}{2} + \frac{(N+1)(2N+1)\sigma^2\Delta t}{12N} \right)\;.
$$ {#eq-asianV02}
...
This implies that the average volatility, in the sense of Sect.~\ref{s_timevaryingvolatility}, is
$$
\sigma_{\text{avg}} = \sqrt{\frac{1}{N}\sum_{k=0}^{N-1} \left(\frac{k+1}{N}\right)^2\sigma^2\,\mathrm{d} t}\mathrm{n}otag
$$
$$
...
$$
V(t) = \mathrm{e}^{-r(T-t)}E^R_t \left[\exp\left(\frac{1}{N} \sum_{i=1}^N \log S(t_i)\right)\right]\mathrm{n}otag
$$
$$
= \mathrm{e}^{-r(T-t)}\exp\left(\frac{1}{N}\sum_{i=1}^{N-k-1} \log S(t_i)\right)\times E^R_t \left[\exp\left(\frac{1}{N} \sum_{i=N-k}^N \log S(t_i)\right)\right]\mathrm{n}otag
$$
$$
...
\begin{multline*}
\sum_{i=0}^{k}\big\{[\log S(t) + (t_{N-k+i}-t)\mathrm{n}u + \sigma [B(t_{N-k+i})-B(t)]\big\}
$$
$$
=(k+1)\log S(t) + \sum_{i=0}^{k} (\mathrm{var}epsilon + i)\mathrm{n}u\Delta t + \sigma\sum_{i=0}^{k} [\Delta_0B + \Delta_1B + \cdots + \Delta_iB] 
$$
$$
=(k+1)\log S(t) + (k+1)\mathrm{var}epsilon\mathrm{n}u\Delta t + \frac{k(k+1)}{2}\mathrm{n}u\Delta t + \sigma\sum_{i=0}^{k} (k+1-i)\Delta_iB\;,
\end{multline*}
where to obtain the last equality we used the fact that $\sum_{i=0}^k i = k(k+1)/2$.
The random variables $\Delta_iB$ are normally distributed with mean zero and variance $\Delta t$ (the variance is $\mathrm{var}epsilon \Delta t$ for $i=0$).  Thus, the sum of logarithms is
 a normally distributed random variable with mean
$$(k+1)\log S(t) + (k+1)\mathrm{var}epsilon\mathrm{n}u\Delta t + \frac{k(k+1)}{2}\mathrm{n}u\Delta t$$
and variance
$$(k+1)^2\sigma^2\mathrm{var}epsilon\Delta t + \sigma^2\sum_{i=1}^{k} (k+1-i)^2\Delta t = (k+1)^2\sigma^2\mathrm{var}epsilon\Delta t +\frac{k(k+1)(2k+1)\sigma^2}{6}\; ,$$
...
$$S(t)^{\frac{k+1}{N}}\exp\left(\left[ \frac{(k+1)\mathrm{var}epsilon\mathrm{n}u}{N}+ \frac{k(k+1)\mathrm{n}u}{2N} + \frac{(k+1)^2\sigma^2\mathrm{var}epsilon}{2N^2}+\frac{k(k+1)(2k+1)\sigma^2}{12N^2} \right]\Delta t\right)\; ,$$
which implies @eq-geometricaveragev2.
:::

## Calculations in VBA {#sec-s_exotics_matlab}
...
\mathrm{n}oindent Notice that this function calls itself.  This is an example of recursion.
...
::: {#exr-nolabel}
 Intuitively, the value of a forward-start call option should be lower the closer is the date~$T$ at which the strike is set to the date $T'$ at which the option matures, because then the option has less time to maturity after being created at $T$.  Create an Excel worksheet to confirm this.  Allow the user to input $S$, $r$, $\sigma$, $q$, and $T'$.  Compute and plot the value of the option for $T=0.1T'$, $T=0.2T'$, \ldots, $T=0.9T'$.
:::
::: {#exr-nolabel}
 Create an Excel worksheet to demonstrate the additional leverage of a call-on-a-call relative to a standard call.  Allow the user to input $S$, $r$, $\sigma$, $q$, and $T'$.  Use the \verb!Black-Scholes_Call! function to compute and output the value $C$ of a European call with strike $K'=S$ (i.e., the call is at the money) and maturity $T'$.  Use the \verb!Call_on_Call! function to compute and output the value of a call option on the call with strike $K=C$ (i.e., the call-on-a-call is at the money) and maturity $T=0.5T'$.  Compute the percentage returns the standard European call and the call-on-a-call would experience if the stock price $S$ instantaneously increased by 10\%.
:::
::: {#exr-nolabel}
 Create an Excel worksheet to illustrate the early exercise premium for an American call on a stock paying a discrete dividend.  Allow the user to input $S$, $r$, $\sigma$, and~$T'$.  Take the date of the dividend payment to be $T=0.5T'$ and take the strike price to be $K=S$.  As discussed in Sect.~\ref{s_discrete}, the value of a European call is given by the Black-Scholes formula with $S-\mathrm{e}^{-rT}D$ being the initial asset price and $q=0$ being the constant dividend yield.  Use the function \verb!American_Call_Dividend! to compute the value of an American call for dividends $D=.1S$, \ldots $D=.9S$.  Subtract the value of the European call with the same dividend to obtain the early exercise premium.  Plot the early exercise premium against the dividend $D$.
:::
::: {#exr-nolabel}
 Create a VBA function to value a simple chooser (a chooser option in which $K_c=K_p$ and $T_c=T_p$) using put-call parity to compute $S^*$ as mentioned in Sect.~\ref{s_choosers}.  Verify that the function gives the same result as the function \verb!Chooser!.
:::
::: {#exr-nolabel}
 Create an Excel worksheet to compare the cost of a simple chooser to that of a straddle (straddle = call + put with same strike and maturity).  Allow the user to input $S$, $r$, $\sigma$, $q$, and $T'$.  Take the time to maturity of the underlying call and put to be $T'$ for both the chooser and the straddle.  Take the strike prices to be $K=S$.  Take the time the choice must be made for the chooser to be $T=0.5T'$.  Compute the cost of the chooser and the cost of the straddle.
:::
::: {#exr-nolabel}
...
::: {#exr-e_standardknockout}
  Suppose you must purchase 100 units of an asset at the end of a year.  Create an Excel worksheet simulating the asset price and comparing the quality of the following hedges (assuming 100 contracts of each):
...
Modify Prob.~\ref{e_standardknockout2} by including the hedge in Prob.~\ref{e_standardknockout3}.  Value the down-and-in call using the function \verb!Down_And_Out_Call! and the fact that a down-and-out and down-and-in with the same strikes and barriers form a standard option. 
:::
::: {#exr-e_averagehedge}
  Each week you purchase 100 units of an asset, and you want to hedge your total quarterly (13-week) cost.  Create an Excel worksheet simulating the asset price and comparing the quality of the following hedges: 
...
:::
::: {#exr-nolabel}
 Derive formula @eq-callonaput for the value of a call on a put.
:::
::: {#exr-nolabel}
 Complete the derivation of formula @eq-chooser2 for the value of a chooser option.
:::
::: {#exr-nolabel}
