...
# Fixed-Income Derivatives {#sec-c_fixedincomederivatives}
...
As explained in @sec-s_swaps, $S_t$ is the value of the floating-rate payments in the swap and $Z_t$ is the value of the fixed-rate payments.  The value of a payer swaption at its maturity $T$ is therefore
$$\max(0,S_T-Z_T)\; .$$
...
in @sec-s_swaps the forward swap rate $\mathcal{R}_t$ was defined to be the rate such that 
...
In @sec-s_swaptions, we considered this as the difference of two pieces, the first piece being $S_T = P(T,t_0)-P(T,t_N)$, which is the value of the floating-rate leg, and the second being 
