...
Denote the price of a dividend-reinvested asset by $S$.  Assume there is a constant continuously-compounded risk-free rate $r$, and set $R_t = \e^{rt}$, which is a dividend-reinvested asset price.  The principle elucidated in @sec-s:change_numeraire tells us that, in the absence of arbitrage opportunities, there is a probability measure (called the risk-neutral probability) with respect to which $S/R$ is a martingale.  
