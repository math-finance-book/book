...
\begin{align*}
 ...
 \frac{\mathrm{d} Y}{Y} &= \theta\,\mathrm{d} t+\phi\,\mathrm{d} W\;,
\end{align*}
...
We consider the dynamics of the asset price $S$ under three different probability measures.  In each case, we follow the same steps: (i) we note that the ratio of an asset price to the numeraire asset price must be a martingale, (ii) we use Ito's formula to calculate the drift of this ratio, and (iii) we use the fact that the drift of a martingale must be zero to compute the drift of $\mathrm{d} S/S$.  Rather than executing these steps, we could instead apply a general result called Girsanov's Theorem, which is discussed at the end of this chapter.
...
## 
$$
\frac{\mathrm{d} S}{S} = (r-q+ \sigma^2_s)\,\mathrm{d} t+\sigma\,\mathrm{d} B^*\;,
$$ {#eq-own11}
...
& = \frac{\mathrm{d} V}{V} -\frac{\mathrm{d} Y}{Y} -\rho\sigma\phi\,\mathrm{d} t+\phi^2\,\mathrm{d} t\\
&= \frac{\mathrm{d} S}{S} -\frac{\mathrm{d} Y}{Y} + (q-\rho\sigma\phi\,\mathrm{d} t+\phi^2)\,\mathrm{d} t\; .
\end{align*}
...
$(r-q+ \rho\sigma\phi)\,\mathrm{d} t$.  We conclude that:

::: Rule
## 
$$
\frac{\mathrm{d} S}{S} = (r-q+ \rho\sigma\phi)\,\mathrm{d} t+\sigma\,\mathrm{d} B^*\;,
$$ {#eq-other11}
...
Then 
$$\mathrm{d} B^* = \frac{1}{\sigma}\left(\frac{\mathrm{d} Z}{Z}\right)$$
and hence is a continuous martingale under the risk-neutral probability.  We can compute its quadratic variation as
$$(\mathrm{d} B^*)^2 = \left(\frac{q-r+\mu}{\sigma}\right)^2(\mathrm{d} t)^2 + 2\left(\frac{q-r+\mu}{\sigma}\right)(\mathrm{d} t)(\mathrm{d} B) + (\mathrm{d} B)^2 = \mathrm{d} t\; .$$
...
$$(q-r)\,\mathrm{d} t + \frac{\mathrm{d} S}{S} = \sigma\,\mathrm{d} B^* \quad \Longleftrightarrow \quad \frac{\mathrm{d} S}{S} = (r-q)\,\mathrm{d} t + \sigma\,\mathrm{d} B^*\;,$$
