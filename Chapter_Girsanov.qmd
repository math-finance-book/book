...
Let $Y$ be the price of another another asset that does not pay dividends.  Let $r_t$ denote the instantaneous risk-free rate at date $t$ and let $R_t = \exp\left(\int_0^t r(s)\,\mathrm{d} s\right)$.  Assume
\begin{align*}
...
$$Z_t = \frac{V_t}{R_t} = \exp\left(-\int_0^t r(s)\,\mathrm{d} s\right)V_t$$
...
$Z_t$ defined as 
$$Z_t = \frac{R_t}{V_t} = \frac{\exp\left(\int_0^t r(s)\,\mathrm{d} s\right)}{V_t}$$
