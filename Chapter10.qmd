...
$$\mathrm{d} X = \mathrm{n}u\,\mathrm{d} t+\sigma\,\mathrm{d} B\; ,$$
where $\mathrm{n}u =r-q-\sigma^2/2$ and $B$ is a Brownian motion under the risk-neutral measure.
...
$$
=\frac{\partial C}{\partial t}\,\mathrm{d} t+ \frac{\partial C}{\partial X}\big(\mathrm{n}u\,\mathrm{d} t+\sigma\,\mathrm{d} B\big) + \frac{1}{2}\frac{\partial^2 C}{\partial X^2}\sigma^2\,\mathrm{d} t\;.
$$
...
$$
rC = \frac{\partial C}{\partial t}+ \mathrm{n}u\frac{\partial C}{\partial X}+ \frac{1}{2}\sigma^2\frac{\partial^2 C}{\partial X^2}\;.
$$ {#eq-3}
...
$$
\frac{\partial C}{\partial \tau} = -rC + \mathrm{n}u\frac{\partial C}{\partial X}+ \frac{1}{2}\sigma^2\frac{\partial^2 C}{\partial X^2}\;.
$$ {#eq-4}
...
$$
rC = \frac{C-C_{\text{left}}}{\Delta t}+ \mathrm{n}u\left(\frac{C_{\text{up}}-C_{\text{down}}}{2\Delta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C_{\text{up}}+C_{\text{down}}-2C}{(\Delta x)^2}\right)\;.
$$ {#eq-explicit}
...
where
$$
p_u = \frac{\sigma^2\Delta t+\mathrm{n}u\Delta t\Delta x}{2(1-r\Delta t)(\Delta x)^2}\; ,
$$
$$
p_d = \frac{\sigma^2\Delta t-\mathrm{n}u\Delta t\Delta x}{2(1-r\Delta t)(\Delta x)^2}\; ,
$$
$$
$$
and
...
$$\Delta x \leq  \frac{\sigma^2}{|\mathrm{n}u|} \qquad \text{and} \qquad \Delta t \leq \frac{(\Delta x)^2}{\sigma^2 + r(\Delta x)^2}\; .$$
...
$$M \geq \frac{|\mathrm{n}u| D}{\sigma^2} \qquad \text{and} \qquad N \geq rT + \left(\frac{\sigma^2T}{D^2}\right)M^2\; .$$
...
$$
rC = \frac{C_{\text{right}}-C}{\Delta t}+ \mathrm{n}u\left(\frac{C_{\text{up}}-C_{\text{down}}}{2\Delta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C_{\text{up}}+C_{\text{down}}-2C}{(\Delta x)^2}\right)\;.
$$ {#eq-implicit}
...
Now,  @eq-3 becomes
$$
rC^{\text{mid}} = \frac{C'-C}{\Delta t}+ \mathrm{n}u\left(\frac{C^{\text{mid}}_{\text{up}}-C^{\text{mid}}_{\text{down}}}{2\Delta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C^{\text{mid}}_{\text{up}}+C^{\text{mid}}_{\text{down}}-2C^{\text{mid}}}{(\Delta x)^2}\right)\;.
$$ {#eq-crank}

Substituting from the formulas for $C^{\text{mid}}$, $C^{\text{mid}}_{\text{up}}$, and $C^{\text{mid}}_{\text{down}}$, we can re-write @eq-crank as
\begin{multline}
\left(\frac{r}{2}+\frac{1}{\Delta t}+\frac{\sigma^2}{2(\Delta x)^2}\right)C - \left(\frac{\sigma^2}{4(\Delta x)^2}+\frac{\mathrm{n}u}{4\Delta x}\right)C_{\text{up}}
$$ {#eq-crank2}

$$ - \left(\frac{\sigma^2}{4(\Delta x)^2}-\frac{\mathrm{n}u}{4\Delta x}\right)C_{\text{down}} 
\quad = \quad \left(\frac{1}{\Delta t}- \frac{r}{2}-\frac{\sigma^2}{2(\Delta x)^2}\right)C' 
$$
$$+ \left(\frac{\sigma^2}{4(\Delta x)^2}+\frac{\mathrm{n}u}{4\Delta x}\right)C'_{\text{up}} + \left(\frac{\sigma^2}{4(\Delta x)^2}-\frac{\mathrm{n}u}{4\Delta x}\right)C'_{\text{down}}
\end{multline}
...
## Calculations in VBA {#sec-s_finitedifferences_matlab}
...
where $z_1$, $b_1$, $z_L$ and $b_L$ are numbers to be calculated or input by the user.
 The equations @eq-crank4a and @eq-crank4b are the special cases in which  $z_1 = \lambda_0(S-S_{\text{up}})$, $b_1 = 1$, $z_L = \lambda_\infty(S-S_{\text{down}})$,  and $b_L=1$.   The additional generality in allowing $b_1$ and $b_L$ to be different from one is important for many purposes, and we will see an example of it in the valuation of barrier options.  
...
The first equation in this array (equation @eq-crank4aa) can be written as
...
The $j$--th equation ($j = 2, \ldots, L-1$) in the array (equation @eq-crank3)  is 
...
The last equation in the array (equation @eq-crank4bb) is 
$$
C_L = z_L + b_LC_{L-1}\; .
$$
...
As elsewhere in this chapter, $N$ denotes the number of time periods, and $2M+1$ will be the number of $x$ values on the grid.  We use the symbol $D$ to denote the distance of the top (or bottom) of the grid from $\log S(0)$.  In other words, $D = x_M-x_0$.  A reasonable value for $D$ would be three standard deviations for $\log S$, which would mean $D = |\mathrm{n}u|T +3 \sigma\sqrt{T}$. For example, for a one-year option on a stock with a volatility of 30\%, it should suffice to input $D=1$.  
...
:::
::: {#exr-nolabel}
 Create a VBA function \verb!Up_And_Out_Put_CN! to value an up-and-out put option by the Crank-Nicolson method.
:::
::: {#exr-nolabel}
Create a VBA function \verb!European_Call_Explicit! that uses the explicit method @eq-fdtrinomial to value a European call option.
:::

::: {#exr-nolabel}

Write the system of equations @eq-implicit for the implicit method, together with boundary conditions of the form @eq-crank4aabbcombined as a matrix system and solve for $u_j$ and $b_j$ in @eq-crank101combined, as in the subsection that defines the function \verb!CrankNicolson!.
:::

::: {#exr-nolabel}
Create a VBA function \verb!Implicit! that solves the system of equations in the preceding exercise.
:::
::: {#exr-nolabel}
