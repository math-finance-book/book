...
$$
Therefore $\\mathrm{E}^R_t\left[A^g_T\right]$ is the expectation of the exponential of a normally distributed random variable.  Equation~@eq-geometricaveragev1 now follows from the fact that if $x$ is normally distributed with mean $\mu$ and variance $\sigma^2$ then $\\mathrm{E}\left[\mathrm{e}^x\right] = \mathrm{e}^{\mu+\sigma^2/2}$.  
