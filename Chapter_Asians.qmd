...
$$
A^g_T = \exp\left(\frac{1}{T}\int_0^T \log S_t\d   t\right)\; .
$$
...
$$
V_t= \mathrm{e}^{-r(T-t)}\\E^R_t\big[A^g_T\big]\; .
$$
Define
$$
...
$$
\frac{ \d  \log \left(S^{\frac{T-t}{T}}) = \text{something}\d    t + \left(\frac{T-t}{T}\right)\sigma\d   B\;.
$$
...
$$
Therefore $\\E^R_t\left[A^g_T\right]$ is the expectation of the exponential of a normally distributed random variable.  Equation~@eq-geometricaveragev1 now follows from the fact that if $x$ is normally distributed with mean $\mu$ and variance $\sigma^2$ then $\\E\left[\mathrm{e}^x\right] = \mathrm{e}^{\mu+\sigma^2/2}$.  
