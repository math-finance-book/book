...
::: {.callout-note}
 ## 
 We will conclude this section with a proof of @eq-holee3.  Let $\varepsilon(t_i)$ denote independent  random variables that equal $\pm \sigma \sqrt{\Delta t}$ with probability $1/2$ each under the risk-neutral measure.  Then we can write
...
\end{multline*}
Equating this to the ratio of market prices $P^{\text{mkt}}(0,t_{n+1})/P^{\text{mkt}}(0,t_{n})$ gives the @eq-holee3.
:::
...
From Ito's formula and the @eq-cir of $\mathrm{d} r$, we have
...
The solution can be obtained by guessing a solution of the same form as the Vasicek bond pricing @eq-vasicekbond, namely^[This guess works because the CIR model, like the Vasicek model, is an affine model.  See @sec-s_ls].
$$
...
is a martingale, and hence a Brownian motion, when $P(t,u)$ is used as the numeraire.  Substituting this into the @eq-cirrhat of $\hat{r}$, we have
...
The expectations @eq-lsdiscount can be written in terms of $r(t)$ and $V(t)$ by substituting from @eq-x1ls - @eq-x2ls.
...
Heath, Jarrow and Morton use the @eq-bondsandforwards and the fact that the expected return of a discount bond under the risk-neutral measure must be the short rate to derive a formula for the drifts of forward rates under the risk-neutral measure.  
...
For example, in the Hull-White model we have $\Sigma(t,u) = \sigma b(\tau)$ where $b$ is defined in @eq-extendedvasicekbond - @eq-extendedvasicek_b, and in the CIR model, we have $\Sigma(t,u) = \sigma b(\tau)\sqrt{\hat{r}(t)}$, where $b$ is defined in @eq-cirbeta - @eq-ciralpha.
