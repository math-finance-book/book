...
# Ho-Lee
The Ho-Lee [@HL] model is a binomial version of the Vasicek model
without mean reversion, in which the one-period interest rate is assumed
...
$t_{i}-t_{i-1}=\varDelta t$ for each $i$. We denote the one-period
interest rate from date $t_i$ to $t_{i+1}$ by $r(t_i)$. We express the
rate as an annualized continuously compounded rate, so the one-period
discount factor from date $t_i$ to $t_{i+1}$ is
$\E^{-r(t_i) \varDelta t}$. We could put this in a continuous-time
framework by assuming the short rate is constant ($= r(t_i)$) during
each time interval $(t_i, t_{i+1})$. As always, the risk-neutral measure
is the probability measure corresponding to the numeraire
...
[\[holeeaccumulation\]](#holeeaccumulation){reference-type="eqref"
reference="holeeaccumulation"} is
...
Chaps. [\[c_introcomputation\]](#c_introcomputation){reference-type="ref"
reference="c_introcomputation"}
and [\[c_montecarlo\]](#c_montecarlo){reference-type="ref"
reference="c_montecarlo"}. The new feature is that the discount rate is
changing over time. So, we now have three trees to consider: the tree
for the underlying, the tree for the one-period interest rate, and the
tree for the derivative value. However, the tree for the underlying can
be created from the interest-rate tree, so the interest-rate tree is the
basic input instead of the tree for the underlying.
...
$\E^{-.07} = 0.932$ at the top node at date 2. This is also the value of
the discount bond at that node. Likewise, the value of the bond at the
middle node at date 2 is $\E^{-.05} = 0.951$. This implies that the
value of the bond at the top node at date 1 is
...
$P^{\text{mkt}}(0,t_{i+1})/P^{\text{mkt}}(0,t_{i})$ is the reciprocal of
one plus the market forward rate at date 0 for loans from date $t_i$ to
$t_{i+1}$, this formula for the parameters $\phi(t_i)$ is equivalent to
equating model forward rates to market forward rates, as was discussed
for the Hull-White and continuous-time Ho-Lee models in
Sect. [\[s_vasicek_fitting\]](#s_vasicek_fitting){reference-type="ref"
reference="s_vasicek_fitting"}. In fact, the fitting of the Ho-Lee model
to market bond prices is often expressed by saying "the market forward
rate curve is an input to the model." This idea was developed further by
Heath, Jarrow and Morton [@HJM]---see
Sect. [\[s_hjm\]](#s_hjm){reference-type="ref" reference="s_hjm"}.
...
horizons, as discussed in
Sect. [\[s_vasicek1\]](#s_vasicek1){reference-type="ref"
reference="s_vasicek1"}. To offset this, one could use a smaller
volatility when valuing long-maturity options. However, for options with
early exercise features, this would imply too little uncertainty about
the level of the one-period rate at short horizons and thereby
undervalue the early exercise option.

\begin{petit} We will conclude this section with a proof of formula
[\[holee3\]](#holee3){reference-type="eqref" reference="holee3"}. Let
$\varepsilon(t_i)$ denote independent random variables that equal
$\pm \sigma \sqrt{\varDelta t}$ with probability $1/2$ each under the
risk-neutral measure. Then we can write
...
The Black-Derman-Toy [@BDT] model is, like the Ho-Lee model and the
...
The volatility $\sigma(t_i)$ has the interpretation of the *percentage*
volatility of the one-period rate (rather than the absolute volatility,
as in the Vasicek and Ho-Lee models). The model implies that the
one-period rate will always be nonnegative.
...
$\eta(t_i)\,\varDelta t$ and $b_i$ for $\sigma(t_i)\sqrt{\varDelta t}$.
\begin{center}
\setlength{\unitlength}{0.6cm}
\begin{picture}(10,8)
...
Equations [\[bdt2\]](#bdt2){reference-type="eqref" reference="bdt2"} and
[\[bdt3\]](#bdt3){reference-type="eqref" reference="bdt3"} define the
Black-Derman-Toy model. The free parameters at each date are
$\sigma(t_i)$ and $\theta(t_i)$, and the parameter $\kappa(t_i)$ is
defined by [\[bdt3\]](#bdt3){reference-type="eqref" reference="bdt3"}.
Alternatively, one can view $\kappa(t_i)$ and $\theta(t_i)$ as free
parameters and define $\sigma(t_i)$ from
[\[bdt3\]](#bdt3){reference-type="eqref" reference="bdt3"}. Note that
$\kappa(t_i)$ can be interpreted as a mean-reversion parameter for
$\log r(t_i)$, at least when it is positive (i.e., when
$\varDelta \sigma(t_i)<0$). Because there are two free parameters at
each date rather than one (as in the Ho-Lee model) it is possible to
match both market bond prices and market yield volatilities or cap
prices.
...
The Black-Karasinski [@BK] model is similar to the Black-Derman-Toy
model---it assumes [\[bdt2\]](#bdt2){reference-type="eqref"
reference="bdt2"} for the changes in the logarithm of the one-period
rate---but it removes the linkage
[\[bdt3\]](#bdt3){reference-type="eqref" reference="bdt3"} between the
mean-reversion parameter and the volatility. It does this by allowing
the lengths of the time steps to vary. Denote the length of the
time-step $t_i-t_{i-1}$ by $\tau_i$. Consider again the two-period
example of the previous section. As a necessary condition for the tree
to be recombining, we deduced (in
[\[bdt4\]](#bdt4){reference-type="eqref" reference="bdt4"}) that
...
# Cox-Ingersoll-Ross {#s_cir}


Cox, Ingersoll, and Ross [@CIR] \index{Cox-Ingersoll-Ross (CIR) model}introduced a continuous-time model[^1]: Cox, Ingersoll and Ross (hereafter CIR) also discuss a variety of
    other continuous-time models, but this particular model is so well
    known that it is often simply called *the* CIR model.
in which the short
rate satisfies
...
## Discount Bond Prices in the CIR Model {#discount-bond-prices-in-the-cir-model .unnumbered}
...
Chap. [\[c_pde\]](#c_pde){reference-type="ref" reference="c_pde"}. Let
$P(t,u)$ denote the price at date $t$ of a discount bond maturing at
date $u$, having remaining time to maturity of $\tau = u-t$. The
discount bond price will depend on the remaining time to maturity and
the short rate at date $t$, because, as in the Vasicek model, the short
rate is the only random factor in this model. Thus, there must be some
deterministic function $f$ such that $P(t,u) = f(r(t),\tau).$ As in
Chap. [\[c_pde\]](#c_pde){reference-type="ref" reference="c_pde"}, the
fundamental pde is obtained by applying Itô's formula to $f$ to compute
$\D f$ in terms of the partial derivatives of $f$ and then using the
fact that the expected return of the discount bond (hence the drift of
$\D f/f$) must equal the short rate under the risk-neutral measure.

From Itô's formula and the definition
[\[cir\]](#cir){reference-type="eqref" reference="cir"} of $\D r$, we
have
\begin{align}
...
The solution can be obtained by "guessing" a solution of the same form
as the Vasicek bond pricing formula
[\[vasicekbond\]](#vasicekbond){reference-type="eqref"
reference="vasicekbond"}, namely[^1]

[^1]: This guess works because the CIR model, like the Vasicek model, is
    an "affine model." See Sect. [\[s_ls\]](#s_ls){reference-type="ref"
    reference="s_ls"}.
...
\end{equation} 
\end{subequations}
where the "primes" denote derivatives. By differentiating, one can
verify that the solution of
[\[betaprime\]](#betaprime){reference-type="eqref"
reference="betaprime"} (which is called a "Riccati equation") with the
boundary condition $b(0)=0$ is
...
Note that  \eqref{cir3} implies $\partial f/\partial r = - b(\tau)f$.  Substituting this into [\[cir10\]](#cir10){reference-type="eqref"
reference="cir10"} gives us the discount bond return as
...
reference="s_vasicekhedging"} for the Vasicek model.

## Calibrating the CIR Model to the Yield Curve {#calibrating-the-cir-model-to-the-yield-curve .unnumbered}

The CIR model can be calibrated to current market conditions by taking
one or more of the parameters $\kappa$, $\theta$ and $\sigma$ to be
time-varying, as in the extended Vasicek model. This was suggested by
...
[\[cir\]](#cir){reference-type="eqref" reference="cir"}. Specifically,
let
$$r(t) = \phi(t) + \hat{r}(t)\; ,$$
...
## Pricing Fixed-Income Derivatives in the CIR Model {#pricing-fixed-income-derivatives-in-the-cir-model .unnumbered}
...
is a martingale, when $P(t,u)$ is used as the numeraire, for $t \leq u$.
Let $Z(t)$ denote this ratio and apply Itô's formula for ratios to
derive
$$\frac{\D Z(t)}{Z(t)} = r(t)\,\D t - \frac{\D P(t,u)}{P(t,u)} + \left(\frac{\D P(t,u)}{P(t,u)}\right)^2\; .$$
Substituting from [\[cirdpp2\]](#cirdpp2){reference-type="eqref"
reference="cirdpp2"} now gives us
\begin{align*}
...
positive constants for $i=1,2$. Longstaff and Schwartz [@LS92]
investigate this model further, including providing an "equilibrium"
foundation, deriving discount bond option prices, and estimating the
coefficients. The model is usually called the Longstaff-Schwartz model.
...
models." An affine model is defined by a set of factors
$x_1, \ldots, x_n$, where
...
2. The drift of each factor is an affine function of the factors.
3. The instantaneous variance of each factor is an affine function of the factors.
4. The instantaneous covariance of each pair of factors is an affine function of the factors.
...
an affine model, as [\[affine\]](#affine){reference-type="eqref"
reference="affine"} shows, the short rate and yields at $n-1$ fixed
times-to-maturity $\tau_i$ can be chosen to be the $n$ factors (except
in the rare case that the linear transformation from factors to the
short rate and yields fails to be invertible). The transformation from
factors to yields is analogous to the transformation from $(x_1,x_2)$ to
$(r,V)$ in the Longstaff-Schwartz model. Important papers on affine
models include Duffie and Kan [@DK] and Dai and Singleton [@DSing].


# Heath-Jarrow-Morton {#s_hjm}

Heath, Jarrow and Morton [@HJM] propose an alternative framework for
modelling. Rather than modelling the evolution of the short rate (and
possibly other factors such as the volatility of the short rate or other
yields), Heath, Jarrow and Morton (hereafter, HJM) propose modelling the
...
As discussed in
Sect. [\[s_vasicek_fitting\]](#s_vasicek_fitting){reference-type="ref"
reference="s_vasicek_fitting"}, the forward rate at date $t$ for an
instantaneous loan at date $u \geq t$ is $f(t,u)$ defined by
...
\end{equation}
Heath, Jarrow and Morton use the definition
[\[bondsandforwards\]](#bondsandforwards){reference-type="eqref"
reference="bondsandforwards"} and the fact that the expected return of a
discount bond under the risk-neutral measure must be the short rate to
derive a formula for the drifts of forward rates under the risk-neutral
measure.
...
specifying initial forward rates---i.e., $f(0,u)$ for all $u$---and the
volatility processes $\sigma(t,u)$ for each $u$ and $t \leq u$. The
generalization to multiple Brownian motions is straightforward and
allows for forward rates that are not instantaneously perfectly
correlated. An important application of the HJM modelling framework is
work by Brace, Gatarek and Musiela [@BGM] and Miltersen, Sandmann, and
Sondermann [@MSS], who derive conditions on the volatility processes
$\sigma(t,u)$ that guarantee forward LIBOR rates of a fixed maturity
(e.g., quarterly or semi-annual rates) have deterministic volatilities,
thus justifying the use of Black's formula in
Sect. [\[s_valuingcaps\]](#s_valuingcaps){reference-type="ref"
reference="s_valuingcaps"} for valuing caps and floors.
...
reference="extendedvasicekbondcombined"}, the instantaneous forward rate
in the Hull-White model is
\begin{align*}
...
satisfy the HJM equation [\[hjm\]](#hjm){reference-type="eqref"
reference="hjm"}, as we knew they must, given that the HJM equation is
based only on the assumption that expected returns of discount bonds
equal the short rate under the risk-neutral measure. The initial forward
rate curve in the Hull-White model is $f(0,u) = \phi(u)$ which is chosen
to fit the market forward rate curve, as discussed in
Sect. [\[s_vasicek_fitting\]](#s_vasicek_fitting){reference-type="ref"
reference="s_vasicek_fitting"}. Thus, rather than defining the
Hull-White model as we did in
Chap. [\[c_vasicek\]](#c_vasicek){reference-type="ref"
reference="c_vasicek"}, it could be defined alternatively as an HJM
model in which the volatility process is the deterministic function
$\sigma(t,u) = \sigma \E^{-\kappa(u-t)}$ for positive constants $\sigma$
and $\kappa$. The normal distribution of the short rate in the
Hull-White model is a consequence of the volatility $\sigma(t,u)$ being
non-random.

Likewise, the Cox-Ingersoll-Ross model fit to market bond prices as we
discussed in Sect. [\[s_cir\]](#s_cir){reference-type="ref"
reference="s_cir"} could be described as an HJM model. Calculations of
the sort we have just done show that the volatility process in the CIR
model is $\sigma(t,u) = \sigma b'(\tau)\sqrt{\hat{r}}(t)$, where
$\tau = u-t$ and the function $b$ is defined in
[\[cirbondcombined\]](#cirbondcombined){reference-type="eqref"
reference="cirbondcombined"}. In this case, the volatility is random,
but it depends only on the short rate at date $t$. Similarly, in any
...
observed in
Chap. [\[c_fixedincomederivatives\]](#c_fixedincomederivatives){reference-type="ref"
reference="c_fixedincomederivatives"} that Black's formula can be
applied to value caps and floors when the underlying simple interest
rates have nonrandom volatilities. Models of this type are called
"market models" or "LIBOR models" or, sometimes, "BGM models," the last
name referring to the paper of Brace, Gatarek and Musiela [@BGM].[^1]
This class of models has become quite popular in recent years. A
thorough and very readable account is given by Rebonato [@Rebonato02].

[^1]: Other important work on this topic includes Miltersen, Sandmann,
    and Sondermann [@MSS] and Jamshidian [@Jamshidian97].

As in
Chap. [\[c_fixedincomederivatives\]](#c_fixedincomederivatives){reference-type="ref"
reference="c_fixedincomederivatives"}, we will use "LIBOR" as a generic
name for simple interest rates. One important fact about forward LIBOR
rates that we have already essentially derived is that they are
...
reference="s_futuresforwards"}, a probability measure corresponding to a
discount bond being the numeraire is called a "forward measure," because
the forward price of any contract maturing at the same time as the
discount bond is a martingale under that measure. We showed in
Sect. [\[s_portfoliosspotrates\]](#s_portfoliosspotrates){reference-type="ref"
reference="s_portfoliosspotrates"} that forward LIBOR rates are forward
prices of portfolios that pay spot rates. Specifically, considering a
LIBOR rate of term (also called "tenor") $\varDelta t$ and the forward
LIBOR rate corresponding to loans over a period $u$ to $u+\varDelta t$,
the forward LIBOR rate is a martingale under the measure corresponding
...
consisting of rolling over an investment at spot LIBOR rates.[^1] We
will derive here the dynamics under different forward measures.

[^1]: Actually, for this theory, it is not even necessary that the short
    rate exist, so the risk-neutral measure may not even be defined. The
    risk-neutral measure uses as numeraire the portfolio that consists
    of continuously rolling over an investment at the instantaneously
    risk-free rate, and the more natural object in a market model is the
    portfolio that consists of rolling over an investment at spot LIBOR
    rates.
...
$\varDelta t$ at the dates $t_1, \ldots, t_N$. Of course, this does not
...
Chap. [\[c_fixedincomederivatives\]](#c_fixedincomederivatives){reference-type="ref"
reference="c_fixedincomederivatives"}. However, even when the $\sigma_i$
are nonrandom, forward swap rates will have random volatilities, as
mentioned in
...
\end{prob}\begin{prob}
Create a VBA function `CIR_Caplet_MC` that values a caplet in the CIR
model using Monte Carlo, without calibrating the model to the current
yield curve. Simulate the CIR process as described in
Sect. [\[s_stochasticvolatility\]](#s_stochasticvolatility){reference-type="ref"
reference="s_stochasticvolatility"} for the Heston model. The inputs
should be $\bar{\mathcal{R}}$, $r(0)$, $\kappa$, $\theta$, $\sigma$,
$T_1$, $T_2$, $N$ and $M$, where $T_1$ is the reset date for the caplet,
$T_2$ is the payment date for the caplet, $N$ is the number of periods
between 0 and $T_1$, and $M$ is the number of simulations. The payoff of
the caplet is as in
Prob. [\[e_holeecaplet2\]](#e_holeecaplet2){reference-type="ref"
reference="e_holeecaplet2"}, where $P(T_1,T_2)$ is the function of
$r(T_1)$ and $T_2-T_1$ given in
[\[cirPcal\]](#cirPcal){reference-type="eqref" reference="cirPcal"} with
$\phi=0$.
\end{prob}
\begin{prob}Modify the function in the preceding exercise to create a function
`CIR_Calibrated_Caplet_MC` that values a caplet in the CIR model using
Monte Carlo, with the model calibrated to the market. Look up market
discount bond prices from a function such as `DiscountBondPrice` in
Prob. [\[exercise121\]](#exercise121){reference-type="ref"
reference="exercise121"}. To compute $\phi(t_i)$ for $i=1,\ldots, N$
from market bond prices, use
[\[circalibration\]](#circalibration){reference-type="eqref"
reference="circalibration"} for dates $t_i$ and $t_{i+1}$ as in
Sect. [\[s_vasicek_swaptions\]](#s_vasicek_swaptions){reference-type="ref"
reference="s_vasicek_swaptions"}.
\end{prob}