...
where $B$ is a Brownian motion.  In the case that the stock pays no dividend, the rate of return is its price change $\mathrm{d} S$ divided by the initial price $S$, so the model states that the expected rate of return in each "instant" $\mathrm{d} t$  is $\mu\,\mathrm{d} t$.  The variance of the rate of return depends on $\sigma$.  This model can be equivalently written in terms of the natural logarithm of $S$.  The above equation for the rate of return is equivalent to
...
We can easily simulate an approximate Brownian motion.  To do so, we take a small time period $\mathrm{d}elta t$ and define the value at the end of the period to be the value of the Brownian motion at the beginning plus a normally distributed variable with mean 0 and variance $\mathrm{d}elta t$.  In the following code, we input the length $T$ of the entire time period over which the Brownian motion is to be simulated and to input the number $N$ of time periods of length $\mathrm{d}elta t$ within the full interval $[0,T]$.  The length $\mathrm{d}elta t$ of each individual time period is then calculated as $T/N$.  The quality of the approximation of this simulation to a true Brownian motion will be always be improved by increasing the number $N$.  Plotting the output of the procedure creates a picture of what we call a path of the Brownian motion, which means that it shows the value taken at each time in one state of the world.  Running the code again (for the same $T$ and $N$) will create a different plot, which can be interpreted as the values of the Brownian motion in another state of the world.  In other words, the path of the Brownian motion is itself random, depending in this approximation on the numbers produced by numpy's random number generating function.

`{python}
#| label: fig-simulateBM
#| fig-cap: "A Simulated Path of a Brownian Motion"
...
dt = T/N
dB = np.random.normal(
    loc=0, scale=np.sqrt(dt), size=N
)
B = np.cumsum([0] + list(dB))

plt.plot(
    np.arange(0, T + dt, dt), B
)
plt.xlabel("Time")
...
## It\^o Processes {#sec-s_itoprocesses}
...
$$\sum_{i=1}^N \sigma(t_{i-1})\,\mathrm{d}elta B(t_i)\; ,$$
where $0=t_0 < \cdots t_N=T$ and the time periods $t_i-t_{i-1}$ are small.  Given that we can simulate the changes $\mathrm{d}elta B(t_i)$ as random normals, we can approximately simulate the random variable $\int_0^T \sigma(t)\,\mathrm{d} B(t)$ and hence we can approximately simulate $X(T)$.

An It\^o process evolves continuously over time.  We interpret $\mu(t)\,\mathrm{d} t$ as the expected change in $X$ in an instant $\mathrm{d} t$.  The quantity $\mu(t)$  is also called the drift of the process $X$ at time $t$.   \index{drift} The coefficient $\sigma(t)$ is called the diffusion coefficient of $X$ at time $t$.  \index{diffusion coefficient}
...
A very important fact is that an It\^o process such as @eq-itoprocess can be a martingale only if $\mu=0$.  This should seem sensible, because $\mu\,\mathrm{d} t$ is the expected change in $X$, and a process is a martingale only if its expected change is zero.\footnote{If the sources of uncertainty in the market can be modeled as Brownian motions, then in fact every martingale is an It\^o process with $\mu=0$.  This is some justification for the assumption we will make in this book, when studying continuous-time models, that all martingales are It\^o processes.}   This observation plays a fundamental role in deriving asset pricing formulas, as we will begin to see in @sec-s_girsanov.
...
Whether $\mu$ is zero or not, and independently of the assumption @eq-regularity1, the quadratic variation of the It\^o process $X$ is 
$$
\lim_{N \rightarrow \infty} \sum_{i=1}^N[\mathrm{d}elta X(t_i)]^2 = \int_0^T \sigma^2(t)\,\mathrm{d} t
...
## Rules for Multiplying Differentials
...
## Differential of Quadratic Variation
...
We integrate this from 0 to $T$ to obtain the quadratic variation @eq-itoquadraticvariation over that time period:\footnote{In a more formal mathematical presentation, one normally writes $\mathrm{d}\langle X,X\rangle$ for what we are writing here as $(\mathrm{d} X)^2$.  This is the differential of the quadratic variation process, and the quadratic variation through date $T$ is
...
## It\^o's Formula {#sec-s_itosformula}
...
We can contrast  @eq-ordinarycalculus  with a special case of It\^o's formula for the calculus of It\^o processes (the more general formula will be discussed in the next section).  If $B$ is a Brownian motion and $Y = g(B)$ for a twice-continuously differentiable function $g$, then \index{It\^o's formula}
$$
Y(T) = Y(0) + \int_0^T g'(B(t))\,\mathrm{d} B(t) + \frac{1}{2}\int_0^T g(B(t))\,\mathrm{d} t\;.
$$ {#eq-itonew1}
Thus, relative to the ordinary calculus, It\^o's formula has an extra term involving the second derivative $g$.  We can write @eq-itonew1 in differential form as
...
To gain some intuition for the extra term in It\^o's formula, we return to the ordinary calculus.  Given dates $t<u$, the derivative defines a linear approximation of the change in $y$ over this time period; i.e., setting $\mathrm{d}elta x = x(u)-x(t)$ and $\mathrm{d}elta y = y(u) - y(t)$, we have the approximation
$$\mathrm{d}elta y \approx g'(x(t)) \,\mathrm{d}elta x\; .$$
A better approximation is given by the second-order Taylor series expansion
$$\mathrm{d}elta y \approx g'(x(t))\,\mathrm{d}elta x + \frac{1}{2} g''(x(t))\,[\mathrm{d}elta x]^2\; .$$
An interpretation of  @eq-ordinarycalculus is that the linear approximation works perfectly for infinitesimal time periods $\mathrm{d} t$, because we can compute the change in $y$ over the time period $[0,T]$ by "summing up" the infinitesimal changes $g'(x(t))\,\mathrm{d} x(t)$.  In other words, the second-order term $\frac{1}{2} g''(x(t))\,[\mathrm{d}elta x]^2$ vanishes when we consider very small time periods.

The second-order Taylor series expansion in the case of $Y=g(B)$ is 
$$\mathrm{d}elta Y \approx g'(B(t))\,\mathrm{d}elta B + \frac{1}{2} g''(B(t))\,[\mathrm{d}elta B]^2\; .$$
...
$$
Y(T) = Y(0) + \sum_{i=1}^N \mathrm{d}elta Y(t_i)  
$$
$$
\approx Y(0) + \sum_{i=1}^N g'(B(t_{i-1}))\,\mathrm{d}elta B(t_i) + \frac{1}{2} \sum_{i=1}^N g''(B(t_{i-1}))\,[\mathrm{d}elta B(t_i)]^2\;.
$$ {#eq-itonew2}
...
$$\lim_{N \rightarrow \infty} \sum_{i=1}^N [\mathrm{d}elta B(t_i)]^2 = T\; ,$$
whereas for the continuously differentiable function $x(t) = f(t)$, the same limit is zero.  In fact it seems sensible to interpret the limit of $[\mathrm{d}elta B]^2$ as $(\mathrm{d} B)^2 =\mathrm{d} t$.
This is perfectly consistent with It\^o's formula: if we take the limit in @eq-itonew2, replacing the limit of $[\mathrm{d}elta B(t_i)]^2$ with $(\mathrm{d} B)^2 = \mathrm{d} t$, we obtain @eq-itonew1.

## Multiple It\^o Processes
...
$$\sum_{i=1}^N \mathrm{d}elta B_x(t_i) \times \mathrm{d}elta B_y(t_i) \rightarrow \int_0^T \rho(t)\,\mathrm{d} t$$
as $N \rightarrow \infty$, with probability one.  

We know that
$$\sum _{i=1}^N [\mathrm{d}elta X(t_i)]^2 \rightarrow \int_0^T \sigma_x^2(t)\,\mathrm{d} t \quad \text{and}\quad  \sum _{i=1}^N [\mathrm{d}elta Y(t_i)]^2 \rightarrow \int_0^T\sigma_y^2(t)\,\mathrm{d} t\;.
$$ {#eq-itoprocess112}
Furthermore, it can be shown that the sum of products satisfies
$$
\sum_{i=1}^N \mathrm{d}elta X(t_i) \times \mathrm{d}elta Y(t_i) \rightarrow \int_0^T \sigma_x(t)\sigma_y(t)\rho(t)\,\mathrm{d} t\;.
$$ {#eq-itoprocess113}

::: {.callout-important}

## Another Rule for Multiplying Differentials

By adding the rule 
$$
(\mathrm{d} B_x)(\mathrm{d} B_y) = \rho\,\mathrm{d} t \tag{\ref{multiplydifferentials}d}
$$ {#eq-dBdB} to the rules @eq-dtsquared--@eq-dBsquared, we can "compute" the limit in @eq-itoprocess113 as
$$
\lim_{N \rightarrow\infty} \sum_{i=1}^N \mathrm{d}elta X(t_i) \times \mathrm{d}elta Y(t_i)  = \int_0^T (\mathrm{d} X)(\mathrm{d} Y)
$$
$$
...
The most general case of It\^o's formula that we will need is for a function $Z(t) = g(t, X(t), Y(t))$ where $X$ and $Y$ are It\^o processes as in @eq-itoprocess110--@eq-itoprocess111.  In this case, It\^o's formula is\footnote{We need to assume $g(t,x,y)$ is continuously differentiable in $t$ and twice continuously differentiable in $(x,y)$ for @eq-itogeneralnew and @eq-itogeneralnew2 to be valid.  Note also that we are using a short-hand notation here.  The partial derivatives of $g$ will generally depend on $t$, $X(t)$ and $Y(t)$ just as $g$ does.}
$$
...
In this equation, we apply the rules @eq-dtsquared--@eq-dBsquared to compute
\begin{align*}
...
\end{align*}
It\^o's formula @eq-itogeneralnew appears a bit simpler (and easier to remember) if we write it in differential form."  We have:

::: {.callout-important}

## Multivariate Ito's Formula

If $Z(t) = g(t, X(t), Y(t))$ where $X$ and $Y$ are It\^o processes as in @eq-itoprocess110--@eq-itoprocess111, then
$$
...
## Examples of It\^o's Formula {#sec-s_examples}
...
## Log Formulation of a Geometric Brownian Motion
The  equation 
$$\frac{\mathrm{d} S}{S} = \mu\,\mathrm{d} t+\sigma\,\mathrm{d} B$$
...
Over a discrete time interval $\mathrm{d}elta t$, @eq-exponential3 implies that the change in the logarithm of $S$ is 
$$
\mathrm{d}elta \log S = \left(\mu -\frac{1}{2}\sigma^2\right)\mathrm{d}elta t+ \sigma\,\mathrm{d}elta B\;.
$$ {#eq-exponential11}
If $S$ is the price of a non-dividend-paying asset, then over the time period $t_{i-1}$ to $t_i$, with $t_i-t_{i-1}=\mathrm{d}elta t$, we have
$$
\mathrm{d}elta \log S = r_i\,\mathrm{d}elta t\;,
$$ {#eq-exponential10}
where $r_i$ is the  continuously compounded annualized rate of return \index{continuously compounded return} during the period $\mathrm{d}elta t$.  This follows from the definition of the continuously compounded rate of return as the constant rate over the time period $\mathrm{d}elta t$ that would cause~$S$ to grow (or fall) from $S(t_{i-1})$ to $S(t_i)$.  To be precise, $r_i$ is defined by
$$\frac{S(t_i)}{S(t_{i-1})} = \mathrm{e}^{r_i\mathrm{d}elta t}\; ,$$
which is equivalent to @eq-exponential10.
Thus, the geometric Brownian motion model @eq-Y implies that the continuously compounded annualized rate of return over a period of length $\mathrm{d}elta t$ is given by
$$r_i = \mu -\frac{1}{2}\sigma^2+ \frac{\sigma\mathrm{d}elta B}{\mathrm{d}elta t}\; .$$
This means that $r_i$ is normally distributed with mean $\mu-\sigma^2/2$ and variance $\sigma^2/\mathrm{d}elta t$.  Given historical data on the rates of return, the parameters $\mu$ and $\sigma$ can be estimated by standard methods (see @sec-c_stochasticvolatility).

We can simulate a path of $S$ by simulating the changes $\mathrm{d}elta \log S$.  The random variable $\sigma \mathrm{d}elta B$  in  @eq-exponential11 has a normal distribution with zero mean and variance equal to $\sigma^2\mathrm{d}elta t$.  We simulate it as $\sigma\sqrt{\mathrm{d}elta t}$ multiplied by a standard normal.
...
We consider the dynamics of the asset price $S$ under three different probability measures.  In each case, we follow the same steps: (i) we note that the ratio of an asset price to the numeraire asset price must be a martingale, (ii) we use It\^o's formula to calculate the drift of this ratio, and (iii) we use the fact that the drift of a martingale must be zero to compute the drift of $\mathrm{d} S/S$.
...
\item when $\mathrm{e}^{qt}S(t)$ is the numeraire, $\alpha = r-q +\sigma_s^2/2$,
...
\begin{prob} Consider a discrete partition $0=t_0 < t_1 < \cdots t_N=T$ of the time interval $[0,T]$ with $t_i - t_{i-1} = \mathrm{d}elta t = T/N$ for each $i$.  Consider the function 
$$X(t)=\mathrm{e}^t\; .$$
Create a VBA subroutine, prompting the user to input $T$ and $N$, which computes and prints $\sum_{i=1}^N [\mathrm{d}elta X(t_i)]^2$, where 
$$\mathrm{d}elta X(t_i) = X(t_i)-X(t_{i-1}) = \mathrm{e}^{t_i} - \mathrm{e}^{t_{i-1}}\; .$$
...
Next i
\end{verbatim}\normalsize
\end{prob}\begin{prob} Repeat the previous problem for the function $X(t) = t^3$.  In both this and the previous problem, what happens to $\sum_{i=1}^N [\mathrm{d}elta X(t_i)]^2$ as $N \rightarrow \infty$?
\end{prob}\begin{prob} Repeat the previous problem to compute $\sum_{i=1}^N [\mathrm{d}elta B(t_i)]^2$, where $B$ is a simulated Brownian motion.  For a given $T$, what happens to the sum as $N \rightarrow \infty$?  
\end{prob}\begin{prob} Repeat the previous problem, computing instead $\sum_{i=1}^N |\mathrm{d}elta B(t_i)|$ where $| \cdot |$ denotes the absolute value.  What happens to this sum as $N \rightarrow \infty$?
\end{prob}\begin{prob} Consider a discrete partition $0=t_0 < t_1 < \cdots t_N=T$ of the time interval $[0,T]$ with $t_i - t_{i-1} = \mathrm{d}elta t = T/N$ for each $i$.  Consider a geometric Brownian motion
...
$$
\mathrm{d}elta \tilde{Z}(t_i) = \tilde{Z}(t_{i-1}) \big[ \mu\,\mathrm{d}elta t + \sigma\,\mathrm{d}elta B\big]\;.
$$ {#eq-exponential111}
The subroutine \verb!Simulating_Geometric_Brownian_Motion! simulates a path~\verb!Z! of a geometric Brownian motion. Modify the subroutine to prompt the user to input $T$, $N$, $\sigma$, $\mu$, and $Z(0)$ and to generate both a path $Z(t)$ and an approximate path $\tilde{Z}(t)$ according to @eq-exponential111, using the same $\mathrm{d}elta B$  for both paths and taking $\tilde{Z}(0) = Z(0)$.  Plot both paths in the same figure.  How well does the approximation work for large $N$?   Warning:  
