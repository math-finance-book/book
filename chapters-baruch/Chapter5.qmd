# Introduction to Monte Carlo and Binomial Models {#sec-c_introcomputation}
...
According to our risk-neutral pricing formula @eq-riskneutralformula, the value of a security paying an amount $x$ at date $T$ is
$$
\mathrm{e}^{-rT}E^R[x]\;.
$$ {#eq-montecarlo1}
To estimate this by Monte-Carlo \index{Monte Carlo} means to simulate a sample of values for the random variable
$x$
and to estimate the expectation by averaging the sample values.\footnote{Boyle~\cite{boyle} introduced Monte-Carlo methods for derivative valuation, including the variance-reduction methods of control variates and antithetic variates to be discussed in @sec-c_montecarlo.  Of course, for this to work, the sample must be generated from a population having a distribution consistent with the risk-neutral probabilities.
...
$$x_i = \max\left(0,\mathrm{e}^{\log S(0)+\nu T + \sigma\sqrt{T}z_i}-K\right)\; .$$
To value options that are path-dependent  we need to simulate the path of the underlying asset price.  Path-dependent options are discussed in @sec-c_exotics and @sec-c_montecarlo.
...
$$ {#eq-standarderror} \index{standard error}
...
To obtain an estimate with an acceptably small standard error may require a large sample size and hence a relatively large amount of computation time.  The complexities of Monte Carlo methods arise from trying to reduce the required sample size.  In @sec-c_montecarlo, we will describe two such methods (antithetic variates and control variates).   For those who want to engage in a more detailed study of Monte Carlo methods, the book of Glasserman \cite{Glasserman} is highly recommended.  J\"ackel \cite{Jackel} is useful for more advanced readers, and Clewlow and Strickland \cite{CS} and  Brandimarte \cite{Brandimarte} are useful references that include computer code.
...
In the binomial model, we assume that if the stock price is $S$ at the beginning of the period, it will be either $uS$ or $dS$ at the end of the period, where the multiplicative factors $u$ and $d$ are constants to be determined.  This means that the rate of return is $\mathrm{d}elta S/S = u\!-\!1$ in the up state and $\mathrm{d}elta S/S = d\!-\!1$ in the down state. There are three parameters to the model: $u$, $d$, and the probability $p$ of the up state (the probability of the down state being necessarily $1\!-\!p$). The following illustrates a three-period model.
...
$$
C = \mathrm{e}^{-r\mathrm{d}elta t}p\,C_{\text{up}} + \mathrm{e}^{-r\mathrm{d}elta t}(1-p)C_{\text{down}}\;.
$$ {#eq-binomial001}
...
$$
C(i) = \mathrm{e}^{-r\mathrm{d}elta t}p\,C(i+1) + \mathrm{e}^{-r\mathrm{d}elta t}(1-p)C(i)\;.
$$ {#eq-binomial002}
...
$$
P(i) = \mathrm{e}^{-r\mathrm{d}elta t}p\,P(i+1) + \mathrm{e}^{-r\mathrm{d}elta t}(1-p)P(i)\;.
$$ {#eq-binomial004}
...
$$
P(i) = \max\big(K-u^{i}d^{n-i}S, \;\mathrm{e}^{-r\mathrm{d}elta t}p\,P(i+1) + \mathrm{e}^{-r\mathrm{d}elta t}(1-p)P(i)\big)\;.
$$ {#eq-binomial005}
...
Several different ways have been proposed for matching the binomial model to the continuous-time model.  Consider an $N$--period binomial model for a time period of $T$ years.  This means that the length of each period is $\mathrm{d}elta t = T/N$.  In the continuous-time model, over a discrete time period $\mathrm{d}elta t$, we have
$$\mathrm{d}elta \log S =\nu\,\mathrm{d}elta t + \sigma\,\mathrm{d}elta B\; ,$$
where $\nu = r-q-\sigma^2/2$ and $B$ is a Brownian motion under the risk-neutral measure.  The mean and variance, under the risk-neutral measure, of $\mathrm{d}elta \log S$ in the continuous-time model are
\begin{align*}
E^R[\mathrm{d}elta \log S] = \nu\,\mathrm{d}elta t\; ,
$$
$$
\mathrm{var}^R[\mathrm{d}elta \log S]=\sigma^2\mathrm{d}elta t\; ,
\end{align*}
so 
\begin{align*}
\frac{E^R[\mathrm{d}elta \log S]}{\mathrm{d}elta t} = \nu\; ,
$$
$$
\frac{\mathrm{var}^R[\mathrm{d}elta \log S]}{\mathrm{d}elta t}=\sigma^2\;.
\end{align*}
In the binomial model, we have
\begin{align*}
\frac{E^R\big[\mathrm{d}elta \log S\big]}{\mathrm{d}elta t} =\frac{p\,\log u+(1-p)\log d}{\mathrm{d}elta t}\; ,
$$
$$
\frac{\mathrm{var}^R\big[\mathrm{d}elta \log S\big] }{\mathrm{d}elta t}=\frac{p\,(1-p)(\log u-\log d)^2}{\mathrm{d}elta t}\;.
\end{align*}
In order for the binomial model to converge in the appropriate sense to the continuous-time model as the number of periods $N \rightarrow \infty$ keeping the total amount of time $T$ fixed (equivalently, as $\mathrm{d}elta t \rightarrow 0$), it is sufficient that
\begin{align*}
\frac{p\log u+(1-p)\log d}{\mathrm{d}elta t} \rightarrow \nu\; ,
$$
$$
\frac{p\,(1-p)(\log u-\log d)^2}{\mathrm{d}elta t} \rightarrow \sigma^2\;.
\end{align*}
...
$$
u = \mathrm{e}^{\sigma\sqrt{\mathrm{d}elta t}}\;,
$$ {#eq-crru}

$$
p = \frac{\mathrm{e}^{(r-q)\mathrm{d}elta t}-d}{u-d}\;.
$$ {#eq-crrp}

Another well-known model is that of Jarrow and Rudd \cite{JR}, \index{Jarrow-Rudd model} who take $p=1/2$ and
$$
u = \exp\left(\left((r-q-\frac{1}{2}\sigma^2\right)\mathrm{d}elta t + \sigma\sqrt{\mathrm{d}elta t}\right)\;,
$$ {#eq-jru}

$$
d = \exp\left(\left((r-q-\frac{1}{2}\sigma^2\right)\mathrm{d}elta t - \sigma\sqrt{\mathrm{d}elta t}\right)\;.
$$ {#eq-jrd}
...
For illustration,  the Cox-Ross-Rubinstein tree will be implemented in @sec-s_introcompvba.  However, when we consider binomial models for multiple assets in @sec-c_montecarlo, we will use the tree proposed by Trigeorgis \cite{Trigeorgis}, \index{Trigeorgis model} because it is the simplest to explain in that context.
Trigeorgis proposes choosing $p$, $u$ and $d$ so that the mean and variance of $\mathrm{d}elta \log S$ in the binomial model match those in the continuous-time model exactly.  This means that
\begin{align*}
\frac{p\log u+(1-p)\log d}{\mathrm{d}elta t} = \nu\; ,
$$
$$
\frac{p(1-p)(\log u-\log d)^2}{\mathrm{d}elta t} = \sigma^2\;.
\end{align*}
...
Solving these two equations yields\footnote{Notice that if we were to drop the $(\mathrm{d}elta t)^2$ term in @eq-trig1 (which we could do because it becomes increasingly negligible as $\mathrm{d}elta t \rightarrow 0)$, then  @eq-trig1 would be the same as @eq-crru.  The different choices of $p$ in @eq-crrp and @eq-trig2 can be understood as follows.  Equation @eq-crrp implies that the expected stock price
$pS_u + (1-p)S_d$ equals $\mathrm{e}^{(r-q)\mathrm{d}elta t}S$, so we have average growth at the rate $r-q$ as in the continuous-time model.  On the other hand,  @eq-trig2 implies that the expected \emph{log} stock price $p \,\log S_u + (1-p) \log S_d$ equals $\log S + \nu \mathrm{d}elta t$, so the expected change in the logarithm is $\nu\mathrm{d}elta t$, also as in the continuous-time model.  Thus, both match the binomial model to the continuous-time model, the Cox-Ross-Rubinstein method focusing on the expected return (equivalently, the expected change in the price of the underlying) and the Trigeorgis method focusing on the expected continuously-compounded return (the expected change in the logarithm of the price).}
$$
\log u=\sqrt{\sigma^2\mathrm{d}elta t + \nu^2(\mathrm{d}elta t)^2}\;,
$$ {#eq-trig1}

$$
p = \frac{1}{2}+\frac{\nu\mathrm{d}elta t}{2\log u}\;.
$$ {#eq-trig2}
...
Assume we have taken $d=1/u$, so after an up and a down move (or a down and an up move) the stock price returns to its initial value $S$.  After fixing the length $\mathrm{d}elta t = T/N$ of each time period, we redefine $N=N+2$.  This results in an $N+2$ period tree covering a time period of length $T+2\mathrm{d}elta t$.
...
The corresponding Monte Carlo estimate of the gamma is also a sample mean.  Simple algebra shows that the formula @eq-binomialgamma100 is equivalent to
$$
...
$$ {#eq-montecarlodelta3}
with the ratio @eq-montecarlodelta2.  
The derivative @eq-montecarlodelta3 takes on two possible values, depending on $S(T)$---we can ignore the case $S(T)=K$ because it occurs with zero probability:
\begin{itemize}
...
\end{itemize}
Therefore, the true delta---the expectation @eq-montecarlodelta4---equals\footnote{By changing numeraires, we can show that @eq-montecarlodelta5 equals $\mathrm{e}^{-qT}E^V[x] = \mathrm{e}^{-qT}\N(d_1)$, as we know from @sec-c_blackscholes is the delta of a European call in the Black-Scholes model (here, as in @sec-c_blackscholes, $V(t)=\mathrm{e}^{qt}S(t)$ denotes the value of the non-dividend-paying portfolio created from the stock).} 
$$
...
To compute pathwise estimates in other models and for other Greeks, we need the Greek to be an expectation as on the right-hand side of @eq-montecarlodelta4.  Additional examples can be found in  Glasserman \cite{Glasserman} and J\"ackel \cite{Jackel}.
...
## Calculations in VBA {#sec-s_introcompvba}

### Monte Carlo Valuation of a European Call

We will illustrate Monte Carlo by valuing a European call under the Black-Scholes assumptions.  We will also estimate the delta by each of the methods described in @se-s_montecarlogreeks1 and @sec-s_montecarlogreeks2.  Of course, we know the call value and its delta from the Black-Scholes formulas, and they can be used to evaluate the accuracy of the Monte Carlo estimates.
...
### Monte Carlo Valuation in a GARCH Model

For another example of Monte Carlo, we will value a European call option and estimate its standard error in a GARCH model.  The underlying asset price is simulated as in @sec-s_garch.  After each path of the underlying is simulated, we compute the date--T value of the option.  We sum these as the simulations proceed in order to compute the average value.  We also sum the squared date--~T option values in order to compute the standard error of the estimate of the date--0 option value.

In addition to the inputs in the previous function, we input  the number $N$ of time periods in the interval $[0,T]$---implying a GARCH model for returns over time intervals of length $\mathrm{d}elta t = T/N$ under the risk-neutral measure---and the GARCH parameters $\kappa$, $\theta$, and~$\lambda$. To value a different type of European derivative, we would simply modify the statement 
\small\begin{center}
...
\noindent Now we calculate the stock price at the bottom node (the node corresponding to all down moves), the probability of reaching that node, and the first term in the sum @eq-binomialsum.
...
Therefore, as we increase the index $i$ in computing the sum @eq-binomialsum, we need to multiply the previous stock price by $u^2$ and multiply the previous probability by $(N-i+1)p/[(1-p)i]$.  We add the result to \verb!CallV! each time and, at the end, discount by $\mathrm{e}^{-rT}$.
...
### Binomial Estimation of Delta and Gamma

We add two periods to the model and then stop the backward induction at date $n=2$, as described in @sec-s_binomial_greeks.
