#A Brief Survey of Term Structure Models {#sec-c_survey

This chapter presets very brief descriptions of several important models.  The list of models is certainly not exhaustive, and our descriptions will be far from complete.   As mentioned previously, there are many good references for this material in book form already, and our goal here is merely to provide a short introduction.

## Ho-Lee
The Ho-Lee \cite{HL model \index{Ho-Lee modelis a binomial version of the Vasicek model without mean reversion, \index{mean reversionin which the one-period interest rate is assumed to have a deterministic drift.  This was the first widely-used model that enabled one to fit the current yield curve.  

Consider discrete times $0=t_0 < t_1 < \cdots t_N$ with $t_{i-t_{i-1=\mathrm{d}elta t$ for each~$i$.  We denote the one-period interest rate from date $t_i$ to $t_{i+1}$ by $r(t_i)$.  We express the rate as an annualized continuously compounded rate, so the one-period discount factor from date $t_i$ to $t_{i+1}$ is $\mathrm{e}^{-r(t_i) \mathrm{d}elta t}$.  We could put this in a continuous-time framework by assuming the short rate is constant ($= r(t_i)$) during each time interval $(t_i, t_{i+1})$.  As always, the risk-neutral measure is the probability measure corresponding to the numeraire
$$
(t) = \exp\left(\int_0^t r(s)\,\mathrm{d} s\right)\; .
...
$$R(t_i) = \exp\left(\sum_{j=0}^{i-1} r(t_{j})\,\mathrm{d}elta t\right)\; .$$
Thus, the continuous-time framework is not necessary.



The model assumes that over each time period $t_{i}-t_{i-1}$ the change $\mathrm{d}elta r(t_i) = r(t_i)-r(t_{i-1})$ in the one-period rate is 
$$ \mathrm{d}elta r(t_i) = \theta(t_i)\,\mathrm{d}elta t \pm \sigma \sqrt{\mathrm{d}elta t}\; ,$$
 ...
 $$\mathrm{d}elta \hat{r}(t_i) = \pm \sigma \sqrt{\mathrm{d}elta t}$$
 for each $i$.  Also, define $\phi(t_i) = \sum_{j=1}^i \theta(t_i)$ for $i \geq 1$.  Then we have, for $i >0$,
 $$r(t_i) = \phi(t_i) + \hat{r}(t_i)\; .$$  

The following illustrates a three-period tree with initial one-period rate $r_0=$5\%, $\mathrm{d}elta t=1$, $\theta(t_i)=0$ for all $i$, and $\sigma=$1\%.    
...
$$
P(0,t_n) = E^R\left[\exp\left(-\sum_{i=0}^{n-1} r(t_i)\,\mathrm{d}elta t\right)\right]
$$
$$
= E^R\left[\exp\left(-r(t_0)\,\mathrm{d}elta t - \sum_{i=1}^{n-1} [\hat{r}(t_i) + \phi(t_i) ]\,\mathrm{d}elta t\right)\right]
$$
$$
= \exp\left(-r(t_0)\,\mathrm{d}elta t-\sum_{i=1}^{n-1}\phi(t_i)\,\mathrm{d}elta t \right)E^R\left[\exp\left(-\sum_{i=1}^{n-1} \hat{r}(t_i) \,\mathrm{d}elta t\right)\right]\;. 
$$ {#eq-holee1}
...
\begin{petit} We will conclude this section with a proof of formula @eq-holee3.  Let $\mathrm{var}epsilon(t_i)$ denote independent  random variables that equal $\pm \sigma \sqrt{\mathrm{d}elta t}$ with probability $1/2$ each under the risk-neutral measure.  Then we can write
$$\hat{r}(t_i) = r(0) + \sum_{j=1}^i \mathrm{var}epsilon(t_j)\; .$$
This implies that
\begin{align*}
...
$$
P(0,t_n) = \exp\left(-nr(0)\,\mathrm{d}elta t-\sum_{i=1}^{n-1}\phi(t_i)\,\mathrm{d}elta t \right)E^R\left[\exp\left(-\sum_{i=1}^{n-1} (n-i)\mathrm{var}epsilon(t_i)\mathrm{d}elta t\right)\right]\;.
$$
Moreover, 
\begin{multline*}
E^R\left[\exp\left(-\sum_{i=1}^{n-1}(n-i)\mathrm{var}epsilon(t_i)\,\mathrm{d}elta t\right)\right]  = E^R \left[ \prod_{i=1}^{n-1}\exp\left(-(n-i)\mathrm{var}epsilon(t_i)\,\mathrm{d}elta t\right)\right]
$$
$$
= \prod_{i=1}^{n-1} E^R \left[ \exp\left(-(n-i)\mathrm{var}epsilon(t_i)\,\mathrm{d}elta t\right)\right]
$$
$$
= \prod_{i=1}^{n-1} \left( \frac{1}{2} \exp\left((n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-(n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right)\;.
\end{multline*}
Thus,
\begin{multline*}
P(0,t_n) = \exp\left(-nr(0)\,\mathrm{d}elta t-\sum_{i=1}^{n-1}\phi(t_i)\,\mathrm{d}elta t\right) 
$$
$$
\times \prod_{i=1}^{n-1} \left( \frac{1}{2} \exp\left((n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-(n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right).
\end{multline*}
Likewise,
\begin{multline*}
P(0,t_{n+1})  = \exp\left(-(n+1)r(0)\,\mathrm{d}elta t-\sum_{i=1}^n\phi(t_i)\,\mathrm{d}elta t\right)
$$
$$
\times \prod_{i=1}^n \left( \frac{1}{2} \exp\left((n+1-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-(n+1-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right)\; ,
\end{multline*}
which we can write as
\begin{multline*}
\exp\left(-(n+1)r(0)\,\mathrm{d}elta t-\sum_{i=1}^n\phi(t_i)\,\mathrm{d}elta t\right)
$$
$$
\times \prod_{i=0}^{n-1} \left( \frac{1}{2} \exp\left((n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-(n-i)\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right)

$$
$$= \exp\left(-r(0)\,\mathrm{d}elta t-\phi(t_n)\,\mathrm{d}elta t\right)
$$
$$
\times \left( \frac{1}{2} \exp\left(n\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-n\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right)P(0,t_n)\;.
\end{multline*}
Thus, the ratio of model prices is
\begin{multline*}
\frac{P(0,t_{n+1})}{P(0,t_n)} = \exp\left(-r(0)\,\mathrm{d}elta t-\phi(t_n)\,\mathrm{d}elta t\right)
$$
$$
\times \left( \frac{1}{2} \exp\left(n\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right) + \frac{1}{2} \exp\left(-n\sigma\sqrt{\mathrm{d}elta t}\,\mathrm{d}elta t\right)\right)\; .
\end{multline*}
Equating this to the ratio of market prices $P^{\text{mkt}}(0,t_{n+1})/P^{\text{mkt}}(0,t_{n})$ gives the formula @eq-holee3.
\end{petit}
...
$$
\mathrm{d}elta \log r(t_i) = \eta(t_i)\,\mathrm{d}elta t \pm \sigma(t_i)\sqrt{\mathrm{d}elta t},
$$ {#eq-bdt1}
...
$$
\mathrm{d}elta \log r(t_i) = \kappa(t_i)[\theta(t_i) - \log r(t_{i-1})]\,\mathrm{d}elta t \pm \sigma(t_i)\sqrt{\mathrm{d}elta t},
$$ {#eq-bdt2}
...
$$a_{2u} = \kappa(t_2)[\theta(t_2) - \{\log r(t_0) +a_1+b_1\}]\,\mathrm{d}elta t\; ,$$
and
$$a_{2d} = \kappa(t_2)[\theta(t_2) - \{\log r(t_0) +a_1-b_1\}]\,\mathrm{d}elta t\; ,$$
so the relation $a_{2d} = a_{2u} + 2b_1-2b_2$ is equivalent to
$$
\kappa(t_2) = \frac{b_1-b_2}{b_1\,\mathrm{d}elta t} = - \frac{1}{\sigma(t_1)}\times\frac{\sigma(t_2)-\sigma(t_1)}{\mathrm{d}elta t} = - \frac{1}{\sigma(t_1)}\times\frac{\mathrm{d}elta \sigma(t_2)}{\mathrm{d}elta t}.
$$ {#eq-bdt4}
...
$$
\kappa(t_i) = - \frac{1}{\sigma(t_{i-1})}\times\frac{\mathrm{d}elta \sigma(t_i)}{\mathrm{d}elta t}.
$$ {#eq-bdt3}

Equations @eq-bdt2 and @eq-bdt3 define the Black-Derman-Toy model.  The free parameters at each date are $\sigma(t_i)$ and $\theta(t_i)$, and the parameter $\kappa(t_i)$ is defined by @eq-bdt3.  Alternatively, one can view $\kappa(t_i)$ and $\theta(t_i)$ as free parameters and define $\sigma(t_i)$ from @eq-bdt3.  Note that $\kappa(t_i)$ can be interpreted as a mean-reversion parameter for $\log r(t_i)$, at least when it is positive (i.e., when $\mathrm{d}elta \sigma(t_i)<0$).  Because there are two free parameters at each date rather than one (as in the Ho-Lee model)  it is possible to match both market bond prices and market yield volatilities or cap prices.
...
towards $\theta$.  This interest rate model was mimicked by Heston \cite{Heston} in his stochastic volatility model discussed in @sec-c_stochasticvolatility.  We will briefly discuss three topics in connection with this model: discount bond prices, calibrating the model to the current market, and pricing fixed-income derivatives.

### Discount Bond Prices in the CIR Model}

Discount bond prices can be most easily computed in the CIR model by solving the fundamental partial differential equation (pde) discussed in @sec-c_pde.  Let $P(t,u)$ denote the price at date $t$ of a discount bond maturing at date $u$, having remaining time to maturity of $\tau = u-t$.  The discount bond price will depend on the remaining time to maturity and the short rate at date $t$, because, as in the Vasicek model, the short rate is the only random factor in this model.  Thus, there must be some deterministic function $f$ such that $P(t,u) = f(r(t),\tau).$  As in @sec-c_pde, the fundamental pde is obtained by applying It\^o's formula to $f$ to compute $\mathrm{d} f$ in terms of the partial derivatives of $f$ and then using the fact that the expected return of the discount bond (hence the drift of $\mathrm{d} f/f$) must equal the short rate under the risk-neutral measure.

From It\^o's formula and the definition @eq-cir of $\mathrm{d} r$, we have
$$
...
Equating the drift to $rf\,\mathrm{d} t$ gives us the fundamental pde: \index{fundamental pde}
$$
...
The solution can be obtained by guessing a solution of the same form as the Vasicek bond pricing formula @eq-vasicekbond, namely\footnote{This guess works because the CIR model, like the Vasicek model, is an affine model.  See Sect.~\ref{.}
$$
...
where the primes denote derivatives.  By differentiating, one can verify that the solution of  @eq-betaprime (which is called a Riccati equation) \index{Riccati equation}with the boundary condition $b(0)=0$ is 
...
$$ {#eq-cirdpp}
where $\tau = u-t$ and $a(\tau)$ and $b(\tau)$ are defined in @eq-cirbondcombined.
}
...
$$ {#eq-cirdpp2}
for each $u$.
Note that the calibration does not affect discount bond returns: the expected return under the risk-neutral measure must still be the short rate and the volatility is unaffected by a deterministic factor.  Therefore, we have, as in  @eq-cirdpp,
$$
\frac{\mathrm{d} P(t,u)}{P(t,u)}  =   r(t)\,\mathrm{d} t - b(\tau)\sigma\sqrt{\hat{r}(t)}\,\mathrm{d} B(t)\;.
$$ {#eq-ciroptionprice}
This can be verified by applying It\^o's formula to  @eq-cirPcal.
...
Consider a call option maturing at date $T$ with the underlying being a discount bond maturing at $u>T$.  Let $K$ denote the strike price.  From our fundamental pricing formula @eq-formula, the value at date~0 of the option is
$$
...
$$\frac{\mathrm{e}^{\int_0^t r(s)\,\mathrm{d} s}}{P(t,u)}$$
...
\end{align*}
Given that $Z$ is a martingale, $\mathrm{d} Z/Z$ cannot have a drift, so it must be that~$B^*$ defined by $B^*(0)=0$ and
$$\mathrm{d} B^*(t) = \mathrm{d} B(t) + b(\tau)\sigma\sqrt{\hat{r}(t)}\,\mathrm{d} t$$
is a martingale, and hence a Brownian motion, when $P(t,u)$ is used as the numeraire.  Substituting this into the definition @eq-cirrhat of $\hat{r}$, we have
$$
...
provided $\sigma_1\neq \sigma_2$.  Making these substitutions for $x_1$ and $x_2$ on the right-hand side of  @eq-ls and noting that $\mathrm{d} r = \mathrm{d} x_1+\mathrm{d} x_2$ and $\mathrm{d} V = \sigma_1^2\,\mathrm{d} x_1 + \sigma_2^2\,\mathrm{d} x_2$, we obtain the following equations presented by Longstaff and Schwartz:
...
$$ {#eq-lsdiscount}
where the functions $a_i$ and $b_i$ are defined in @eq-cirbondcombined, using the parameters $\kappa_i$, $\theta_i$ and $\sigma_i$.
The expectations @eq-lsdiscount can be written in terms of $r(t)$ and $V(t)$ by substituting from @eq-x12lscombined.
...
$$
P(t,u)= \exp\left( - \int _t^u f(t,s)\,\mathrm{d} s \right)\; .\tag{\ref{hjmforward}$'$}
$$ {#eq-bondsandforwards}
Heath, Jarrow and Morton use the definition @eq-bondsandforwards and the fact that the expected return of a discount bond under the risk-neutral measure must be the short rate to derive a formula for the drifts of forward rates under the risk-neutral measure.  
...
To see how some of the models we have discussed can be written in the HJM form, let us re-examine the Hull-White model.  From the Hull-White bond price formula @eq-extendedvasicekbondcombined,  the instantaneous forward rate in the Hull-White model is
\begin{align*}
...
then we have $\mathrm{d} \Sigma(t,u)/\mathrm{d} u = \sigma(t,u)$, and the HJM @eq-hjm can be written as
...
Specifying the volatilities and correlations of  forward rates, and inputting initial  forward rates, must therefore determine the value of any security whose cash flows depend on the LIBOR rates of term $\mathrm{d}elta t$ at the dates $t_1, \ldots, t_N$.  Of course, this does not mean that there are simple formulas.  Obviously, the easiest case is when the volatilities and correlations are nonrandom.  In this case, we can use Black's formula to price caps and floors as in @sec-c_fixedincomederivatives.  However, even when the $\sigma_i$ are nonrandom, forward swap rates will have random volatilities, as mentioned in 
Sect.~\ref{s_consistency}.

If $i = n-1$, then @eq-marketmodel1 implies
$$\mathcal{R}_i(t) = \frac{P(t,t_{n-1})-P(t,t_n)}{P(t,t_n)\,\mathrm{d}elta t}\; .$$
...
$$ {#eq-marketmodelz}
Note that $Y$ is the ratio of a non-dividend-paying asset price to the price of the numeraire asset and hence is a martingale.  Furthermore,  @eq-marketmodel1 gives us
$$ Z(t) = \frac{P(t,t_{i})-P(t,t_{i+1})}{P(t,t_n)\,\mathrm{d}elta t}\; ,$$
...
It\^o's formula applied to @eq-marketmodelz yields
...
$$
X_j(t) = 1+ \mathcal{R}_j(t)\,\mathrm{d}elta t
$$ {#eq-marketmodelx}
for $j = 1, \ldots , N$.  Then we have
$$
\frac{\mathrm{d} X_j}{X_j} = \left(\frac{\mathcal{R}_j\,\mathrm{d}elta t}{1+\mathcal{R}_j\,\mathrm{d}elta t}\right)\big(\mu_j\,\mathrm{d} t + \sigma_j\,\mathrm{d} B_j\big)\;.
$$ {#eq-marketmodel5}
...
$$
$$
= \sum_{j=i+1}^{n-1} \left(\frac{\mathcal{R}_j\,\mathrm{d}elta t}{1+\mathcal{R}_j\,\mathrm{d}elta t}\right)\sigma_i\sigma_j\rho_{ij}\,\mathrm{d} t\;.
\end{align*}
...
$$
$$
= -\sum_{j=n}^{i} \left(\frac{\mathcal{R}_j\,\mathrm{d}elta t}{1+\mathcal{R}_j\,\mathrm{d}elta t}\right)\sigma_i\sigma_j\rho_{ij}\,\mathrm{d} t\;.
\end{align*}
We conclude:
\mybox{
...
$$
\mu_i(t) = \begin{cases}
\sum_{j=i+1}^{n-1} \left(\frac{\mathcal{R}_j(t)\,\mathrm{d}elta t}{1+\mathcal{R}_j(t)\,\mathrm{d}elta t}\right)\sigma_i\sigma_j\rho_{ij}  \text{if $i < n-1$}\; ,
$$
$$
0  \text{if $i = n-1$}\; ,
$$
$$
-\sum_{j=n}^{i} \left(\frac{\mathcal{R}_j(t)\,\mathrm{d}elta t}{1+\mathcal{R}_j(t)\,\mathrm{d}elta t}\right)\sigma_i\sigma_j\rho_{ij}  \text{if $i > n-1$}\;. \end{cases}
$$ {#eq-e_holeecaplet}}

\section*{Problems}\addcontentsline{toc}{section}{Problems}
\begin{prob}  Create an Excel worksheet demonstrating a four-period Ho-Lee model.  Allow the user to input $\sigma$, $\mathrm{d}elta t$, and $P^{\text{mkt}}(t_n)$ for $n=1, \ldots, 5$.  Compute $\phi(t_n)$ from @eq-holee3 for $n=1,\ldots, 4$.  Create the one-period interest rate tree (starting from $r(0) = -\log P^{\text{mkt}}(t_1)/\mathrm{d}elta t$) and the valuation tree for a discount bond maturing at $t_5$.  Verify that the tree gives the price $P^{\text{mkt}}(t_5)$.  Note: to create a binomial tree in a spreadsheet, it is probably easiest to put the topmost (or bottommost) nodes along one row and the other nodes in a triangle below (or above).
\end{prob}\begin{prob} 
...
$$\max\left(0, \mathcal{R}(t_4)-\bar{\mathcal{R}}\right)\mathrm{d}elta t,$$
where
$$\mathcal{R}(t_4) = \frac{1}{P(t_4,t_5)}-1.$$
Allow the user to input $\bar{\mathcal{R}}$.
\end{prob}\begin{prob}
Create a function \verb!Ho_Lee_Caplet! that values a caplet in the Ho-Lee model.  Look up market discount bond prices from a function such as \verb!DiscountBondPrice! in Prob.~\ref{exercise121} and calibrate to the market from @eq-holee3.  The inputs to the function should be $\bar{\mathcal{R}}$, $\sigma$, $T_1$, $T_2$, $N_1$ and $N_2$, where $T_1$ is the reset date for the caplet, $T_2$ is the payment date for the caplet, $N_1$ is the number of periods between date 0 and~$T_1$, and $N_2$ is the number of periods between $T_1$ and $T_2$.  Note that the payoff of the caplet at date $T_2$ is
$$ {#eq-e_holeecaplet2}
\max\left(0, \mathcal{R}(T_1)-\bar{\mathcal{R}}\right) \times (T_2-T_1),$$
where
$$\mathcal{R}(T_1) = \frac{1}{P(T_1,T_2)}-1.$$
\end{prob}
\begin{prob} Create an Excel worksheet demonstrating a four-period Black-Derman-Toy model.  Allow the user to input $\mathrm{d}elta t$, $r(0)$, and $\sigma(t_i)$ and $\theta(t_i)$ for $i=1,\ldots, 4$.  Create the one-period interest rate and the valuation tree for a discount bond maturing at $t_5$. 
\end{prob}
\begin{prob}Modify the preceding exercise by including the valuation tree for a caplet with reset date $t_4$ and payment date $t_5$, as in Prob.~\ref{e_holeecaplet}. 
\end{prob}\begin{prob}
