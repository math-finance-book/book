...
where $\nu =r-q-\sigma^2/2$ and $B$ is a Brownian motion under the risk-neual measure.
...
$$\frac{\partial S}{\partial X}=\mathrm{e}^X=S\; .$$  Therefore, by the chain rule of calculus,
...
Consider a point $(t_i, x_j)$.  We could denote the estimated value of the call at this point by $C_{ij}$ but for now we will just use the symbol $C$.  Think of $t$ being on the horizontal axis and $x$ on the vertical axis.  There are four points that can be reached from $(t_i,x_j)$ by one step (an increase or decrease) in either~$t$ or $x$.  Let's denote the estimated call value at $(t_i, x_j+\mathrm{d}elta x)$ as $C_{\text{up}}$, the value at $(t_i,x_j-\mathrm{d}elta x)$ as $C_{\text{down}}$, the value at $(t_i+\mathrm{d}elta t, x_j)$ as $C_{\text{right}}$ and the value at $(t_i-\mathrm{d}elta t,x_j)$ as $C_{\text{left}}$.
...
\frac{\partial C}{\partial X} \approx \frac{C_{\text{up}}-C_{\text{down}}}{2\mathrm{d}elta x}\; .
$$ {#eq-pdedelta}
There are two other obvious estimates of this derivative:
$$\frac{C_{\text{up}}-C}{\mathrm{d}elta x} \qquad \text{and} \qquad \frac{C-C_{\text{down}}}{\mathrm{d}elta x}\; .$$
The first of these should be understood as an estimate at the midpoint of $x_j$ and $x_j+\mathrm{d}elta x$ and the second as an estimate at the midpoint of $x_j$ and $x_j-\mathrm{d}elta x$.  The distance between these two midpoints is $\mathrm{d}elta x$, so the difference in these two estimates of $\partial C/\partial X$ divided by $\mathrm{d}elta x$ is an estimate of the second derivative:
$$
\frac{\partial^2 C}{\partial X^2} \approx \frac{C_{\text{up}}-2C+C_{\text{down}}}{(\mathrm{d}elta x)^2}\; .
$$ {#eq-pdegamma}

The obvious estimate of $\partial C/\partial t$, which is analogous to the estimate of $\partial C/\partial X$, is
$$\frac{C_{\text{right}}-C_{\text{left}}}{2\mathrm{d}elta t}\; .$$
...
$$
\frac{\partial C}{\partial t} \approx \frac{C-C_{\text{left}}}{\mathrm{d}elta t}\;,
$$ {#eq-explicit_dt}
...
or
$$
\frac{\partial C}{\partial t} \approx \frac{C_{\text{right}}-C}{\mathrm{d}elta t}\;. \tag{\ref{explicit_dt}$'$}
$$ {#eq-implicit_dt}
...
$$
rC = \frac{C-C_{\text{left}}}{\mathrm{d}elta t}+ \nu\left(\frac{C_{\text{up}}-C_{\text{down}}}{2\mathrm{d}elta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C_{\text{up}}+C_{\text{down}}-2C}{(\mathrm{d}elta x)^2}\right)\;.
$$ {#eq-explicit}
...
p = 1- p_u-p_d\;.
\end{align*}
This can be interpreted as discounting the probability-weighted values of the call at the next date, where we consider that starting at the grid point $(t_{i},x_j)$, the logarithm of the stock price takes three possible values ($x_j-\mathrm{d}elta x$, $x_j$, and $x_j+\mathrm{d}elta x$) at the next date $t_{i+1}$, and where we use $1-r\mathrm{d}elta t$ as the discount factor. Thus, it is essentially a trinomial model.  \index{trinomial model}This relationship was first noted by Brennan and Schwartz \cite{BrennanSchwartz}. 

Actually, for this to be a sensible trinomial model, the probabilities $p_u$, $p$ and $p_d$ should be nonnegative.  Assuming $1-r\mathrm{d}elta t>0$, this will be the case if and only if
$$\mathrm{d}elta x \leq  \frac{\sigma^2}{|\nu|} \qquad \text{and} \qquad \mathrm{d}elta t \leq \frac{(\mathrm{d}elta x)^2}{\sigma^2 + r(\mathrm{d}elta x)^2}\; .$$
The first of these conditions characterizes $p_u$ and $p_d$ being nonnegative.  The second is derived from $p_u+p_d \leq 1$.  It is interesting to examine these conditions in terms of the number $N$ of time periods and the number of steps in the $x$ dimension, which is $2M$.  To simplify the notation in the following somewhat, denote the distance of the upper $x$ boundary from $x_0$ by $D$ (i.e., $D=x_M-x_0$).  Then $\mathrm{d}elta t=T/N$ and $\mathrm{d}elta x = D/M$.  The probabilities are nonnegative if and only if
...
$$
rC = \frac{C_{\text{right}}-C}{\mathrm{d}elta t}+ \nu\left(\frac{C_{\text{up}}-C_{\text{down}}}{2\mathrm{d}elta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C_{\text{up}}+C_{\text{down}}-2C}{(\mathrm{d}elta x)^2}\right)\;.
$$ {#eq-implicit}
...
$\partial C/\partial t$ at the midpoint of $(t_i,x_j)$ and $(t_{i+1},x_j)$, i.e., at $(t_i+\mathrm{d}elta t/2,x_j)$.  This is the basic idea of the Crank-Nicolson method.  With this method, we continue to estimate the call values at the grid points, but we do so by forcing  @eq-3 to hold at midpoints of this type.  To do this, we also need estimates of $C$, $\partial C/\partial X$ and $\partial^2 C/\partial X^2$ at the midpoints, but these are easy to obtain.

Let's modify the previous notation somewhat, writing $C'$ for $C_{\text{right}}$ and $C'_{\text{up}}$ and $C'_{\text{down}}$ for the values to the right and one step up and down, i.e., at the grid points $(t_i+\mathrm{d}elta t,x_i+\mathrm{d}elta x)$ and $(t_i+\mathrm{d}elta t,x_i-\mathrm{d}elta x)$ respectively.  The obvious estimate of the call value at the midpoint $(t_i+\mathrm{d}elta t/2,x_j)$ is the average of $C$ and $C'$, so set
...
$$ {#eq-cn1000}
The formulas @eq-cn1000 give us estimates of the call value at the midpoints one space step up and one space step down from $x_j$---i.e., at $(t_i+\mathrm{d}elta t/2,x_{j+1})$ and $(t_i+\mathrm{d}elta t/2,x_{j-1})$.  We can now estimate
$\partial C/\partial X$ and $\partial^2 C/\partial X^2$ at the midpoint $(t_i+\mathrm{d}elta t/2,x_j)$ exactly as before:
$$\frac{\partial C}{\partial X} \approx \frac{C^{\text{mid}}_{\text{up}}-C^{\text{mid}}_{\text{down}}}{2\mathrm{d}elta x}\; ,$$
and
$$\frac{\partial^2 C}{\partial X^2} \approx \frac{C^{\text{mid}}_{\text{up}}+C^{\text{mid}}_{\text{down}}-2C^{\text{mid}}}{(\mathrm{d}elta x)^2}\; .$$
Now,  @eq-3 becomes
$$
rC^{\text{mid}} = \frac{C'-C}{\mathrm{d}elta t}+ \nu\left(\frac{C^{\text{mid}}_{\text{up}}-C^{\text{mid}}_{\text{down}}}{2\mathrm{d}elta x}\right)+ \frac{1}{2}\sigma^2\left(\frac{C^{\text{mid}}_{\text{up}}+C^{\text{mid}}_{\text{down}}-2C^{\text{mid}}}{(\mathrm{d}elta x)^2}\right)\;.
$$ {#eq-crank}
Substituting from the formulas for $C^{\text{mid}}$, $C^{\text{mid}}_{\text{up}}$, and $C^{\text{mid}}_{\text{down}}$, we can re-write @eq-crank as
\begin{multline}
\left(\frac{r}{2}+\frac{1}{\mathrm{d}elta t}+\frac{\sigma^2}{2(\mathrm{d}elta x)^2}\right)C - \left(\frac{\sigma^2}{4(\mathrm{d}elta x)^2}+\frac{\nu}{4\mathrm{d}elta x}\right)C_{\text{up}}
$$ {#eq-crank2}

$$ - \left(\frac{\sigma^2}{4(\mathrm{d}elta x)^2}-\frac{\nu}{4\mathrm{d}elta x}\right)C_{\text{down}} 
\quad = \quad \left(\frac{1}{\mathrm{d}elta t}- \frac{r}{2}-\frac{\sigma^2}{2(\mathrm{d}elta x)^2}\right)C' 
$$
$$+ \left(\frac{\sigma^2}{4(\mathrm{d}elta x)^2}+\frac{\nu}{4\mathrm{d}elta x}\right)C'_{\text{up}} + \left(\frac{\sigma^2}{4(\mathrm{d}elta x)^2}-\frac{\nu}{4\mathrm{d}elta x}\right)C'_{\text{down}}\tag{\ref{crank}$'$}
\end{multline}
...
As before, we start at the final date $t_N$ and define the call value at that date by its intrinsic value.  Consider a grid point $(t_{N-1},x_j)$.  Forcing  @eq-3 to hold at the midpoint $(t_{N-1}+\mathrm{d}elta t/2,x_j)$ leads us to @eq-crank3.  In this equation, $C'$, $C'_{\text{up}}$ and $C'_{\text{down}}$ are known from the intrinsic value at maturity, and we need to solve for $C$, $C_{\text{up}}$ and $C_{\text{down}}$.  There are $2M-1$ linear equations of this type and we will add linear equations at the upper and lower boundaries of the grid and solve the resulting system of $2M+1$ linear equations for the $2M+1$ call values.  After finding the call values at date $t_{N-1}$, we then repeat the calculation at $t_{N-2}$ and continue backing up in this way until we reach the initial date.

Notice that the Crank-Nicolson @eq-crank2 are similar to the @eq-implicit in the implicit method, but more information is used in each step of the Crank-Nicolson method than is used in each step of the implicit method. Equation @eq-crank2 links the call values $C$, $C_{\text{up}}$ and $C_{\text{down}}$ to the previously calculated $C'$, $C'_{\text{up}}$ and $C'_{\text{down}}$, whereas in the implicit method they were linked only to $C'$ (which we called $C_{\text{right}}$).
...
$$
$$
and by forcing each value $C$ at a point $(t_i,x_{M
$$)$ on the top of the grid to satisfy}
...
$$ {#eq-crank4aabbcombined}
 {#eq-s_finitedifferences_matlab}
 {#eq-s_finitedifferencebarriers}
...
### Crank-Nicolson for Barrier Options}
...
The main new issue that we encounter in valuing barriers is locating the boundary of the grid at the barrier.  For the down-and-out, we will input the value of the stock price at which the option is knocked out as \verb!Bar!.  We want the bottom of the grid to lie at the natural logarithm of this number.  This will influence our choice of the space step $\mathrm{d}elta x$, because we want to have an integer number of steps between the bottom of the grid and $\log S(0)$.  We assume that the value $M$ input by the user represents the desired number of space steps above $\log S(0)$.  We start with $\mathrm{d}elta x=D/M$ as an initial estimate of the size of the space step.  We then decrease it, if necessary, to ensure that the distance between \verb!Bar! and $\log S(0)$ is an integer multiple of $\mathrm{d}elta x$.  We then increase~$M$, if necessary, to ensure that the top of the grid will still be at or above $D+\log S(0)$.  Finally, we define the top of the grid to be at $\log S(0) +M \cdot \mathrm{d}elta x$.  
...
\end{prob}\begin{prob}Create a VBA function \verb!European_Call_Explicit! that uses the explicit method @eq-fdtrinomial to value a European call option.
\end{prob}
\begin{prob}
Write the system of @eq-implicit for the implicit method, together with boundary conditions of the form @eq-crank4aabbcombined as a matrix system and solve for $u_j$ and $b_j$ in @eq-crank101combined, as in the subsection that defines the function \verb!CrankNicolson!.
\end{prob}
