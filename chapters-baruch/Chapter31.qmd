...
\end{cases}
$$
 Using the risk-neutral pricing formula @eq-riskneutralformula, the value of the digital at date~0 is $\mathrm{e}^{-rT}E^R[x]$.  Note that
\begin{align*}
...
From this fact and the formula @eq-tailprob02, we conclude:
...
From @sec-s_reinvestingdividends, we know that $V(t)=\mathrm{e}^{qt}S(t)$ is the price of a non-dividend-paying portfolio.  From our fundamental pricing formula @eq-formula, using $V$ as the numeraire, we have
\begin{align*}
...
\end{align*}
and from the formula @eq-tailprob02 we have
$$\text{prob}^V\!\big(S(T)<K\big) = \mathrm{N}(-d_1)\; .$$
We conclude:

::: {.callout-important}

## Lower-Tail Share Digital
...
To compute the real-world distribution of gains and losses from a discretely-rebalanced delta hedge, we input the expected rate of return $\mu$.  We consider adjusting the hedge at dates $0=t_0<t_1<\cdots<t_N=T$, with $t_i-t_{i-1}=\mathrm{d}elta t = T/N$ for each $i$.  The changes in the natural logarithm of the stock price between successive dates $t_{i-1}$ and $t_i$ are simulated as
$$\mathrm{d}elta \log S = \left(\mu-q-\frac{1}{2}\sigma^2\right)\,\mathrm{d}elta t + \sigma\,\mathrm{d}elta B\; ,$$
where $\mathrm{d}elta B$ is normally distributed with mean zero and variance $\mathrm{d}elta t$.  The random variables $\mathrm{d}elta B$ are simulated as standard normals multiplied by
$\sqrt{\mathrm{d}elta t}$.  We begin with the portfolio that is short a call, long $\delta$ shares of the underlying, and short $\delta S-C$ in cash.  After the stock price changes, say from $S$ to $S'$, we compute the new delta $\delta'$.  The cash flow from adjusting the hedge is $(\delta-\delta')S'$.  Accumulation (or payment) of interest on the cash position is captured by the factor $e^{r\mathrm{d}elta t}$.  Continuous payment of dividends is modelled similarly: the dividends earned during the period $\mathrm{d}elta t$ is taken to be $\delta S\left(e^{q\mathrm{d}elta t}-1\right)$.  The cash position is adjusted due to interest, dividends, and the cash flow from adjusting the hedge.  At date $T$, the value of the portfolio is the cash position less the intrinsic value of the option.
...
\begin{prob}  Create an Excel worksheet in which the user inputs $K$, $r$, $\sigma$, $q$ and $T$.  Compute the delta of a call option 
