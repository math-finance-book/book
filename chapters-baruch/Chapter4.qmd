...
It is important to know how much variation there would be in $\bar{x}$ if one had access to multiple random samples.  More variation means that an $\bar{x}$ computed from a single sample will be a less reliable estimate of $\mu$.  The variance of $\bar{x}$ in repeated samples is $\sigma^2/N$,\footnote{The variance of $\bar{x} = (1/N)(x_1 + \cdots + x_N)$ is, by independence of the $x_i$, equal to $(1/N)^2(\mathrm{var}{x_1} + \cdots + \mathrm{var}{x_N})$, and, because the $x_i$ all have the same variance $\sigma^2$, this is equal to $(1/N)^2 \times N\sigma^2 = \sigma^2/N$.} and our best estimate of this variance is $s^2/N$.  The standard deviation of $\bar{x}$ in repeated samples, which is called the standard error of \index{standard error} $\bar{x}$, is $\sigma/\sqrt{N}$, and we estimate this by $s/\sqrt{N}$, which equals
...
Over a discrete time period of length $\mathrm{d}elta t$, this implies
$$
\mathrm{d}elta \log S = \left(\mu-\frac{1}{2}\sigma^2\right)\mathrm{d}elta t + \sigma \mathrm{d}elta B\;.
$$ {#eq-dlogs}
Suppose we have observed the asset price $S$ at dates $0=t_0<t_1<\cdots< t_N=T$, where $t_i-t_{i-1}=\mathrm{d}elta t$.  If the asset pays dividends, we will take~$S$ to be the value of the portfolio in which the dividends are reinvested in new shares.  Thus, in general, $S(t_i)/S(t_{i-1})$ denotes the gross return (one plus the rate of return) between dates $t_{i-1}$ and $t_i$.  This return is measured on a non-compounded and non-annualized basis.  The annualized continuously-compounded rate of return is the rate $r_i$ defined by 
$$\frac{S(t_i)}{S(t_{i-1})} = \mathrm{e}^{r_i\mathrm{d}elta t}\; .$$
This implies that
$$
r_i = \frac{\log S(t_i)-\log S(t_{i-1})}{\mathrm{d}elta t} = \mu-\frac{1}{2}\sigma^2 + \sigma \frac{B(t_i)-B(t_{i-1})}{\mathrm{d}elta t}\;.
$$ {#eq-contcompreturn}
Because $B(t_i)-B(t_{i-1})$ is normally distributed with mean zero and variance~$\mathrm{d}elta t$, the sample $\{r_1,\ldots,r_N\}$ is a sample of independent random variables each of which is normally distributed with mean $\mu-\sigma^2/2$ and variance $\sigma^2/\mathrm{d}elta t$.  We are focused on estimating $\sigma^2$, so it will simplify things to define
$$
y_i = r_i\sqrt{\mathrm{d}elta t} = \frac{\log S(t_i)-\log S(t_{i-1})}{\sqrt{\mathrm{d}elta t}}\;.
$$ {#eq-volyi}
The sample $\{y_1,\ldots,y_N\}$ is a sample of independent random variables each of which is normally distributed with mean $(\mu-\sigma^2/2)\sqrt{\mathrm{d}elta t}$ and variance $\sigma^2$.
...
$$\hat{\mu} = \frac{\bar{y}}{\sqrt{\mathrm{d}elta t}} + \frac{1}{2}\hat{\sigma}^2 = \bar{r}+ \frac{1}{2}\hat{\sigma}^2\; .$$ 

Let us digress for a moment to discuss the reliability of $\hat{\mu}$ as an estimate of $\mu$.  Notice that 
$$
\bar{r} 
= \frac{\sum_{i=1}^N \log S(t_i)-\log S(t_{i-1})}{N\mathrm{d}elta t}
$$
$$
=   \frac{\log S(T)-\log S(0)}{N\mathrm{d}elta t}
$$
$$
...
$$ {#eq-estimator_sig2}
From the definition @eq-volyi of $y_i$ and @eq-volrbar, we have
$$\bar{y} =  \frac{\sqrt{\mathrm{d}elta t}}{T}[\log S(T)-\log S(0)]\; .$$
Hence, the second term in @eq-estimator_sig2 is
$$ \frac{N}{N-1}\left(\frac{\mathrm{d}elta t}{T^2}\right)[\log S(T)-\log S(0)]^2\; .$$
If we observe the stock price sufficiently frequently, so that $\mathrm{d}elta t$ is very small, this term will be negligible.  In this circumstance,  $\hat{\sigma}^2$ is approximately
$$
\frac{1}{N-1}\sum_{i=1}^N y_i^2 = \frac{1}{N-1}\sum_{i=1}^N \frac{[\log S(t_i)-\log S(t_{i-1})]^2}{\mathrm{d}elta t}
$$
$$
= \frac{N}{N-1}\times \frac{1}{T}\times \sum_{i=1}^N [\log S(t_i)-\log S(t_{i-1})]^2 \;.
$$ {#eq-estimator_sig2_3}

If we observe $S$ more and more frequently, letting $\mathrm{d}elta t \rightarrow 0$ and $N \rightarrow \infty$, the sum 
...
$\mu=r-q$, and we have
$$
\log S(t_{i+1}) - \log S(t_i) = \left(r-q-\frac{1}{2}\sigma_{i+1}^2\right)\mathrm{d}elta t + \sigma_{i+1} \mathrm{d}elta B\;.
$$ {#eq-dlogs2}
...
$$y_i = \frac{\log S(t_i)-\log S(t_{i-1})-\left(r-q-\frac{1}{2}\sigma_i^2\right)\mathrm{d}elta t}{\sqrt{\mathrm{d}elta t}}\; .$$
...
Under these assumptions, the random process $\log S$ is called a \index{GARCH process} GARCH(1,1) process.\footnote{GARCH is the acronym for Generalized Autoregressive Conditional Heteroskedastic.  GARCH(1,1) means that there is only one past $y$  (no $y_{i-1}$, $y_{i-2}$, etc.) and one past $\sigma$ (no  $\sigma_{i-1}$, $\sigma_{i-2}$, etc.) in  @eq-garch.  See Bollerslev \cite{Bollerslev}.}  There are many varieties of GARCH processes that have been proposed in the literature, but we will only consider GARCH(1,1), which is the simplest.
...
The virtue of modelling volatility as in @eq-heston2 is that these probabilities can be computed quite efficiently, as shown by Heston \cite{Heston}.\footnote{Further discussion can be found in Epps \cite{Epps}.}  There are many other ways in which one could model volatility, but the computations may be more difficult.  For example, one could  replace @eq-heston2 by
$$\tag{\ref{heston2}$'$}
...
\begin{prob}  The purpose of this exercise is to generate a fat-tailed distribution from a model that is simpler than the GARCH and stochastic volatility models but has somewhat the same flavor.  The distribution will be a mixture of normals. Create an Excel worksheet in which the user can input $S$, $r$, $q$, $T$, $\sigma_1$ and $\sigma_2$.  Use these inputs to produce a column of 500 simulated $\log S(T)$.  In each simulation, define $\log S(T)$ as
$$ {#eq-e_mixture}
