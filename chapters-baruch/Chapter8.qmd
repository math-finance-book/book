...
W wi oy discss  w exotic optios.  e easo fo stdyig the derivatio of an option pricing forma is that it may better eqip one to analyze new prodcts.  Hopeflly, this chapter will be of some assistance in that regard.  Of course, one cannot expect to derive a closed-form solution for the value of every product, and often numerical methods  will be necessary.  For a much more comprehensive presentation of exotic option pricing formulas, the book by Haug \citeHaug and the Excel software that accompanies it are highly recommended.  Zhang \citeZhang is also a comprehensive reference for exotics. 

The valuation of an American call option on an asset paying a discrete cash dividend (rather than a continuous dividend) is considered in Sect.~\ref.  Under a particular assumption on the volatility, valuing this option is very similar to valuing a compound option.  Except for the assumption of a discrete dividend in Sect.~\refs_discrete}, we will make the Black-Scholes assumptions throughout this chapter: there is a constant risk-free rate $r$ and the underlying asset has a  constant volatility~$\sigma$ and a constant dividend yield~$q$. 
...
$$
\mathrm{e}^{-qT'}S(0)\N(d_1) - \mathrm{e}^{-qT-r(T'-T)}S(0)\N(d_2)\;,
$$ {#eq-fstrikecall}
where $d_1$ and $d_2$ are defined in @eq-forwardstartcombined.}
...
\begin{multline}
-\mathrm{e}^{-rT}K\N(d_2) + \mathrm{e}^{-q T'}S(0)\M\!\left(d_1,d_1',\sqrt{T/T'}\right)
 
$$
$$- \mathrm{e}^{-rT'}K'\M\!\left(d_2,d_2',\sqrt{T/T'}\right)\;,
 \end{multline}
...
\begin{multline}
-\mathrm{e}^{-rT}K\N(-d_2) + \mathrm{e}^{-rT'}K'\M\!\left(-d_2,-d_2',\sqrt{T/T'}\right) 
$$ {#eq-callonaput}

$$- \mathrm{e}^{-q T'}S(0)\M\!\left(-d_1,-d_1',\sqrt{T/T'}\right) \;,
 \end{multline}
 where $d_1$ and  $d_2$ are defined in @eq-calloncalld1d2 and $d_1'$ and $d_2'$ are defined in @eq-callcallds.}
...
If we assume as usual that the asset price $S$ has a constant volatility, then, using formula @eq-exponential1 for a geometric Brownian motion and letting $B^*$ denote a Brownian motion under the risk-neutral measure, we have
\begin{align*}
...
$$Z(0)\N(d_1) + \mathrm{e}^{-rT}(D-K)\N(d_2)\; ,$$
where

$$ {#eq-americancallstarcombined}
...
$$[S(0)-\mathrm{e}^{-rT}D]\N(d_1')-\mathrm{e}^{-rT}K\N(d_2')\; ,$$
where $d_1'$ and $d_2'$ are defined in @eq-americancallcombined.
On the other hand, if 
$$D-K > -\mathrm{e}^{-r(T'-T)}K\;,$$
then the value of the call is
\begin{multline}
[S(0)-\mathrm{e}^{-rT}D]\N(d_1) + \mathrm{e}^{-rT}(D-K)\N(d_2)
$$
$$ +[S(0)-\mathrm{e}^{-rT}D]\M\!\left(-d_1,d_1',-\sqrt{T/T'}\right) 
$$
$$- \mathrm{e}^{-rT'}K\M\!\left(-d_2,d_2',-\sqrt{T/T'}\right)\;,
\end{multline}
where $d_1$ and $d_2$ are defined in @eq-americancallstarcombined and $d_1'$ and $d_2'$ are defined in @eq-americancallcombined.}
...
\begin{multline}
\mathrm{e}^{-q T_c}S(0)\M\!\left(d_1,d_{1c},\sqrt{T/T_c}\right) - \mathrm{e}^{-rT_c}K_c\M\!\left(d_2,d_{2c},\sqrt{T/T_c}\right)
$$ {#eq-chooser2}
 {#eq-chooser1}

$$ +\mathrm{e}^{-rT_p}K_p\M\!\left(-d_2,-d_{2p} , \sqrt{T/T_p}\right) 
$$
$$- \mathrm{e}^{-q T_p}S(0)\M\!\left(-d_1 ,-d_{1p} ,\sqrt{T/T_p}\right)\;,
\end{multline}
where
$$
\begin{array}{ll}
...
\begin{multline*}
\mathrm{e}^{-q_1T}S_1(0)\times\text{prob}^{V_1}(x=1) + \mathrm{e}^{-q_2T}S_2(0)\times\text{prob}^{V_2}(y=1) 
$$
$$- \mathrm{e}^{-rT}K\times\text{prob}^R(z=1)\; .
\end{multline*}
...
$$ {#eq-max3}
 ...
 The random variables on the left-hand sides of @eq-max12combined have standard normal distributions and their correlation is
...
\item The probability $\text{prob}^{V_2}(y=1)$ is exactly symmetric to $\text{prob}^{V_1}(x=1)$, with the roles of $S_1$ and $S_2$ interchanged. Note that the mirror image of $d_1$ defined in @eq-max4 is
...
\begin{multline}
\mathrm{e}^{-q_1T}S_1(0)\M\!\left(d_{11},d_1,\frac{\sigma_1-\rho\sigma_2}{\sigma}\right) + \mathrm{e}^{-q_2T}S_2(0)\M\!\left(d_{21},-d_2,\frac{\sigma_2-\rho\sigma_1}{\sigma}\right)
$$ {#eq-callonmaxformula}

$$+ \mathrm{e}^{-rT}K\M(-d_{12},-d_{22},\rho) - \mathrm{e}^{-rT}K\;,
\end{multline}
where $\sigma$ is defined in @eq-max2a and  $d_1$, $d_2$, $d_{11}$, $d_{12}$, $d_{21}$ and $d_{22}$ are defined in @eq-max34combined.}
...
The combination of a knock-out call and a knock-in call (or knock-out puts) with the same barrier and different strikes creates an option with a strike that is reset when the barrier is hit.  This is a hedge that adjusts automatically to the market.  An example is given in Probs.~\ref{ and~\ref{.
...
\text{prob}^{V}(y=1) = \left(\frac{L}{S(0)}\right)^{2\left(r-q+\frac{1}{2}\sigma^2\right)/\sigma^2}\N(d_1')\;,
$$ {#eq-casea4}

$$
\text{prob}^{R}(y=1) = \left(\frac{L}{S(0)}\right)^{2\left(r-q-\frac{1}{2}\sigma^2\right)/\sigma^2}\N(d_2')\;.
$$ {#eq-casea41}
...
$$ {#eq-caseb3}
Moreover, $\text{prob}^{V}(y=1)$ and $\text{prob}^{R}(y=1)$ are given by @eq-casea441combined but with $K$ replaced by $L$, which means that
$$
...
\begin{multline}
\mathrm{e}^{-qT}S(0)\left[\N(d_1)-\left(\frac{L}{S(0)}\right)^{2\left(r-q+\frac{1}{2}\sigma^2\right)/\sigma^2}\N(d_1')\right]
$$ {#eq-downout100}

$$ - \mathrm{e}^{-rT}K\left[\N(d_2) - \left(\frac{L}{S(0)}\right)^{2\left(r-q-\frac{1}{2}\sigma^2\right)/\sigma^2}\N(d_2')\right]\;,
\end{multline}
where 
\begin{enumerate}
\renewcommand{\labelenumi}{(\arabic{enumi})}
\item if $K>L$, $d_1$, $d_2$ , $d_1'$ and $d_2'$ are defined in @eq-casea35combined,
\item if $K\leq L$, $d_1$, $d_2$, $d_1'$ and $d_2'$ are defined in @eq-caseb35combined.
\end{enumerate}
}
...
### Calculations}
The value at date 0 of the piece $S(T)$ is simply $\mathrm{e}^{-qT}S(0)$.  Using the result in Appendix~\ref{a_minimum} on the distribution of $z$, it can be shown (see, e.g., Musiela and Rutkowski \cite{MR} for the details) that the value at date 0 of receiving 
$$ {#eq-s_lookbacks}
\min(z, S_{\text{min}})$$
at date $T$ is
\begin{multline*}
\mathrm{e}^{-rT}S_{\text{min}}\N(d_2) -\frac{\sigma^2}{2(r-q)}\left(\frac{S_{\text{min}}}{S(0)}\right)^{2(r-q)/\sigma^2}\mathrm{e}^{-rT}S(0)\N(d_2') 
$$
$$
+\left(1+ \frac{\sigma^2}{2(r-q)}\right)\mathrm{e}^{-qT}S(0)\N(-d_1)\;.
\end{multline*}
where
...
\begin{multline}
\mathrm{e}^{-qT}S(0)\N(d_1)-\mathrm{e}^{-rT}S_{\text{min}}\N(d_2) 
$$ {#eq-fslc100}

$$+\frac{\sigma^2}{2(r-q)}\left(\frac{S_{\text{min}}}{S(0)}\right)^{2(r-q)/\sigma^2}\mathrm{e}^{-rT}S(0)\N(d_2') 
$$
$$
-\frac{\sigma^2}{2(r-q)}\mathrm{e}^{-qT}S(0)\N(-d_1)\;,
\end{multline}
where $d_1$, $d_2$, and $d_2'$ are defined in @eq-fslc100combined.}

\section{Basket and Spread Options}
...
\begin{enumerate} 
\item At date 0, purchase $\mathrm{e}^{-q_1T}$ units of the asset with price $S_1$ and reinvest dividends, leading to a value of $S_1(T)$ at date $T$, 
\item At date 0, purchase $\mathrm{e}^{-q_2T}$ units of the asset with price $S_2$ and invest $\mathrm{e}^{-rT}K$ in the risk-free asset.  Reinvesting dividends and accumulating interest means that we will have $S_2(T)+K$ dollars at date $T$.
\end{enumerate}
...
We can also value a discretely-sampled geometric-average-price call by the same arguments.  Consider dates $0<t_0< t_1 < \cdots t_N=T$, where $t_i-t_{i-1}=\mathrm{d}elta t$ for each $i$ and suppose the price is to be sampled  at the dates $t_1,\ldots,t_N$.  Now let $V(t)$ denote the value at date $t$ of the contract that pays
$$
\exp\left(\frac{1}{N} \sum_{i=1}^N \log S(t_i)\right) = \left(\prod_{i=1}^N S(t_i)\right)^{1/N}
$$ {#eq-geometricaveragev0}
at date $T$.  The call option will pay $\max(0,V(T)-K)$ at date $T$.  Let $k$ denote the integer such that $t_{N-k-1} \leq t < t_{N-k}$.  This means that we have already observed the prices $S(t_1), \ldots, S(t_{N-k-1})$ and we have yet to observe the $k+1$ prices $S(t_{N-k}), \ldots ,S(t_N)$.  Define $\mathrm{var}epsilon = (t_{N\!-k}-t)/\mathrm{d}elta t$, which is fraction of the interval $\mathrm{d}elta t$ that must pass before we reach the next sampling date $t_{N\!-k}$.  We will show at the end of this section that 
\begin{multline}
V(t) = \mathrm{e}^{-r(T-t)}S(t)^{\frac{k+1}{N}}\prod_{i=1}^{N-k-1}S(t_i)^{\frac{1}{N}}
$$ {#eq-geometricaveragev2}

$$
\times\,\exp\left(\left[ \frac{(k+1)\mathrm{var}epsilon\nu}{N}\!+\! \frac{k(k+1)\nu}{2N}\! +\! \frac{(k+1)^2\sigma^2\mathrm{var}epsilon}{2N^2}\!+\!\frac{k(k+1)(2k+1)\sigma^2}{12N^2} \right]\mathrm{d}elta t\right)\;,
\end{multline}
where $\nu = r-q-\sigma^2/2$

Again, two points are noteworthy.  Assume the call was written at date 0 and the first observation date $t_1$ is $\mathrm{d}elta t$ years away.  Then, we have $k+1=N$ and $\mathrm{var}epsilon=1$ so
$$
V(0) = \mathrm{e}^{-rT}S(0)\exp\left( \frac{(N+1)\nu\mathrm{d}elta t}{2} + \frac{(N+1)(2N+1)\sigma^2\mathrm{d}elta t}{12N} \right)\;.
$$ {#eq-asianV02}
...
Let  $\mathrm{d}elta_0B = B(t_{N-k})-B(t)$ and $\mathrm{d}elta_iB = B(t_{N\!-k+i})-B(t_{N\!-k+i-1})$ for $i \geq 1$.  We can write the sum of logarithms inside the expectation above as
\begin{multline*}
\sum_{i=0}^{k}\big\{[\log S(t) + (t_{N-k+i}-t)\nu + \sigma [B(t_{N-k+i})-B(t)]\big\}
$$
$$
=(k+1)\log S(t) + \sum_{i=0}^{k} (\mathrm{var}epsilon + i)\nu\mathrm{d}elta t + \sigma\sum_{i=0}^{k} [\mathrm{d}elta_0B + \mathrm{d}elta_1B + \cdots + \mathrm{d}elta_iB] 
$$
$$
=(k+1)\log S(t) + (k+1)\mathrm{var}epsilon\nu\mathrm{d}elta t + \frac{k(k+1)}{2}\nu\mathrm{d}elta t + \sigma\sum_{i=0}^{k} (k+1-i)\mathrm{d}elta_iB\;,
\end{multline*}
where to obtain the last equality we used the fact that $\sum_{i=0}^k i = k(k+1)/2$.
The random variables $\mathrm{d}elta_iB$ are normally distributed with mean zero and variance $\mathrm{d}elta t$ (the variance is $\mathrm{var}epsilon \mathrm{d}elta t$ for $i=0$).  Thus, the sum of logarithms is
 a normally distributed random variable with mean
$$(k+1)\log S(t) + (k+1)\mathrm{var}epsilon\nu\mathrm{d}elta t + \frac{k(k+1)}{2}\nu\mathrm{d}elta t$$
and variance
$$(k+1)^2\sigma^2\mathrm{var}epsilon\mathrm{d}elta t + \sigma^2\sum_{i=1}^{k} (k+1-i)^2\mathrm{d}elta t = (k+1)^2\sigma^2\mathrm{var}epsilon\mathrm{d}elta t +\frac{k(k+1)(2k+1)\sigma^2}{6}\; ,$$
...
$$S(t)^{\frac{k+1}{N}}\exp\left(\left[ \frac{(k+1)\mathrm{var}epsilon\nu}{N}+ \frac{k(k+1)\nu}{2N} + \frac{(k+1)^2\sigma^2\mathrm{var}epsilon}{2N^2}+\frac{k(k+1)(2k+1)\sigma^2}{12N^2} \right]\mathrm{d}elta t\right)\; ,$$
which implies @eq-geometricaveragev2.
\end{petit}
\section{Calculations in VBA}s_exotics_matlab}
...
### Discretely-Sampled Geometric-Average-Price Calls}
...
\end{prob}\begin{prob}  Suppose you must purchase 100 units of an asset at the end of a year.  Create an Excel worksheet simulating the asset price and comparing the quality of the following hedges (assuming 100 contracts of each):
\begin{enumerate}
...
\end{prob}\begin{prob} 
Modify Prob.~\ref{e_standardknockout} by including a third hedge: a combination of a down-and-out call as in part (b) of Prob.~\ref{e_standardknockout} and a down-and-in call with knockout barrier and strike 10\% below the current price of the asset.  Note that this combination forms a call option with strike that is reset when the underlying asset price hits a barrier.e_standardknockout3}
\end{prob}\begin{prob}
Modify Prob.~\ref{e_standardknockout2} by including the hedge in Prob.~\ref{e_standardknockout3}.  Value the down-and-in call using the function \verb!Down_And_Out_Call! and the fact that a down-and-out and down-and-in with the same strikes and barriers form a standard option. e_standardknockout4}
\end{prob}\begin{prob}  Each week you purchase 100 units of an asset, and you want to hedge your total quarterly (13-week) cost.  Create an Excel worksheet simulating the asset price and comparing the quality of the following hedges: 
\begin{enumerate}
\renewcommand{\labelenumi}{(\alph{enumi})}
...
\end{prob}\begin{prob} Derive formula @eq-callonaput for the value of a call on a put.
\end{prob}\begin{prob} Complete the derivation of formula @eq-chooser2 for the value of a chooser option.
...
$$ {#eq-e_averagehedge2}
 {#eq-e_averagehedge}
 {#eq-e_standardknockout4}
 {#eq-e_standardknockout3}
 {#eq-e_standardknockout2}
 {#eq-e_standardknockout}
 {#eq-s_exotics_matlab}

$$
= \mathrm{e}^{-q_2T}S_2(0) + \text{Value of option to exchange asset 2 for asset 1} 
$$
