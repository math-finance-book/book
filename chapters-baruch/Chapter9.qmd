...
To value a path-dependent option by Monte Carlo, we need to simulate an approximate path of the stock price.  We do this by considering time periods of length $\mathrm{delta t = T/N$ for some integer $N$.  Under the risk-neutral measure, the logarithm of the stock price changes over such a time period by
$$
\mathrm{delta \log S = \nu\,\mathrm{delta t + \sigma\sqrt{\mathrm{delta t}\,z\;,
$$ {#eq-pathdependent}
where $\nu = r-q-\sigma^2/2$ and $z$ is a standard normal.  Given that there are~$N$ time periods of length $\mathrm{d}elta t$, we need to generate $N$ standard normals to generate a stock price path.  If we generate $M$ paths to obtain a sample of $M$ option values, then we will need to generate $MN$ standard normals.  


Consider for example a floating-strike lookback call.  \index{lookback option} The formula for this option given in Sect.~\ref{s_lookbacks} assumes the minimum stock price is computed over the entire path of the stock price, i.e., with continuous sampling of the stock price.  In practice, the minimum will be computed by recording the price at a discrete number of dates.  We can value the discretely sampled lookback using Monte-Carlo by choosing $\mathrm{d}elta t$ to be the interval of time (e.g., a day or week) at which the price is recorded.  For example, if the contract calls for weekly observation, we will attain maximum precision by setting $N$ to be the number of weeks before the option matures.  


Asian and barrier options are also subject to discrete rather than continuous sampling and can be valued by Monte-Carlo in the same way as lookbacks.  We will discuss Asian options in Sect.~\ref{.
...
\begin{align*}
\log u_i=\sqrt{\sigma_i^2\mathrm{d}elta t + \nu_i^2(\mathrm{d}elta t)^2}\; ,
$$ {#eq-s_curse}

$$
p_i = \frac{1}{2}+\frac{\nu_i\mathrm{d}elta t}{2\log u_i}\;.
\end{align*}
...
p_{uu}+p_{ud} = \frac{1}{2}+\frac{\nu_1\mathrm{d}elta t}{2\log u_1}\;,
$$ {#eq-prob1}

$$
p_{uu}+p_{du} = \frac{1}{2}+\frac{\nu_2\mathrm{d}elta t}{2\log u_2}\;.
$$ {#eq-prob2}


In the continuous time model, over a discrete time period $\mathrm{d}elta t$, the covariance of $\mathrm{d}elta \log S_1$ and $\mathrm{d}elta \log S_2$ is $\rho\sigma_1\sigma_2\mathrm{d}elta t$.  In the binomial model, with $d_i=1/u_i$, we have
$$E \big[\mathrm{d}elta \log S_1 \times \mathrm{d}elta \log S_2\big] = (p_{uu}-p_{ud}-p_{du}+p_{dd})\log u_1\log u_2\; .$$  Given that $E[\mathrm{d}elta\log S_i] =\nu_i\mathrm{d}elta t$, this implies a covariance of
$$(p_{uu}-p_{ud}-p_{du}+p_{dd})\log u_1\log u_2 - \nu_1\nu_2(\mathrm{d}elta t)^2\; .$$
Matching the covariance in the binomial model to the covariance in the continuous-time model therefore implies
$$
p_{uu}-p_{ud}-p_{du}+p_{dd} =\frac{\rho\sigma_1\sigma_2\mathrm{d}elta t + \nu_1\nu_2(\mathrm{d}elta t)^2}{\log u_1\log u_2}\;.
$$ {#eq-prob3}


We can solve the system @eq-prob13combined, together with the condition that the probabilities sum to one, to obtain the probabilities $p_{uu}$, $p_{ud}$, $p_{du}$, and $p_{dd}$.  This solution and a VBA function for valuing an American spread call option are given in Sect.~\ref{.  This function operates much like the binomial valuation of American options described in @sec-c_introcomputation.  The primary difference is that the value of the option at maturity depends on both stock prices, so we have to consider each possible combination of stock prices.  In an $N$--period model, there are $N+1$ nodes at the final date for each of the two stocks, and hence $(N+1)^2$ possible combinations of nodes.  In fact, at each date $n$ ($n=0,\ldots,N$) there are $(n+1)^2$ combinations of nodes to be considered.  
...
$$ {#eq-montecarlomultiple}
\mathrm{d}elta B_1 = \sqrt{\mathrm{d}elta t}\,Z_1\;, \qquad \text{and} \qquad \mathrm{d}elta B_2 = \sqrt{\mathrm{d}elta t}\,Z\; ,$$
...
\begin{align*}
\mathrm{d}elta \log S_1 = \nu_1\mathrm{d}elta t + \sigma_1\sqrt{\mathrm{d}elta t}Z_1 \; ,
$$
$$
\mathrm{d}elta \log S_2 = \nu_2\mathrm{d}elta t + \sigma_2\rho\sqrt{\mathrm{d}elta t}Z_1 + \sigma_2\sqrt{1-\rho^2}\sqrt{\mathrm{d}elta t}Z_2\;,
\end{align*}
...
\mathrm{d}elta \log S_1 = \nu_1\mathrm{d}elta t + a_{11}\sqrt{\mathrm{d}elta t}Z_1 + a_{12}\sqrt{\mathrm{d}elta t}Z_2\;,
$$ {#eq-mc_twoa}

$$
\mathrm{d}elta \log S_2 = \nu_2\mathrm{d}elta t + a_{21}\sqrt{\mathrm{d}elta t}Z_1 + a_{22}\sqrt{\mathrm{d}elta t}Z_2\;,
$$ {#eq-mc_twob}
...
\end{array}$$
These are not the only possible choices for the constants $a_{ij}$.  Given that $Z_1$ and $Z_2$ are independent standard normals, the conditions the $a_{ij}$ must satisfy in order to match the variances $\sigma_i^2\mathrm{d}elta t$ and correlation $\rho$ of the changes in the logarithms are
...
$$\mathrm{d}elta \,\text{LogS(1)} = \nu\,\mathrm{d}elta t + \sigma \mathrm{d}elta B$$
and
$$\mathrm{d}elta \,\text{LogS(2)} = \nu\,\mathrm{d}elta t - \sigma \mathrm{d}elta B\; .$$
...
\end{center}\normalsize
for various values of $N$.  Allow the user to input $S$, $K$, $r$, $\sigma$, $q$, and $T$.  To assess the quality of the estimates, provide also the estimate given by the function \verb!American_Put_Binomial! with a large value of $N$ (say $N = 1000$).  \end{prob}
...
\begin{prob}  Create a VBA function \verb!European_Basket_Call_Binomial! to value a basket option on two assets. \end{prob}
