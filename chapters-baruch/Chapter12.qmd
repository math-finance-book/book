# Itroduction to Fixed Income Derivatives {#sec-c_fixedincomederivatives

In this chapter, we wil introduce some fundamental fixed-income derivatives (caps, floors and swaptions) and explain the market model approach to valuation.  We will also explain the relation between caps, floors and swaptions on the one hand and discount and coupon bond options on the other.  This leads to other approaches for valuing caps, floors and swaptions, which will be developed in the following chapters.  

## Caps and Floors

Caps \index{cap and floors \index{floor have a structure very similar to that of swaps, as described in Sect.~\ref{s_swaps.  At each reset date $t_i$, the simple interest rate $\mathcal{R_i$ for period $\mathrm{delta t$ is observed.  This rate determines a payment at the following date $t_{i+1$.  As discussed in the preceding chapter, the cash flow to the payer in a swap is $(\mathcal{R_i - \bar{\mathcal{R}})\,\mathrm{d}elta t$ and the cash flow to the receiver is $(\bar{\mathcal{R}}-\mathcal{R}_i)\,\mathrm{d}elta t$, for each \$1 of notional principal.  A swap is really a series of forward contracts, in which both parties have obligations.  On the other hand, caps and floors are series of options.  A premium is paid up-front by the buyer of a cap or floor to the seller and all future cash flows are paid by the seller to the buyer.  The owner of a cap with cap rate $\bar{\mathcal{R}}$ receives $\max(0,\mathcal{R}_i-\bar{\mathcal{R}})\mathrm{d}elta t$ at date $t_{i+1}$ for each \$1 of notional principal, and the owner of a floor receives $\max(0,\bar{\mathcal{R}}-\mathcal{R}_i)\mathrm{d}elta t$ at date $t_{i+1}$ for each \$1 of notional principal.  
...
A caplet can be viewed as a call option on the spot rate with strike equal to the fixed rate.  Thus, it is a bet on higher interest rates.  Because interest rates and bond prices are inversely related, it can also be viewed as a bet on lower bond prices.  In this regard, it is similar to a put option on bond prices.  In fact, we will see in  Sect.~\ref{ that a caplet is exactly equivalent to a put option on a discount bond.  Likewise, a floorlet can be viewed either as a put option on the spot rate or a call option on a discount bond.

## Forward Rates}

Suppose we wish to borrow money at date $u$ for a period of $\mathrm{d}elta t$ years, and we want to lock in the rate on the loan at date $t<u$.  To do this, we can buy the discount bond maturing at $u$ and finance the purchase by shorting $P(t,u)/P(t,u+\mathrm{d}elta t)$ units of the bond maturing at $u+\mathrm{d}elta t$.  This generates a cash flow of \$1 at date $u$ and $-P(t,u)/P(t,u+\mathrm{d}elta t)$ dollars at date $u+\mathrm{d}elta t$.  This implies a simple interest rate of $\mathcal{R}$ defined as 
$$ {#eq-s_forwardrates}

\frac{P(t,u)}{P(t,u+\mathrm{d}elta t)} = 1+\mathcal{R}\,\mathrm{d}elta t\;.
$$ {#eq-forward1}
...
To obtain the spot rate $\mathcal{R}_i$ at date $t_{i+1}$, one needs \$1 to invest at date~$t_i$.  This can be arranged at date $t<t_i$ by buying one unit of the bond maturing at $t_i$.  Investing the dollar paid by the bond at the spot rate at $t_i$ will generate $1+\mathcal{R}_i\,\mathrm{d}elta t$ dollars at date $t_{i+1}$.  The extra dollar can be eliminated by being short one unit of the bond maturing at $t_{i+1}$, leaving $\mathcal{R}_i\,\mathrm{d}elta t$ dollars.  Thus, the portfolio that pays the spot rate multiplied by the period length $\mathrm{d}elta t$ consists of being long one unit of the bond maturing at $t_i$ and short one unit of the bond maturing at $t_{i+1}$.  
This implies that the value at date $t<t_i$ of receiving $\mathcal{R}_i\,\mathrm{d}elta t$ dollars at date $t_{i+1}$ is $P(t,t_i)-P(t,t_{i+1})$.  

At date $t_i$ the spot rate $\mathcal{R}_i$ becomes known.  Between $t_i$ and $t_{i+1}$, the value of receiving $\mathcal{R}_i\,\mathrm{d}elta t$ dollars at date $t_{i+1}$ is the present value of this known cash flow, which is $\mathcal{R}_i\,\mathrm{d}elta t\,P(t, t_{i+1})$.  To summarize, the value of receiving $\mathcal{R}_i\,\mathrm{d}elta t$ dollars at date $t_{i+1}$ is
$$ {#eq-s_portfoliosspotrates}

S_i(t) = \begin{cases} P(t,t_i)-P(t,t_{i+1})  \text{if } t < t_{i}\; ,
$$
$$
\mathcal{R}_i\,\mathrm{d}elta t\, P(t,t_{i+1})  \text{if }  t_{i} \leq t \leq t_{i+1}\;. \end{cases}
$$ {#eq-S_i}
...
$$
$$
\mathcal{R}_i\,\mathrm{d}elta t  \text{if }  t_{i} \leq t \leq t_{i+1}\;. \end{cases}
$$
$$
= \begin{cases}\mathcal{R}_i(t)\,\mathrm{d}elta t  \text{if } t < t_{i}\; ,
$$
$$
\mathcal{R}_i\,\mathrm{d}elta t  \text{if }  t_{i} \leq t \leq t_{i+1}\;, \end{cases}
$$ {#eq-F_i}
...
We can apply Black's formula to the forward contract with price $F_i$ described in the previous section.  We view the caplet \index{caplet} as a call option maturing at $t_i$ on this forward contract that matures at $t_{i+1}$.  As explained in Chapter~\ref{c_forwardexchange}, the value at the maturity date $T$ of a call option with strike $K$ on a forward contract with price $F$ maturing at $T' \geq T$ is $\max(0,F(T)-K)P(T,T')$.  Therefore, the value at maturity of a call option maturing at~$t_i$ with strike $\bar{\mathcal{R}}\,\mathrm{d}elta t$ on the forward contract with price $F_i$ is
$$ {#eq-s_valuingcaps}
\max(0,F_i(t_i)-\bar{\mathcal{R}}\,\mathrm{d}elta t)P(t_i,t_{i+1})\; .$$
Since $F_i(t_i)=\mathcal{R}_i\,\mathrm{d}elta t$, this equals 
$$
\max(0,\mathcal{R}_i-\bar{\mathcal{R}})\,\mathrm{d}elta t\,P(t_i,t_{i+1})\;.
$$ {#eq-capletvalue}
This is also the value of the caplet at date $t_i$.

It follows that the value of the caplet at any date $t\leq t_i$ is the value of the call option on the forward contract.  To apply Black's formula, \index{Black's formula}we need the forward price to have a constant (or at least non-randomly varying) volatility.  As noted earlier, at dates $t\leq t_i$, $F_i(t)=\mathcal{R}_i(t)\,\mathrm{d}elta t$, where $\mathcal{R}_i(t)$ is the forward rate, so the volatility of the forward price is the volatility of the forward rate. Black's formula yields:
...
$$
P(0,t_{i+1})\mathcal{R}_i(0)\,\mathrm{d}elta t\,N(d_1) - P(0,t_{i+1})\bar{\mathcal{R}}\,\mathrm{d}elta t\,N(d_2)\;,
$$ {#eq-blackcaplet100}
and the value at date $0<t_i$ of a floorlet \index{floorlet} with reset date $t_i$ and payment date $t_{i+1}$ is
$$
P(0,t_{i+1})\bar{\mathcal{R}}\,\mathrm{d}elta t\,N(-d_2)- P(0,t_{i+1})\mathcal{R}_i(0)\,\mathrm{d}elta t\,N(-d_1)\;,
$$ {#eq-blackfloorlet100}
where
$$
...
The put-call parity relationship for caplets and floorlets can be seen as follows. \index{put-call parity} If we add $\bar{\mathcal{R}}\mathrm{d}elta t$ to the caplet payment, we obtain
$$\bar{\mathcal{R}}\,\mathrm{d}elta t +  \max(0,\mathcal{R}_i-\bar{\mathcal{R}})\,\mathrm{d}elta t = \max( \bar{\mathcal{R}}, \mathcal{R}_i)\,\mathrm{d}elta t\; .$$
On the other hand, if we add $\mathcal{R}_i\,\mathrm{d}elta t$ to the floorlet payment, we obtain the same thing:
$$\mathcal{R}_i\,\mathrm{d}elta t + \max(0,\bar{\mathcal{R}}-\mathcal{R}_i)\,\mathrm{d}elta t = \max( \mathcal{R}_i,\bar{\mathcal{R}} )\,\mathrm{d}elta t\; .$$
Hence, the value of a caplet plus the value of receiving $\bar{\mathcal{R}}\,\mathrm{d}elta t$ at date $t_{i+1}$ must equal the value of a floorlet plus the value of receiving $\mathcal{R}_i\,\mathrm{d}elta t$ at date $t_{i+1}$.  The value at any date $t\leq t_i$ of receiving $\bar{\mathcal{R}}\,\mathrm{d}elta t$ at date $t_{i+1}$ is the value of $\bar{\mathcal{R}}\,\mathrm{d}elta t$ discount bonds maturing at date $t_{i+1}$, which we are denoting by
$\bar{\mathcal{R}}\,\mathrm{d}elta t P(t,t_{i+1})$.  The value at at any date $t\leq t_i$ of obtaining $\mathcal{R}_i\,\mathrm{d}elta t$ dollars at date $t_{i+1}$ is  
$S_i(t) =P(t,t_i)-P(t,t_{i+1})$.  We conclude that
$$\text{ Value of Caplet} \;+ \;\bar{\mathcal{R}}\,\mathrm{d}elta t P(t,t_{i+1}) = \text{Value of Floorlet} \;+\; P(t,t_i)-P(t,t_{i+1})\; .$$
...
Consider a payer swaption with maturity date $T$ and swap rate $\bar{\mathcal{R}}$, where the underlying swap has payment dates $t_1,\ldots,t_N$ with the first reset date of the swap being $t_0 = t_1-\mathrm{d}elta t \geq T$.  We assume the notional principal of the swap is \$1.  Expression @eq-swapvalue100 in Sect.~\ref{s_swaps} gives the value at date $T$ to the payer in the swap as
$$ {#eq-s_swaptions}
S(T)-Z(T)\; ,$$
where we define
$$
...
$$
$$
Z(t) =  \bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)\;.
$$ {#eq-fixedrateside}
...
$$P(t,t_0)- P(t,t_N) = \mathcal{R}(t)\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)\; ,$$
which means that the swap would have zero value if initiated at date $t$ at the rate $\mathcal{R}(t)$---cf.\ @eq-swaprate1.  Thus,
$$
\frac{S(t)}{Z(t)} = \frac{P(t,t_0)- P(t,t_N)}{\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)} = \frac{\mathcal{R}(t)\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)}{\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)} = \frac{\mathcal{R}(t)}{\bar{\mathcal{R}}}\;,
$$
where $\mathcal{R}(t)$ is the forward swap rate.  
Margrabe's formula implies:\footnote{To improve the clarity of the typesetting, we have written  $\log(S)-\log(Z)$ in   @eq-margrabepayerswaption100d1 instead of our customary $\log(S/Z)$.}
...
$$
\big[P(0,t_0)-P(0,t_N)\big]N(d_1) - \left[\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(0,t_i)\right]N(d_2)\;,
$$ {#eq-margrabepayerswaption100}
and the date--0 value of a European receiver swaption is
$$
\left[\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(0,t_i)\right]N(-d_2) - \big[P(0,t_0)-P(0,t_N)\big]N(-d_1)\;,
$$ {#eq-margrabereceiverswaption100}
where
$$
d_1 = \frac{\log\big(P(0,t_0)-P(0,t_N)\big) - \log\left(\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(0,t_i)\right) + \frac{1}{2}\sigma^2T}{\sigma\sqrt{T}}\;,
...
\begin{multline*} \bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(T,t_i) + \max\left(0, \,P(T,t_0)- P(T,t_N)-  \bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(T,t_i)\right) 
$$
$$= P(T,t_0)- P(T,t_N) + \max\left(0,\,  \bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(T,t_i)-P(T,t_0)+ P(T,t_N)\right).
\end{multline*}
Therefore, at any date $t \leq T$,
\begin{multline}
\bar{\mathcal{R}}\,\mathrm{d}elta t \sum_{i=1}^N P(t,t_i)\; + \;\text{Value of Payer Swaption}
$$ {#eq-swaptionparity}
...
$$ {#eq-s_consistency}
\mathcal{R}_i(t)\,\mathrm{d}elta t = \frac{P(t,t_i)-P(t,t_{i+1})}{P(t,t_{i+1})}\; ,$$
and
$$\mathcal{R}(t)\,\mathrm{d}elta t = \frac{P(t,t_0)-P(t,t_N)}{\sum_{i=0}^{N-1} P(t,t_{i+1})}\; .$$
The numerator in the last equation is the sum (over $i=0,\dots,N\!-\!1$) of the numerators in the previous equation; hence, it is the sum of the $\mathcal{R}_i(t)\,\mathrm{d}elta t\,P(t,t_{i+1})$.  This implies that
...
Previously, we considered a caplet \index{caplet} as a call option on the forward rate and thus a bet on higher interest rates.  This is equivalent to a bet on lower bond prices, and we will now show that a caplet with payment date $t_{i+1}$ is equivalent to $1+\bar{\mathcal{R}}\,\mathrm{d}elta t$ put options on the $t_{i+1}$--maturity discount bond.  The put options mature at the reset date $t_i$ of the caplet and have strike equal to $1/(1+ \bar{\mathcal{R}}\,\mathrm{d}elta t)$.  To see this equivalence, note that the value of $1+\bar{\mathcal{R}}\,\mathrm{d}elta t$ such options at their maturity date~$t_i$~is
\begin{multline*}
\big[1+\bar{\mathcal{R}}\,\mathrm{d}elta t\big] \max\left(0,\frac{1}{1+ \bar{\mathcal{R}}\,\mathrm{d}elta t}-P(t_i,t_{i+1})\right) 
$$ {#eq-s_caps2}

$$ \begin{array}{l}
= \max\big(0,\,1-\big[1+\bar{\mathcal{R}}\,\mathrm{d}elta t\big]P(t_i,t_{i+1})\big)
$$
$$
=P(t_i,t_{i+1})\max\left(0,\frac{1}{P(t_i,t_{i+1})}-1-\bar{\mathcal{R}}\,\mathrm{d}elta t\right)\;. \end{array}
\end{multline*}
Given that $1/P(t_i,t_{i+1}) = 1+\mathcal{R}_i\,\mathrm{d}elta t$, this equals
$$P(t_i,t_{i+1}) \max(0,\mathcal{R}_i-\bar{\mathcal{R}})\,\mathrm{d}elta t\; .$$
This is the value  at date $t_i$ of the caplet with payment date $t_{i+1}$ shown in expression @eq-capletvalue.  It follows that the  caplet and the $1+\bar{\mathcal{R}}\,\mathrm{d}elta t$ put options must have the same value at any date prior to~$t_i$.

Similarly, a floorlet \index{floorlet} with payment date $t_{i+1}$ is equivalent to $1+\bar{\mathcal{R}}\,\mathrm{d}elta t$ call options on the $t_{i+1}$--maturity discount bond, with the call options maturing at date $t_i$ and having strike equal to $1/(1+ \bar{\mathcal{R}}\,\mathrm{d}elta t)$.  
...
$$ {#eq-s_swaptionscouponbondoptions}
P(T,t_0)-P(T,t_N) - \bar{\mathcal{R}}\,\mathrm{d}elta t\sum_{i=1}^N P(T,t_i)\; .$$
...
$Z(T) = \bar{\mathcal{R}}\,\mathrm{d}elta t\sum_{i=1}^N P(T,t_i)$, the value of the fixed-rate leg.  We can also separate it differently---the first part being $P(T,t_0)$, which is the value at the swaption maturity of the discount bond maturing at $t_0$, and the second part being    $P(T,t_N)+\bar{\mathcal{R}}\,\mathrm{d}elta t\sum_{i=1}^N P(T,t_i)$, which is the value of a fixed-rate bond including the face value at maturity.  Thus, a payer swaption is equivalent to an option to exchange a fixed-rate coupon bond for a discount bond.  A receiver  swaption is an option to engage in the reverse exchange. \index{coupon bond option}
...
### Valuing Caps with Black's Formula}
...
$$ {#eq-s_fixedincomederivatives_matlab}

F_i(0) = \mathcal{R}_i(0)\,\mathrm{d}elta t = \frac{P(0,t_i)}{P(0,t_{i+1})} - 1\;,
$$ {#eq-capletforwardprice}
as given in @eq-forward2 and @eq-F_i.  The exercise price of each caplet is $K=\bar{\mathcal{R}}\,\mathrm{d}elta t$.  We also need to input the discounting factor $P(0,t_{i+1})$.
...
### Valuing Swaptions with Margrabe's Formula}
...
\end{prob}\begin{prob}  The following exercise is motivated by an example presented in one of my classes by David Eichhorn of NISA Investment Advisors.
