...
## Black-Scholes Valuation of Currency Options

Calls and puts on foreign currency can be valued by the Black-Scholes formulas with inputs 

- initial asset price $=$ exchange rate $X(0)$
- risk-free rate $=$ domestic rate $r$
- volatility $=$ volatility $\sigma_x$ of exchange rate, and
- dividend yield $=$ foreign rate $r_f$.
:::
...
A call option with domestic strike price $K$ on a foreign asset with price $S$ pays
$$\max(X(T)S(T)-K,0)$$
...
## Proof of @eq-growthreciprocalX

The assumption that $S$ and $X$ have constant volatilities and  correlation means that  
\begin{align*}
