...
Therefore, the true delta---the expectation @eq-montecarlodelta4---equals^[By changing numeraires, we can show that @eq-montecarlodelta5 equals $\mathrm{e}^{-qT}\\mathrm{E}^V[x] = \mathrm{e}^{-qT}\mathrm{N}(d_1)$, as we know from @sec-c:blackscholes is the delta of a European call in the Black-Scholes model (here, as in @sec-c:blackscholes, $V_t=\mathrm{e}^{qt}S_t$ denotes the value of the dividend-reinvested portfolio created from the stock).] 
$$
