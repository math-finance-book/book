...
The **intrinsic value** of an option is the maximum that anyone would be willing to pay for it if it had to be exercised immediately or would expire.  For a call option, this is the difference between the underlying asset price and the strike of the call, if that is positive, and is zero otherwise.  Letting $S$ denote the price of the underlying asset and $K$ the strike price, the intrinsic value of a call is
$$\begin{cases} S-K & \text{if $S>K$}\\
0 $\text{if $S \le K}\end{cases}\,.$$
This can be written more compactly as $\max(S-K, 0)$.  For example, if the underlying asset is trading at \$50, and the strike is \$40, then the intrinsic value is \$10.  This reflects the fact that we could pay \$10 for the option, pay the strike of \$40 to exercise, then sell the underlying asset in the market for \$50 and break even.  Of course, we would never bother to do this but in principle if the option could be purchased at any lower price, then we would want to make these trades.  Thus, the maximum price anyone would pay for the option in this circumstance is \$10.  Symmetrically, the intrinsic value of a put option is
$$\max(K-S, 0) = \begin{cases}K-S & \text{if  $K>S$}\\
0 $\text{if $K \le S$}\end{cases}\,.$$

If the intrinsic value of an option is positive, then the option is said to be **in the money**.  Thus, if $S>K$, then a call option is in the money.  If $S=K$ (or, in practice, approximately equal), then a call is said to be **at the money**.  If $S<K$, then a call is said to be **out of the money**.  The same terminology applies to puts, but with the roles of $S$ and $K$ reversed.

American option values are always at least as large as their intrinsic values.  This is because exercising immediately is a possibility with an American option.  As mentioned above, we would want to buy call (put) options trading below their intrinsic values, exercise, and simultaneously sell (buy) the underlying asset to make a riskless profit.  This is called an arbitrage opportunity, and it is safe to assume we will not find such arbitrage opportunities in the market.
...
Understanding option payoffs is fundamental to options trading. A **payoff diagram** shows the intrinsic value of an option position as a function of the price of the underlying asset.  This is the value the option position would have at expiration.  A **profit diagram** shows the profit from an option position as a function of the price of the underlying asset.  The difference between the profit and the payoff is the premium paid (for long positions) or received (for short positions).  Here are the payoff and profit diagrams for each of the four basic option positions:

#### Long Call

::: {#fig-long-call fig-height=400}
```{python}
#| label: fig-long-call
...
::: {#fig-protective-put fig-height=400}
```{python}
...
A common option strategy is to sell an out-of-the-money call option on a stock that one owns.  This generates premium income, at the expense of reducing the maximum gain on the stock (it is capped at the call strike).  Whether this turns out to be profitable depends on how high the underlying goes prior to the option maturity - if it goes up sufficiently far, then the trader will regret having sold the call; if it goes up less, or doesn't go up at all, then the trader will be happy to have captured the call premium.  The following is the payoff diagram of a portfolio consisting of a long position in the underlying asset and a short call.  

::: {#fig-covered-call fig-height=400}
```{python}
#| label: fig-covered-call
#| fig-cap: Selling covered calls is profitable if the underlying doesn't go up too much before the option expires.
#| out-width: "100%"

# Collar: Long stock + Long put + Short call
call_payoff = -np.maximum(S - K_call, 0)  # Short call payoff at maturity
collar_payoff = stock_payoff + call_payoff
...
::: {#fig-bull-spread fig-height=400}
```{python}
...
A bear put spread involves buying a put option and selling another put option with a lower strike price. This strategy profits from moderate downward price movement.  The motivation for the spread is to buy a put either for protection (i.e., hedging/insurance) or as a bet on the downside of an asset.  The put with a lower strike is sold to reduce the cost of the bet.  As with a bull call spread, the payoff is always nonnegative, so there must be a net premium paid to put it on (the put that is sold will have a lower price than the put that is bought, so the trader must pay the difference).

::: {#fig-bear-spread fig-height=400}
```{python}
...
A butterfly spread involves buying options at two different strike prices and selling two options at a middle strike price. This strategy profits when the stock price stays near the middle strike (low volatility).  As with the other spreads we have examined, the payoff is nonnegative, so the trader must pay a net premium to put the position on.

::: {#fig-butterfly fig-height=400}
```{python}
#| label: fig-butterfly
...
For European options on stocks that do not pay dividends prior to option expiry, put-call parity states that:
...
The term $Ke^{-rT}$ represents the present value of the strike price - the amount of cash that, when invested at the risk-free rate, will grow to exactly $K$ at expiration.  Here we are using continuous compounding/discounting, so $e^{-rT}$ is the discount factor.  This is a common and convenient way to represent discount factors when considering various fractions of a year.  The appropriate discount rate for a particular period can be imputed from the yields of Treasury bonds at nearby maturities.
...
*1. Option Pricing*: If we know the prices of a call, the underlying stock, and the risk-free rate, we can determine what the put price should be (and vice versa).
...
When you exercise an option early, you capture the intrinsic value but forfeit all remaining time value. Therefore, early exercise is generally only optimal when the time value has become negligible.
...
American calls on dividend-paying stocks present a more complex situation. Early exercise might be optimal just before the ex-dividend date: When a stock pays a dividend, the stock price typically drops by approximately the dividend amount on the ex-dividend date.  Call holders do not receive the dividend, so they may prefer to exercise the call just before the ex-dividend date to capture the dividend.
...
::: {#exr-intro-options-5}
Suppose you have a short position in an asset.  You can protect the position and pay for the protection by collaring the position as in exr-intro-options-1.  There are other options that could be sold to pay for protection (options that do not create a collar).  Write python code to generate the payoff diagrams of the following option portfolios, in conjunction with the short position in the underlying asset: (a) a long call and two short calls with higher strikes, (b) a short call and two long calls with higher strikes.  
:::