...
## Exercise Styles and Contract Features
...
Options also differ by their relationship to the current underlying price:
...
As expiration approaches, options lose **time value** - the portion of their premium beyond intrinsic value. This time decay accelerates as expiration nears, making timing crucial in options trading.

## Motivations for Trading Options

### Speculation and Leverage
One motive for buying options is to speculate on price movements with higher leverage than direct stock ownership. A small price change in the underlying can produce large percentage changes in option value, amplifying both potential gains and losses.

### Hedging and Insurance  
Options serve as insurance contracts for existing positions. For example, owning a protective put on your stock holdings limits downside risk, much like buying car insurance protects against accident costs. The option premium represents the "insurance cost" for this protection.

### Income Generation
Option sellers collect premiums upfront, similar to insurance companies collecting policy premiums. However, this income comes with obligations and potential risks if the market moves against the seller's position.

## Option Positions and Obligations
...
- Have obligations but no rights
- Must fulfill contract if exercised against
- Face potentially unlimited losses (especially for calls)

Most options are closed before expiration through offsetting trades rather than exercise. This allows traders to capture profits, limit losses, or adjust strategies as market conditions change.
...
### Understanding Options Market Data

@fig-options-market-data allows us to browse current market data for exchange-traded options. Understanding each column helps interpret market conditions and liquidity:

**Contract Specifications:**
- **Strike**: The price at which the option can be exercised
- **Expiration**: When the contract expires (shown in the interactive tool's controls)

**Pricing Information:**
- **Bid Price**: Highest price buyers are willing to pay (where you can sell)
- **Ask Price**: Lowest price sellers will accept (where you can buy)  
- **Last Price**: Most recent transaction price
- **Bid-Ask Spread**: The difference between ask and bid (ask - bid)

**Trading Activity:**
- **Volume**: Number of contracts traded during the current session
- **Open Interest**: Total outstanding contracts that haven't been closed
- **Time of Last Trade**: When the option last changed hands

::: {.callout-tip}
## Reading the Bid-Ask Spread

A bid of $2.50 and ask of $2.60 means:
- You can sell the option for $2.50 (bid)
- You can buy the option for $2.60 (ask)  
- The spread is $0.10, representing the market-maker's profit margin
...
**Market options data courtesy of Yahoo Finance.** The left sidebar has user controls for ticker, call versus put, and maturity.  If it is not visible, use the  <i class="bi bi-chevron-right"></i> icon at the top of the figure to open it.  To move around the table, first hover over it to bring up an inner vertical slider.
:::

### How Exchanges Structure Options Markets

**Strike Price and Expiration Cycles**: Exchanges determine which strike prices and expiration dates are available for trading. As time passes and options expire, new expiration cycles are introduced. New strike prices are added when the underlying price moves, ensuring strikes bracket the current market price. The number of available strikes and expirations depends on trading interest and exchange policies.
...
- Limit order at $2.40: Very unlikely to execute unless market falls 
...
**Margin Requirements**: 
Option sellers must post collateral (margin) to ensure they can meet their obligations. This protects the clearinghouse and, ultimately, option buyers from default risk.
...
## Patterns in Options Pricing

Experimenting with @fig-options-market-data reveals consistent patterns in how options are priced relative to each other. Understanding these patterns helps build intuition about option values.

### Strike Price Relationships

**Call Options**: Prices decrease as strike prices increase
- A call with strike $50 is worth more than a call with strike $100  
- Intuition: The right to buy at $50 is more valuable than the right to buy at $100
...
- A put with strike $100 is worth more than a put with strike $50
- Intuition: The right to sell at $100 is more valuable than the right to sell at $50
...
Option prices must respect certain minimum values to prevent arbitrage:
...
These arbitrage bounds ensure that option prices reflect their intrinsic value as a minimum, with additional time value on top.
...
```{python}
#| label: fig-long-call
#| fig-cap: "Payoff diagram for a long call option"
...
```{python}
#| label: fig-long-put
#| fig-cap: "Payoff diagram for a long put option"
...
#| fig-cap: "Payoff diagram for a short call option"
...
```{python}
#| label: fig-short-put
#| fig-cap: "Payoff diagram for a short put option"
...
#| fig-cap: "Protective put: long stock + long put"
...
```{python}
#| label: fig-collar
#| fig-cap: "Collar: long stock + long put + short call"
...
#| fig-cap: "Bull call spread: long low-strike call + short high-strike call"
...
#| fig-cap: "Bear put spread: long high-strike put + short low-strike put"
...
```{python}
#| label: fig-straddle
#| fig-cap: "Long straddle: long call + long put (same strike)"
...
```{python}
#| label: fig-butterfly
#| fig-cap: "Butterfly spread: long low call + short 2 middle calls + long high call"
