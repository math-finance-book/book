...
A common option strategy is to sell and out-of-the-money call option on a stock that one owns.  This generates premium income, at the expense of reducing the maximum gain on the stock (it is capped at the call strike).  Whether this turns out to be profitable depends on how high the underlying goes prior to the option maturity - if it goes up sufficiently far, the trader will regret having sold the call; if it goes up less, or doesn't go up at all, then the trader will be happy to have captured the call premium.  The following is the payoff diagram of a portfolio consisting of a long position in the underlying asset and a short call.  
