\chapter[Fixed Income Concepts]{Fixed Income Concepts}
...
However, in practice, we will typically be unable to find yields $y(\tau_{ij})$ such that @eq-yieldcurve1 holds exactly for each bond $i$.  This is due to measurement errors in the form of bid-ask spreads and nonsynchronous pricing.  Furthermore, even if we can find such yields, we still face the issue of estimating the yields at other maturities $\tau \mathrm{n}otin \{\tau_{ij}\}$.   The cubic spline is one way to address these issues.
...
\vfil\eject
\mathrm{n}oindent the following approximation for the return $\Delta P/P$:
$$
...
$$ \vdots 
$$
$$ z_{iN} \end{pmatrix} = C^\top \begin{pmatrix} \Delta_{i1} 
$$
$$ \vdots 
$$
$$ \Delta_{iN} \end{pmatrix}\;.
$$
Thus,
$$
\begin{pmatrix} \Delta_{i1} 
$$ {#eq-pc2}
...
$$\begin{matrix} \Delta_{i1} 
$$ {#eq-pc3}

$$ \vdots 
$$
$$ \Delta_{iN} \end{matrix} 
\;\begin{matrix}= 
$$
$$ \vdots 
$$
...
$$ \vdots 
$$
$$ \beta_{N1}z_{i1} + \cdots + \beta_{NN}z_{iN} \;.\end{matrix}$$
...
$$ \vdots 
$$
$$ \beta_{N1}z_{i1} + \beta_{N2}z_{i2}+ \beta_{N3}z_{i3} +\mathrm{var}epsilon_{iN}\; ,\end{matrix}$$
where 
$$
...
As in Sect.~\ref{s_duration}, because of convexity, the approximation understates the new price $P' = P + \Delta P$.  In Sect.~\ref{s_duration} we considered this equation assuming equal changes in the yields (a parallel shift in the yield curve).  Here, we will discuss the more general case.
...
:::
::: {#exr-nolabel}
 Create a VBA function \verb!DiscountBondPrice! that takes the time $\tau$ to maturity as an input and returns an estimated price for a discount bond maturing at $\tau$, using the cubic spline formula @eq-cubicspline00, knot points at 2 and 4 years, and the coefficients in the previous exercise.  Confirm that it gives approximately the same discount bond prices as those at the beginning of this set of exercises.  \emph{A Warning about Extrapolating to Longer Maturities}:  What price does the function give for a discount bond maturing in ten years?
:::
::: {#exr-nolabel}
