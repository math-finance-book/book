...
::: {#exr-nolabel}
 Create a python program to compare the estimates of the value of a discretely sampled barrier option given by the functions Down_And_Out_Call_MC created in @exr-e_knockoutmc and the function Down_And_Out_Call_CN.  Allow the user to input $S$, $K$, $r$, $\sigma$, $q$, the knock-out barrier, the number of Monte Carlo simulations, and the number of space steps above $\log S(0)$ in the Crank-Nicolson algorithm.  
:::
::: {#exr-nolabel}
...
Write the system of equations @eq-implicit for the implicit method, together with boundary conditions of the form @eq-crank4aa - @eq-crank4bb as a matrix system and solve for $u_j$ and $b_j$ in @eq-crank101 - @eq-crankC_L, as in the subsection that defines the function CrankNicolson.
:::
