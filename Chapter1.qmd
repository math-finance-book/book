...
Chap. [\[c_continuoustime\]](#c_continuoustime){reference-type="ref"
reference="c_continuoustime"}. We will begin with a brief description of
...
$x\E^{rT}$ dollars at the end of the time period.
...
that the investment will grow in six months to $x\E^{0.5 r}$ and that it
will grow in one year to $x\E^r$. We can interpret this rate as being
...
The risk-neutral probabilities are defined as $\pi_u\E^{rT}$ for the up
state and $\pi_d\E^{rT}$ for the down state. Denoting these as $p_u$ and
...
$p_u = \pi_u\E^{rT}$. Likewise, we can define the probabilities using
...
portfolio at date $T$ will be $a\E^{rT} + bS_x/S$, where
