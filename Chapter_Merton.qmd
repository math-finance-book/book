...
$$R_t = \exp\left(\int_0^t r(s)\,ds\right)\; ,$$
...
Now we turn to fact (2).  Consider a forward contract with maturity $T'$ and a discount bond also maturing at $T'$.  Let $F_t$ denote the forward price and let $P(t,T')$ denote the price of the discount bond at dates $t \leq T'$.  We observed in @sec-s_black that there is a non-dividend paying asset with price $P(t,T')F_t$.  When we use the discount bond as the numeraire, the ratio $P(t,T')F_t/P(t,T') = F_t$ must be a martingale, which is fact (2).  Because of this fact, a probability measure  corresponding to a discount bond being the numeraire is called a forward measure. \index{forward measure}
...
$$\text{prob}^P[A] = \\E\left[1_A\phi_{T'}\frac{P(T',T')}{P(0,T')}\right] = \exp\left(-\int_0^{T'} r_t\,\mathrm{d} t\right)\\E[1_A\phi_{T'}]\; ,$$
...
$$\text{prob}^R[A] = \\E\left[1_A\phi_{T'}\frac{R_{T'}}{R_0}\right] = \exp\left(-\int_0^{T'} r_t\,\mathrm{d} t\right)\\E[1_A\phi_{T'}]\; .$$
...
For example, the result of @sec-s_replicatingquantos on replicating the payoff $\bar{X}S_T$ with forward contracts leads to the following:
