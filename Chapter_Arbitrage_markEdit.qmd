...
# Arbitrage Pricing {#sec-c:arbitrage}
...
1.  If $X$ and $Y$ are time $t$ random cash flows, the time 0 price today of the random cash flow $aX+bY$ is t $a$ times the time 0 price of $X$ plus $b$ times the time zero price of $Y$.  If $P()$ is a pricing function which gives us the price of a random cash flow today, then $P(aA+bB)=aP(A) + bP(B)$.  In other words, the pricing operator is linear.  This follows because if this fails to hold, there is an arbitrage assuming an agent can buy and sell any quantities at these prices.  Since the pricing operator is linear, it must have the form $P(X) = \sum_{s=1}^S \pi_s X(s)$.  We call $\pi_s$ the price of a dollar at time $t$ in state $s$ or a state price.
...
We can think of any security as a portfolio of what are called Arrow securities (in recognition of the seminal work of Kenneth Arrow [@Arrow]).  Since state prices are the prices of these primitive Arrow securities, given these state prices, we can value any security.  The state prices can be found by inverting the pricing relationship for prices of traded securities;  in other words use traded securities to find the prices of securities which pay off a dollar in each time state contingency.  While in general this can be challenging, the procedure is easily illlustrated in the binomial model which we examine next.
...
A somewhat more enlightening description of the state prices is that $\lambda_u$ is the price of an Arrow securtity which pays $1$ if the stock price goes up and nothing if the stock price goes down.  We can solve for a portfolio of the stock and the risk-free asset which has these payoffs.  Let $A$ be the number of shares of stock and $B$ be the amount borrow or lent at time zero.  Then this portfolio will have the desired payoffs if
$$A S_u + B e^{rt} = 1$$
and 
$$A S_d + B e^{rt} =0$$.
...
$$  \frac{X_0}{R_0} = \sum_{s=1}^S p_s \frac{X_s}{e^{r t}} = \E^R\left[\frac{X}{R_t}\right] $$
where $R_0 = 1$, the initial investment in the risk free asset.  In this sense, the discounted value today is the expected discounted discounted value in one period, where the expectation uses the risk neutral probabilities.
...
Up until now, no mention of the true proabilities has been made.    However, another pricing formula which is commonly encountered is to use a stochastic discount factor.  Denote the actual probability of state $s$ as $\text{prob}_s$.If we define $m_s = \frac{\pi_s}{\text{prob}_s}$, we can write 
...
$$  P(X) = \E\left[m e^{rt} \frac{X}{e^rt}\right] = \sum_{s=1}^S \text{prob}_s m_s e^{rt} \frac{X}{e^{rt}} =\E^R\left[\frac{X}{e^{rt}}\right]$$
...
Our valuation formulae work for dividend paying assets or trading strategies which make peiodic payments. We simply sum up the value of the individual cash flows.  If a stock pays dividends $D_t$ up to and including time $T$ and the stock price at time $T$ is $S(T)$ then the stock price is given by
