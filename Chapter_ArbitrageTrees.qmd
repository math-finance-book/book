...
This chapter introduces the change of measure (or change of numeraire or martingale) method for valuing derivative securities. The method is illustrated in a finite-state model and then extended to more general models with a continuum of states.
...
Solving these equations gives $\pi_u  = \frac{S-e^{-rt}S_d}{S_u - S_d}$ and $\pi_d = \frac{e^{-rt}S_u-S}{S_u - S_d}$. Notice that the requirement that $\pi_s > 0$ is precisely the no arbitrage condition @eq-binomialnoarbitrage in the previous section.

A somewhat more enlightening description of the state prices is that $\pi_u$ is the price of an Arrow security that pays $1$ if the stock price goes up and nothing if the stock price goes down. We can solve for portfolios of the stock and risk-free asset that have these payoffs. Let $A$ be the number of shares of stock and $B$ be the amount borrowed or lent at time zero. Then this portfolio will have a payoff of 1 in the up state and 0 in the down state if
$$A S_u + B e^{rt} = 1$$
and 
$$A S_d + B e^{rt} =0\,.$$

Solving these for $A$ and $B$, we get $A= \frac{1}{S_u - S_d}$ and $B = -e^{-rt}\frac{S_d}{S_u - S_d}$. The time zero cost of this portfolio is $AS +B = \frac{S - e^{-rt} S_d}{S_u - S_d} = \pi_u$. A similar exercise reveals that $\pi_d$ is the time 0 price of a portfolio that has a value of 0 in the up state and 1 in the down state.
...
::: Example
Suppose a stock price today is $S=100$, and suppose that the price at a future date $t$ will be either $S_u=110$ or $S_d=90$. The following figure depicts this situation.
...
which can be thought of as the expected discounted value of the future cash flows. The risk-neutral probabilities are so-called since we discount by the risk-free rate. However, the risk-neutral probabilities should not be confused with actual probabilities since they are really prices.
...
Notice the expected return using the risk-neutral probabilities for any investment is the risk-free rate:
